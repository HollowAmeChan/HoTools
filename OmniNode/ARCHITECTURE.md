...
- MC2 self primitive、grid run、broadphase candidate、contact cache与intersect record都属于slot-owned native context。当前已完成Point→Edge→Triangle静态注册、每frame首substep的primitive dynamic更新、grid排序/run构建及EE/PT候选过滤；空contact/intersect容器不得被解释为self-collision求解已启用。
