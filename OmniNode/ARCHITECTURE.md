...
- **应该写**：OmniNode 通用节点模型、编译 IR、执行上下文、runtime cache、调试机制、模块装卸和 native 开发边界。
- **不应该写**：具体业务域的阶段合同、后端选型、完成度、算法、验收计划或提交流水。
- **内容路由**：业务域的稳定事实写入各自 `doc/` 合同或蓝图；实现状态写状态文档；已经失效的设计过程只留 Git 历史。
- **示例原则**：示例只说明通用框架机制，不在本文承担某个业务域的实现记录。
...
- `OmniHostEvaluation.py` 是所有业务域共用的 Blender 宿主求值边界。`frame_change_post` 进入 `frame_evaluation_scope(depsgraph)` 后，adapter 只能复用 handler 传入的短生命周期 depsgraph；帧内禁止 `ViewLayer.update()`、`Context.evaluated_depsgraph_get()` 和其它主动推进宿主求值的调用。渲染期间每帧先调用 `refresh_frame_references("render_frame_start")`，重新绑定 committed owner 持有的数据块引用，再执行所有树；该上下文不进入 compiled graph、Physics World cache 或 native owner；帧外的显式 operator 才能通过同一模块请求主动求值。
...
- `PhysicsWorld/xpbd/` 是一个多任务域 solver 家族，不按对象类型复制顶层解算器菜单。家族层拥有统一模拟步、强类型任务分流、整族失败事务、公共碰撞适配、native 模块装载和通用调试；`simple_mesh_xpbd/` 与 `bone_xpbd/` 只拥有各自对象、拓扑、slot、结果与特有调试。两个域继续使用稳定的 `mesh_xpbd` / `bone_xpbd` 运行标识，不得因目录合并而伪造融合 payload 或共享可变 slot。
...
- 业务逻辑、数据转换、Blender 数据读写、数值计算和 domain 包装，默认都做成函数节点。
...
- 全局临时状态、跨帧滚动状态、运行时中间状态必须暴露为 cache 读写/删除节点，而不是藏在模块全局变量、节点实例字段、函数闭包或 C++ 全局状态里。
...
- root run 成功结束后 pending 才提交；执行失败时 pending 写入和删除都会丢弃，但已经原地修改的 owner 不能由 runtime 回滚。
...
Runtime cache 可以保存普通 Python 值，也可以保存持有外部资源的 owner 对象，例如 GPU buffer、native handle 或长期运行的 domain session。资源型对象不需要强制 tag，也不需要专用 GraphNode；它只需要按需提供 duck-typed 生命周期方法：
...
runtime cache 不负责通用标脏、不负责重建判定，也没有成功提交回调。状态是否脏、是否要重建、如何把当前输入同步到外部资源，都属于生产该 cache 的高封装函数节点或 domain owner 职责。runtime 只负责 owner 的绑定、提交和显式释放，不猜测业务状态。
...
默认的裸 Python value 缓存走**快照隔离**语义：`read_cache` 返回深拷贝，`write_cache` 在进入 pending 时保存私有副本，提交阶段不再重复复制。对小状态没有问题，但对**每帧读改写的大状态**（嵌套 dict、numpy 数组、大量矩阵记录）仍会产生明显深拷贝和递归扫描成本。只有确认状态适合共享可变 owner 时，才使用下面的零拷贝协议。
...
它在所有 `isinstance(x, dict)` 检查下仍是 dict，业务代码无需改访问方式。首轮 replace、后续 mutate 的滚动模式：

```python
if cache_state is None:
    state = OmniCacheOwnerDict(frame=current_frame, records=new_records)
    return _OmniCache.replace(state)

# read 返回 committed 本体（零拷贝）
state = cache_state
state["frame"] = current_frame
state["records"] = new_records
return _OmniCache.mutate(state)
```

连续更新时 runtime 会校验 mutate value 就是 committed owner，不产生 dispose 候选，也不复制或递归扫描 owner 内部。domain 应在自己的产出边界把 payload 规范成 owner；普通 dict 可以浅包装成 `OmniCacheOwnerDict`，已是 owner 或 `None` 原样返回。

注意事项（零拷贝会改变缓存语义，务必权衡）：

- **失去快照隔离**：committed 缓存变成共享可变。如果某次运行中途抛异常，缓存可能留下改了一半的状态；runtime 无法回滚已经发生的原地 mutation。owner 必须在下一次运行前自检 generation、输入签名或 valid 标志，必要时自行重建；需要事务性的状态不要用零拷贝。
- **多读者别名**：若两个 cache read 节点读同一个 key，零拷贝下它们拿到同一对象本体，一个改另一个会看到。需要隔离时仍用裸 value（默认深拷贝）。
- **owner 身份要全程保持**：任何 `dict(state)` 浅重建会退化成普通 dict、丢掉零拷贝身份。要么在产出边界用 domain 自己的幂等 owner 包装 helper 重新包装，要么避免重建。
...
- **范围最小化**：只在确实拥有大状态且能自检的 domain 产出边界改，不要动全局 `cache_replace` / `read_cache` 默认行为，否则会破坏其它缓存使用者的隔离假设。

实现要点：（1）`finish_run` 两阶段提交，`committed_ids` 从每个候选重复扫描降为一次最终状态扫描；（2）`write_cache` 入 pending 时建立私有副本，提交不再二次 `_snapshot_value`；（3）owner 协议让读、写、提交全程复用同一本体。零拷贝省下的是深递归与深扫描，而不是浅拷贝本身。

### 7.2 事务边界与资源回收

一次 root run 只有两种结束方式：

- **成功**：先应用 namespace/key 删除和 pending replace/mutate，再对最终 committed 状态做一次可达性扫描，最后释放已经不可达的旧值。
- **失败**：pending 删除全部取消；本轮新建但未提交的 replace value 以 `run_failed` 释放；committed 绑定保持不变。

两阶段提交的目的不只是性能。一个 owner 可能同时被多个 key 或 namespace 引用，不能在处理第一个替换时立即销毁。runtime 先得到最终 committed 图，再用 `active_ids` 排除仍然可达的对象，并用 `seen` 防止同一对象被重复释放。

失败回滚只对普通值和 pending replace 完整成立。`cache_mutate(owner)` 表示业务代码已经修改 committed owner，本轮失败不能撤销这些修改；这正是零拷贝 owner 必须自检和幂等重建的原因。runtime 不提供 clone、通用 dirty 标志或 commit callback。

同一 key 在一轮执行内被多次 replace 时，被后续写入覆盖的 pending owner 会进入 `discarded_pending`，最终以 `discard_pending` 释放。delete/clear/recompile 分别使用明确 reason，owner 可以记录 reason 做诊断，但不应依赖某个 reason 才保证资源释放正确。

### 7.3 缓存失效与 key 设计

缓存优化的核心风险是**结果已经失效，但 key 或 owner 的业务指纹没有变化**。新增或优化缓存前，先按失效频率分类：

| 数据类别 | 典型失效条件 | 正确策略 |
|---|---|---|
| 当前调用临时值 | 每次执行 | 放寄存器或局部变量，不进 runtime cache |
| 同一帧可复用快照 | frame 或输入版本变化 | key 包含 frame/version，只保留当前窗口 |
| 结构派生数据 | 数据结构、拓扑或 schema 变化 | owner 内维护结构指纹，变化时局部重建或 replace |
| 跨帧滚动状态 | 每次运行推进 | 小状态用快照 replace；大状态且可自检时用 owner mutate |
| 外部资源 | owner 更换、删除、清空、重编译不兼容 | 实现 dispose，所有权随 committed 绑定移动 |

判定 cache key 是否正确的经验法则：**key 或 owner 指纹必须覆盖所有会导致结果变化的输入维度**。如果一个输入会变、但没有进入任何失效条件，就是一个“改了不生效”的潜在回归。Blender 数据块指针通常只代表对象身份，不代表其内容版本，不能单独证明派生结果仍然有效。
...
和缓存失效边界并列的另一个框架级原则：**同一个语义只能有一个真值来源**。当一份配置既能从 A 处推断、又能从 B 处标记时，A 和 B 迟早会不一致，其中一个会腐烂成误导性的死数据。运行时推断（节点输入、已有数据、结构）和持久标记（Blender 属性）尤其容易脱钩。
...
- runtime cache 保存节点业务状态，例如上一轮计算结果、滚动状态和外部资源 owner。
...
- 编译缓存命中时没有发生编译，不清 runtime cache；因此 `run_frame_cached()` 的正常逐帧路径不会重置业务状态。
...
- 校验 Blender 数据和输入版本。
- 把输入整理成窄 spec、数组或 buffer。
- 管理 runtime cache、owner 生命周期和重建条件。
- 批量读取 Blender 数据、消费 result，并执行写回计划。

C++ 侧负责：

- 只处理已经整理好的数组和标量参数。
- 原地更新数值 buffer，或持有由明确 owner 管理的 native context。
- 不直接访问 `bpy`，不保存 Blender 对象指针。
- 不保存隐藏的跨帧全局状态；持久状态必须挂在显式 runtime owner 上，并实现可观察的 dispose/debug 协议。
...
正确分层是固定结论：**Python 管调度和 Blender 边界，C++ 管经测量确认的数值与批处理热点**（domain kernel、persistent native context、批量转换）。一次性构建不因“能迁移”就进入 C++；只有代表性数据证明它是瓶颈时才调整边界。
...
执行器实现了“输入未变则跳过节点”的懒求值：连续运行时只有输入真正变化的节点才重新执行，参数类节点在用户不调参时被跳过并复用上轮输出。每轮必须推进或产生副作用的节点通过 `always_run` 绕过跳过。
...
- **always_run**：`@omni(always_run=True)` 由 `FunctionNodeCore` 识别，`OmniCompiler` emit OpCall 时写入 `has_always_run`（默认取 `is_output_node`）；编译期 `_compute_has_always_run` 递归标注 `CompiledGraph.has_always_run_node`。滚动状态、Cache、输出和 debug 等每轮必须执行的节点需要显式标注。
...
取得 overlay 采样权的普通函数节点会在调用期间获得一个可选 `OmniNodeTiming.current()` session。节点或 domain 可以用 `checkpoint(显示文字)` 切分连续阶段，也可以用 `record(显示文字, seconds)` 发布已经由自己的 profiler 测得的耗时。显示文字由节点原样提供；通用层不维护 domain 阶段标签表，只负责排序、合并和显示。

这条端口不替业务节点决定是否需要常开诊断 socket。复杂 domain 可以用默认关闭的开关创建自己的 profiler，再把同一次阶段测量通过 `record()` 扇出到当前可选 session，禁止为浮层重复测量。诊断开关只控制观察，不进入业务设置指纹、缓存身份或重建判定，也不能读取树 RNA 或直接调用绘制模块。
...
部分复杂功能节点有自己的 `debug_output` 输入。这类日志用于观察节点内部阶段，例如 validate、cache、transform、native、write。它和树级 runtime timing 互补：
...
OmniNode/Tree/TopTreeName
  ...
  OmniNode/step1:ComputeNode:run_native_step
...
1. 明确公开语义和唯一 owner，确认不是在堆叠第二套产品入口。
2. Python 层把 Blender 数据整理成 spec / buffer，不让 C++ 直接碰 `bpy`。
3. C++ 桥接层只暴露窄接口。
4. C++ 核心只处理经测量确认的数值或批处理热点。
...
- 先用 timing/benchmark 证明热点确实在可迁移的数值或批处理区域。
- 保留一个公开函数节点和一套 cache/错误合同；Python 继续负责 Blender 边界与 owner 生命周期。
- native 桥接只暴露窄 spec/buffer 接口，不直接接触 `bpy`。
- reference 实现可以存在于测试或审计工具中，但不作为第二套用户节点长期维护。
- 不做隐式全局 backend 开关，也不暴露只为技术实现差异服务的重复节点。
