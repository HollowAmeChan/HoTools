...
- 后续 solver 迁移时，VRM 骨链对象、MC2 设置对象、Jolt 批量生成约束等都应使用 `implicit_objects`，solver step 在 prepare 阶段读取声明的 tag。跨模块识别名称必须集中定义在 `physicsWorld/names.py`，包括 solver id、slot kind、result channel、exchange channel、backend resource key 和 implicit object tag，不能在注册节点和 solver 内各写一份字符串。
