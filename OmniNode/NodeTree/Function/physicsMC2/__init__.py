...
@omni(
    ...
    input_init={
        ...
        "gravity_falloff": {
            "min_value": 0.0,
            "max_value": 1.0,
            "description": "MC2 重力衰减：根据初始重力方向与当前姿态夹角削弱重力。",
        },
        "stablization_time_after_reset": {
            "min_value": 0.0,
            "max_value": 1.0,
            "description": "MC2 重置后速度稳定时间；重建或重置后 velocityWeight 从 0 恢复到 1。",
        },
        "blend_weight": {
            "min_value": 0.0,
            "max_value": 1.0,
            "description": "MC2 物理结果混合权重；最终显示位置在 BasePose 与物理显示位置之间混合。",
        },
        ...
        "anchor_inertia": {
            "min_value": 0.0,
            "max_value": 1.0,
            "description": "MC2 Anchor Influence：0 完全剔除 Anchor 运动惯性，1 不剔除。",
        },
        ...
        "animation_pose_ratio": {
            "min_value": 0.0,
            "max_value": 1.0,
            "description": "MC2 Animation Pose Ratio：0 使用初始姿态距离，1 使用当前 BasePose 动画距离。",
        },
        ...
        "normal_axis": {
            "min_value": 0,
            "max_value": 5,
            "description": "MC2 法线轴：0右，1上，2前，3负右，4负上，5负前。",
        },
    ...
    输入 mesh 永远是被直接驱动的低模代理；解算器不做减面或高低模映射。
    状态在世界空间中计算，并沿用 SpringBone 风格的连续帧语义：只有下一个连续帧
    可以继承 cache 中的速度继续推进。
    """,
)
def meshClothMC2(
