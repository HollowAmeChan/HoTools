        ...
        for frame in range(1, 601):
            ...
            if owner is None:
                ...
                np.testing.assert_allclose(
                    ...
                    np.asarray(gravity_direction, dtype=np.float32)
                    / np.linalg.norm(gravity_direction),
                    rtol=0.0,
                    atol=1.0e-6,
                )
                np.testing.assert_allclose(
                ...
                np.testing.assert_allclose(
                    values["gravity_falloff"], gravity_falloff,
                    rtol=0.0,
                    atol=1.0e-6,
                )
            else:
...
def test_bone_product_gravity_axes_falloff() -> None:
    ...
    first_digest, first = _run_bone_gravity_case(
        0,
        gravity_direction=direction,
        gravity_falloff=0.35,
    )
    second_digest, second = _run_bone_gravity_case(
        1,
        gravity_direction=direction,
        gravity_falloff=0.35,
    )
    ...
    x_digest, x_axis = _run_bone_gravity_case(
        2,
        gravity_direction=(1.0, 0.0, 0.0),
        gravity_falloff=0.0,
    )
    z_digest, z_axis = _run_bone_gravity_case(
        3,
    ...
    print(
        "MC2_BONE_PRODUCT_GRAVITY_DIGESTS",
        first_digest,
        x_digest,
        z_digest,
    )
