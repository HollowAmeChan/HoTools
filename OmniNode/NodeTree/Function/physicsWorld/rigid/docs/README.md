...
HoTools 当前支持 `FIXED / POINT / DISTANCE / HINGE / SLIDER / CONE / SWING_TWIST / SIX_DOF`。其中 SixDOF 已接显式与生成约束、六轴 Free/Fixed/Limited、范围和专用调试绘制，逐轴 motor/friction 仍待接。Jolt 原生还提供 Path、Gear、RackAndPinion、Pulley 和 Vehicle；这些类型尚未进入 HoTools 的公共 spec、binding 和调试绘制器。
