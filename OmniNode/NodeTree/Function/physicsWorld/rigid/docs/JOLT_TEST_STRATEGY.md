...
当前 S1 已验收 body 积分/阻尼/速度上限/DOF、shape offset/rotation、八种约束的基础语义、Distance/Hinge/Slider 数值行为、SwingTwist 摆角/扭转限制/摩擦/双 motor、SixDOF 六轴 Free/Fixed/Limited、逐轴 friction 与逐轴 motor、动态-动态反作用、碰撞恢复/摩擦/filter/CCD，以及 contact 状态机和 RayCast 几何语义。复杂 Cone/SwingTwist A/B frame 组合、SixDOF 逐轴 spring、Blender semantic runner、跨 ABI 报告和 golden 尚未实现，不能据此宣称完整 Jolt 语义通过。
