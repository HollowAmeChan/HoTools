...
_PROFILE_INPUT_INIT = {
    ...
    "teleport_mode": _profile_input("Teleport处理模式。\n0:None\n1:Reset：重建状态并清速度\n2:Keep：整体搬运现有状态", min_value=0, max_value=2),
    "teleport_distance": _profile_input("组件位移触发阈值。\n实际阈值=输入值x当前Scale。\n与旋转条件是OR关系。", min_value=0.0),
    "teleport_rotation": _profile_input("组件旋转触发阈值，单位为度。\n与位移条件是OR关系。", min_value=0.0),
