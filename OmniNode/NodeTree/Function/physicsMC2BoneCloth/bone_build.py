...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#
# 分岔是指某根骨骼拥有多个子骨的情形。无论分岔发生在哪一层，
# 分岔点骨骼都应当被视为"锚点"：位置锁定到动画，旋转跟随子骨方向。
# 这与 depth=0 链首固定骨的行为完全一致，原因相同：
#   — 分岔点有多个子骨朝向不同方向，没有唯一的"物理运动方向"可以跟随
#   — 让其位置自由漂移会导致所有子分支的起点漂移，产生连锁不稳定
#   — 锁位置 + 旋转写回，是在视觉正确性与物理稳定性之间的合理权衡
#
# 分岔的两种情形及当前支持状态：
#
# ① 顶层分岔（root_bone 有多个直接子骨）—— 已原生支持
#
#     SkirtCtrl（root_bone 输入）
#     ├── Skirt_A_01  ← 自动成为独立链的 depth=0，位置锁定，旋转写回 ✓
#     ├── Skirt_B_01  ← 同上 ✓
#     └── Skirt_C_01  ← 同上 ✓
#
#   用户只需把所有链头的共用父级填入根骨骼输入，每个子骨自动成为
#   各自链的 depth=0 链首固定骨，享有正确的锁位置 + 旋转写回行为。
#
# ② 链内分岔（depth≥1 的骨骼有多个子骨）—— 当前不支持
#
#     Skirt_A_01（depth=0）
#     └── Skirt_A_02（depth=1）
#         ├── Skirt_A_03_L（depth=2）← 只追踪此分支
#         └── Skirt_A_03_R（depth=2）← 被忽略 ✗
#
#   _pose_bone_chain_names 遇到分叉只跟随 children[0]，其余子骨及其
#   后裔不进入模拟。分岔点骨骼（Skirt_A_02）作为普通可动骨处理，
#   不会被自动降级为锚点。
#
#   处理链内分岔的正确方式（用户侧）：
#     在分岔点骨骼处截断原链，将分岔点骨骼作为新的 root_bone，
#     把它的每个子分支分别作为独立链填入根骨骼列表。
#     这样分岔点骨骼自动变成共用父级（不入模拟），其所有子骨
#     各自成为新的 depth=0 链首固定骨。
#
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
def _pose_bone_chain_names(root_pose_bone) -> list[str]:
    ...
    遇到分叉只跟随 children[0]，不采集其他分支。
    原因：BoneCloth 的横向连接依赖”每条链是清晰的纵向序列”；
    且链内分岔点若作为可动骨自由漂移，会使所有子分支起点不稳定。

    【链内分岔的正确处理方式】
    在分岔点处截断：
      — 把分岔点骨骼本身作为新的根骨骼（root_bone）输入
      — 其每个子分支分别作为独立链
      — 分岔点骨骼自动成为共用父级（不入模拟），子骨各自成为 depth=0 链首固定骨
    参见模块顶部”分岔处理原则”一节。
    """
    names: list[str] = []
