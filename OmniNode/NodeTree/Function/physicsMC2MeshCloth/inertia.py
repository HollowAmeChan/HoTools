"""MC2 帧惯性 Center/Inertia 实现

处理 MeshCloth 每帧世界空间的惯性状态，包含 teleport 检测、anchor 对象修正。
depth inertia 按粒子深度缩放惯性强度；BasePose 代理模式下跳过对象位移惯性，由 solver 直接处理。
world inertia 控制整体对象运动传递到粒子的比例：0 = 完全跟随，1 = 完全惯性滞后。
"""
