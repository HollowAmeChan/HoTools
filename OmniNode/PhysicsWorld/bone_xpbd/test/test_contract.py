...
specs = importlib.import_module(
    "HoTools.OmniNode.PhysicsWorld.bone_xpbd.specs"
)
...
def test_registry_discovers_bone_xpbd_as_independent_solver_domain():
    assert "bone_xpbd" in registry.builtin_solver_domains()
    descriptor = registry.all_solver_module_descriptors()["bone_xpbd"]
    declaration = registry.resolve_solver_declaration("bone_xpbd")
    assert descriptor["menu_name"] == "Bone XPBD"
