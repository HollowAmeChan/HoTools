...
27. self narrowphase/contact geometry 保留为E6前重大优化项，不因broadphase收紧而关闭。当前每个候选分别执行Edge-Edge最近线段或Point-Triangle最近点、预测位移阈值、法线/符号与half量化，接触更新和四轮求解还会重复部分几何计算。后续必须先把两类closest、半径/位移快速拒绝、法线/量化和各轮contact update细分计时，再评估平方距离上界早退、SoA/SIMD批处理及成熟C++几何内核；外部库只能作为同fixture对照实现，不能改变退化分支、float32/half结果、稳定顺序或CPU reference合同。
