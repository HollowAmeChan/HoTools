...
| Bone XPBD | 领域边界已确定，尚未实现 | 双端 fixed、显式 head/tail、双边界 stretch/bend/weld、Tail 吸附和场消费已从 MC2 改造范围移出；复用 Mesh XPBD 数值核心与 PhysicsWorld 生命周期的路线见 `MC2_BONECLOTH_BILATERAL_ENDPOINT_PLAN.md` | 先冻结 `BoneSegment`、端点复用/weld、Pin、碰撞/场注册和多任务域契约，再开始实现 |
