...
最新 E7-S forwarder 批次删除了 Mesh domain draft 纯类型别名、两个 setup 名称 wrapper、产品 solver 私有 slot-id wrapper，以及只供测试使用的单 fragment compiler wrapper；统一 collector/collider/slot identity 与集合 compiler 直接成为唯一入口。生产模块保持 69 个，已分类 forwarder 由 84 降为 80；Blender 5.2 mixed-output 900 帧 digest 不变，Domain E3 golden 全部通过。
