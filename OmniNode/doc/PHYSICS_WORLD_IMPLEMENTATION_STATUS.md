...
1. Bone XPBD 的独立 Armature Pose Pin 硬锚、完整目标图优先与统一 basis 反算、共享点 Pin 冲突预检、通道 basis restart 重建、Pin 静态切换、独立反馈指纹、双 ABI Moving Pin 子步轨迹、Blender 双端 fixed/反馈/生命周期回归和产品工程高速自动 soak 已完成；当前明确拒绝所选 PoseBone Constraint/IK。发布前先完成交互式剧烈甩动复测，再固化可重复 13 点探针。维持统一 `XPBD模拟步`的强类型分流和每 task 独立 slot/context。Field 只能走后续 native 子步扩展，不能增加 Python sampler；跨 solver 同骨 owner 仲裁必须作为 Physics World 公共合同另行落地。multiscale chord/rod 只按量化残差决定，见 `BONE_XPBD_BLUEPRINT.md`。
