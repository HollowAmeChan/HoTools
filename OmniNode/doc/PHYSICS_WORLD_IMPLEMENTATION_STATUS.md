...
| Rigid/Jolt | 可用基线 + Object Voronoi 破碎链 | 7 种基础 shape、11 种约束、均匀三维 Voronoi 封闭单元 + GN Boolean 预览、固定 Piece ID、按体积分配质量、显式刷新、Source/Piece manifest、Scene 根/独立 Collection resolver、初始 `DontActivate`、接触自动唤醒、列式结果/Collection 批写回；内置墙体和用户工程派生 `.blend` 均通过 | 下一阶段优先补作者 Mesh `CONVEX_HULL` 和 glue/锚定图，再做局部传播、显式属性与世界设置；Full Mesh/动态 GN 实例后置；见 `JOLT_PHYSICS_BACKGROUND_ANALYSIS.md` 与 `PhysicsWorld/rigid/docs/RIGID_FRACTURE_BLUEPRINT.md` |
...
3. `FULL_MESH_STATIC` 与作者 mesh 的 `CONVEX_HULL` 独立立项；不在此阶段做自动凸分解或运行时 GN 拓扑生成。
