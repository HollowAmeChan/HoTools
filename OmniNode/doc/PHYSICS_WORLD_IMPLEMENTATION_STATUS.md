...
MC2 E7-CPU基础数值oracle迁移（2026-07-23）：删除前由py313同输入V0/DomainV1已通过对照采集最小JSON golden，新增不导入`native_context/specs/solver`的`test_domain_reference_golden.py`。前三组已直接锁定Integration prediction、无碰撞完整混合pass、Post history、StepBasic、Angle/Motion，以及5种两帧Center事务的float32位置/旋转/速度、native shift、Teleport flags、暂停零子步和逐字段原容差；对应七个测试函数已从`test_e3_v0_tolerance.py`删除。Center长期测试直接使用Domain自有history，不冻结旧`derived_center_pose/read_center_step`包装schema。新Domain golden `7/7`；旧文件只余点/边/自碰三个运行时V0对照。
