...
MC2 产品 Center stabilization 参数迁移（2026-07-23）：`center_stabilization_controls` 在三种 setup 上以 Reset 后 `stabilization_time_after_reset=0.2`、`blend_weight=0.6` 与基线 `0.0` 配置各跑 600 帧双跑；它从 compiled parameter SoA 验证真实产品参数值，并锁定 Reset 后 real velocity 轨迹有限且确定。产品 Center debug ABI 没有旧 V0 的内部 `velocity_weight` 字段，因此本项只声明参数配置与行为确定性，不声明旧 ramp 公式等价；旧 `stabilization_blend_ramp_exact` 仍待专门产品观察面迁移。仅使用 Blender 5.2/Python 3.13，4.5/py311 继续冻结。
