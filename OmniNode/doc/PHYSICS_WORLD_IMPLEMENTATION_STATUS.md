...
MC2 状态补充：Bone Line 已从 private candidate 推进到稳定 armature/data pointer + bone name 的 public `bone_transform_batch`，整批 parent-local `matrix_basis` plan 由统一 PoseBone writeback 消费。当前剩余结果门槛是 writeback 执行中途失败时整批恢复旧 basis；Automatic/Sequential、negative scale 与碰撞边界不变。
