...
MC2 E7-CPU基础数值oracle迁移（2026-07-23）：删除前由py313同输入V0/DomainV1已通过对照采集最小JSON golden，新增不导入`native_context/specs/solver`的`test_domain_reference_golden.py`。首两组已直接锁定Integration prediction、无碰撞完整混合pass、Post real-velocity history、StepBasic pose、Angle Restoration、Tether→Motion和完整Angle Limit+Motion流水线的float32位置/旋转/速度、shape、有限性与逐字段原容差；对应六项已从`test_e3_v0_tolerance.py`删除。新Domain golden `6/6`；旧文件只余Center一个参数化函数和点/边/自碰三个函数，共8次运行时V0对照。golden只保存有独立价值的数值输出，不保存旧context schema或调试快照。
