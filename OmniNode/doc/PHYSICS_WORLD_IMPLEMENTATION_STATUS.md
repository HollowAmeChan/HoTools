...
| MC2 | 维护态；debug/长时矩阵审计进行中 | 旧实现删除、Python/C++重组、依赖/ABI/事务终审、单一蓝本及分阶段热点benchmark均已关闭；三setup、逐帧all-task step、每中控骨独立Bone产品拓扑、自动跨物体self、单一派生self厚度、全隐式debug、官方预设及C++自产自用成立；细分debug已验收；Angle Restoration、Motion、外碰scope、Distance/Tether、Triangle Bending、Angle Limit、Center/Keep Teleport和跨task self已完成600-1800帧专项 | 继续补齐BoneCloth/BoneSpring输出及三setup混合soak，随后终审矩阵实际case覆盖。Spring/wind仍为隐藏无native行为字段，不得先开放 |
