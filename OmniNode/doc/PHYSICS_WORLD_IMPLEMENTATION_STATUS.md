...
2. 按`MC2_ACCEPTANCE_MAP.md`先完成MC2替代资格审计；未取得“允许删除”结论前保留旧实现作为语义、性能和依赖审计输入。
   P-06d当前已用单一`MC2BoneRawSnapshot`和同task同Armature `foreach_get`消除重复rest读取/冻结树，rest frame、vertex-to-transform、children/baseline/pose-depth已迁入C++ bulk producer，Baseline以10组owner直接move进context；native注册后完整Bone tuple bundle已压缩为仅含identity/debug/count/signature的metadata，large Bone首建约74.96ms、热帧约6.13ms。下一步收口Distance/Center/Self owner，再迁移Bone registration剩余8组数组。
