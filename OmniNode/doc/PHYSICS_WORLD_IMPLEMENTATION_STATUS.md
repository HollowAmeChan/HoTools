...
| MC2 | Mesh native no-collision + public GN result foundation | 唯一 solver id与三种 setup；slot reuse/rebuild/prune；source-aligned proxy/baseline/Distance/Bending/Center immutable contract、签名与只读 packer；完整 Mesh connectivity token进入 static dirty signature；N2 value ABI与独立Team options热更新ABI；World frame context保留raw/effective dt；N3 reset/continuity、old/current frame interpolation、source world linear与 evaluated component/anchor pose snapshot；per-slot staged native context；Center fixed/component frame adapter、persistent reset/step/pause/anchor/smoothing/scale-direction history、Move depth inertia与 animated step-basic scratch；单位正缩放world/limit/anchor/smoothing、正/零time scale、Fixed-derived Center、configured Keep/Reset teleport与无parent/shear的X轴negative-scale teleport已由12个 Tier A Host对拍、native frame-shift/negative-matrix/reset ABI、Blender显式第4至9帧、独立Keep frame 41、独立Reset frame 51、自动BasePose frame 12及独立Fixed World闭环；Keep在smoothing前检测并强制100% shift，Reset执行staged Center/native/Host particle reset后继续本帧三子步；MC2 scheduler job/updateCount/skipCount/step ratios与Center consumer已由Tier A冻结，settings显式提供源码90Hz与每帧上限3，slot scheduler现驱动一次frame shift或configured reset、完整Center上传和N次native子步，Blender 50Hz/0.1秒已闭环5/3/2、三step、两次interpolation update、4/36度skip shift及单candidate发布；Fixed World额外覆盖正缩放baseline parent-first重建、三子步animated blend与animation pose ratio热关闭；zero scale仅推进dynamic/candidate并应用shift，不执行native/Center step；gravity/damping prediction、Pin、ordered Distance、Bending fixed-point scratch、persistent velocity；公共Mesh result transaction与GN writeback闭环；双 ABI/soak。Bone/stats channel仍仅 planned | configured teleport/negative-scale组合、baseline与负缩放组合、父级/shear负缩放、Bone connection/topology/output、collision/self-collision |
...
2. MC2 按 `MC2_SOURCE_ALIGNMENT_EXECUTION_PLAN.md` 继续收口configured teleport/negative-scale组合与baseline/negative-scale组合；禁止按旧 solver 或未验证近似扩展 spec。
