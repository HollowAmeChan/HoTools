...
| Bone XPBD | experimental vertical slice 可用，尚未冻结 | 面板对象、自定义对象、任务、可视化调试四个 Bone 节点已注册；任务强类型接入现有 `XPBD模拟步`，与 Mesh task 共用用户调度入口但各持独立 slot/native context；显式 `BoneSegment(head_particle, tail_particle)`、rest 几何共点共享且禁止传递折叠同一骨段、无 depth/父级方向、`use_connect=True` 注册拒绝、Pin 双端、默认开启且可关闭的 Tail 吸附；只走共享 native XPBD context，无 Python solver fallback；公共 Bone 写回以 pending/confirmed + 成功 receipt 隔离反馈，事务失败零部分写回且不前移指纹；调试请求一次性消费并读取真实运行快照；py311/py313 隔离构建与原生回归均通过，py313 已部署 | 已安装 py311 因运行中 Blender 文件锁尚未覆盖，当前 4.5.8 安装路径不能使用 moving Pin ABI；当前外碰为 task 统一半径/16-bit mask；Field Wind 仅为未来 native 子步扩展；独立 `2N+weld`、multiscale chord 与 rod/shape matching 未实现；跨 solver 同一 PoseBone owner 仲裁仍是后续公共契约，当前图必须避免重复目标；继续固化双端 fixed/反馈/生命周期矩阵和 13 点数值探针，再决定约束扩展；见 `BONE_XPBD_BLUEPRINT.md` |
...
运行态调试合同：只有明确请求的专用调试节点才可读取 live native 状态；当前“场-运行可视化调试”核对 World FrameContext、runtime inspect 与同签名 snapshot，关闭时不读 cache、不采样也不发布绘制批次，高级 scope 缺少消费上下文时只画边界。全局 draw handler 首次请求时懒注册，在 World Begin、store 替换和 world dispose 之间保持稳定并允许空 store 休眠，只在 load/unregister 的组件停机边界注销，避免可见 View3D 正在回调时移除句柄。Physics World 已增加统一宿主求值门禁：`OmniHostEvaluation` 管理 `frame_change_post` 的短生命周期 depsgraph，渲染期间每帧先通过 `OmniReferenceGuard` 刷新 committed owner 的数据块引用；Field/MC2 adapter 在帧内只复用该上下文，Simple Cloth/BasePose 校验及 Bone/Jolt restart 清理只读当前状态或打更新标签，不再重入 `rna_ViewLayer_update_tagged` 或 `Context.evaluated_depsgraph_get()`；native runtime 替换立即从 registry 摘除，shared_ptr 只保护在途调用，不在 World 中积累退休 runtime。4.5.8 下“播放中复制场、编译、删除复制对象”的 CDB 精确复现已通过。碰撞、接触和其它裸读能力未来必须各自提供同类专用节点，不能由作者预览、Blender RNA 或通用快照猜测。

## 当前优先级

1. Bone XPBD 先完成双 ABI moving Pin 回归、Blender 双端 fixed/反馈/生命周期矩阵与可重复 13 点探针；维持统一 `XPBD模拟步`的强类型分流和每 task 独立 slot/context。Field 仍是后续 native 子步扩展，不能增加 Python sampler；跨 solver 同骨 owner 仲裁必须作为 Physics World 公共合同另行落地。multiscale chord/rod 只按量化残差决定，见 `BONE_XPBD_BLUEPRINT.md`。
2. Field 公共 authoring/preview、MC2 CPU 产品接入和三 setup 的600帧长期能力矩阵均已成立；当前只冻结 Volume 衰减与参与权重的后续权责，不再恢复旧 MC2 wind 参数。
3. MC2 E6 只允许新增独立 GPU backend。CPU DomainV1 的算法、状态、ABI、加载和性能是冻结基线，GPU 工作不能以任何理由造成 CPU 回归。
4. E6 先建立可选 provider、CPU-only 加载隔离和固定 fixture 闭环，再实现 whole-domain self 与完整 mixed pass；不能从产品节点直接开始拼 GPU 分支。
