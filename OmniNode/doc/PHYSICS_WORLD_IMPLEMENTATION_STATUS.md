...
| Rigid/Jolt | vertical slice 可用，P0 语义门禁收口中 | body/constraint spec、约束引用拓扑、Jolt resource、scope hook、result/writeback、query/event/debug、dispose；S1/S2/S3 58 fixture、py311/py313 自动容差差分、`max_bodies` 容量溢出、接触事件有界 overflow 与 clear 恢复 | soak、性能、golden；Path、剩余高级 shape/query 的 binding、native、debug 和 fixture 同步 |
