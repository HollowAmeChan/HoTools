...
| Mesh XPBD | Physics World vertical slice 可用，尚未完成生产冻结 | 面板对象/自定义对象输入独立任务，source Mesh topology/reference、累计 lambda nanobind context、四类公共 collider、slot/debug、事务化 GN result/writeback；不建立无运行语义的融合域 | 完成生产场景、时间矩阵、dirty、dispose 与 soak 验收，再删除旧双节点/私有 cache 与写回；见 `MESH_XPBD_BLUEPRINT.md` |
