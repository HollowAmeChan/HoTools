...
2. 按`MC2_ACCEPTANCE_MAP.md`先完成MC2替代资格审计；未取得“允许删除”结论前保留旧实现作为语义、性能和依赖审计输入。
   P-06d已关闭：单一`MC2BoneRawSnapshot`与同task同Armature `foreach_get`消除重复rest读取/冻结树，production assembly与rest/finalizer/baseline/constraint派生保持连续数组；Proxy/Baseline/Distance/Center/Self/Bone registration共6次直接move，完整spec/packer仅oracle可达，slot仅保留metadata。large Bone首建约18.06ms、热帧约6.15ms。P-06e已建立真实变化基线：large Bone rest重建约18.97ms，仅gravity direction变化因全量重建仍约12.24ms；当前执行native静态复用与Center-only配置重建。
