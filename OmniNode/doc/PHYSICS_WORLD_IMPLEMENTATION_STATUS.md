...
2. 按`MC2_ACCEPTANCE_MAP.md`先完成MC2替代资格审计；未取得“允许删除”结论前保留旧实现作为语义、性能和依赖审计输入。
   P-06d当前已用单一`MC2BoneRawSnapshot`消除Bone fingerprint/topology/static的重复rest读取和生产路径`_thaw`，并以同task同Armature的`foreach_get`批量读取取代逐source逐骨属性访问；下一步迁移Bone派生static所有权。
