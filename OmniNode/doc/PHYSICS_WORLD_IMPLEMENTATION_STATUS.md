...
| MC2 | 旧 solver 已移除；人工验收复核重新打开 | Python/C++重组、依赖/ABI/事务终审、单一蓝本及热点benchmark已完成；单层自碰、Mesh深度、参数归属、无consumer离心力隐藏与任务级Teleport自动矩阵已关闭；五类约束的六个有序pass已按需发布真实逐粒子correction | 用真实高速collider复验Teleport；把pass correction推进到记录级active/near-limit与一级视图；补Center分层量、接触时间层及兼容重编译缓存。统一入口：`MC2_MANUAL_VALIDATION_DECISIONS.md` |
