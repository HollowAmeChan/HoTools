...
| MC2 | 生产V0可用；统一粒子域E4进行中 | 既有产品数值/debug验收保持闭环；E0-E3、P0、P1-B、粒子级覆盖、partitioned StepBasic、compiled whole-domain self/external及native-owned完整pass已完成；collector draft可从Physics World公共snapshot一次生成whole-domain immutable collider POD，排除全部域内owner但不按任一partition mask预筛选，并由native逐partition过滤；完整Tier A fragment已有按快照签名与重力方向的两阶段事务缓存，失败stage不改变live cache；py311/py313均有缓存、point/edge允许与阻断、真实位移、非法输入零pass及E3标量ABI等价证据；1600粒子120稳定样本的static observation p95为`0.171 ms`，Bone静态观察仍保守全扫 | 下一入口是把fragment cache提交点与native domain staged replacement合并为产品fused owner，再接domain collider POD；E5多目标事务和产品collector通过后才切换owner并执行E7-CPU清理 |
