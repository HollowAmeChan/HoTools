...
| MC2 | 维护态；debug/长时矩阵审计进行中 | 旧实现删除、Python/C++重组、依赖/ABI/事务终审、单一蓝本及分阶段热点benchmark均已关闭；三setup、逐帧all-task step、每中控骨独立Bone产品拓扑、自动跨物体self、单一派生self厚度、全隐式debug、官方预设及C++自产自用成立；简单布料RNA公开enabled/BasePose/半径组/Pin/碰撞组，缺失BasePose在automatic首帧按空pointer一次性创建 | 关闭Motion/Angle/Output/每task collider细分debug门禁；按代码级矩阵补齐各约束族、参数群和solver能力的600-1800帧专项及混合soak。Spring/wind仍为隐藏无native行为字段，不得先开放 |
