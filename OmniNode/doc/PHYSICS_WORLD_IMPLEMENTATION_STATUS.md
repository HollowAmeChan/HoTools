...
| Physics Bake | Bone + Mesh vertical slice可用 | `物理烘焙` OmniNode、单条/batch Bone result精确解析、专用Action复制/创建/复用、三种Bone rotation mode、真实GN target解析、双modifier live/cache、每对象目录、原子manifest、首个Mesh pass记录Action、播放留存分离、schema 2迁移 | Bone component ownership、Object Action、Clear节点、边界首帧回填/暂停、取消/进度、delete/pack/unpack与多Mesh性能门槛；总合同见`PHYSICS_BAKE_NODE_BLUEPRINT.md` |
...
1. 推进 Physics Bake 的 Bone component ownership、Object Action 与独立 Clear 节点，同时保持现有 Bone/Mesh 留存合同。
