...
| MC2 | Mesh native no-collision + public GN result foundation | 唯一 solver id与三种 setup；slot reuse/rebuild/prune；source-aligned proxy/baseline/Distance/Bending/Center immutable contract、签名与只读 packer；完整 Mesh connectivity token进入 static dirty signature；N2 value ABI；World frame context保留raw/effective dt；N3 reset/continuity、old/current frame interpolation、source world linear与 evaluated component/anchor pose snapshot；per-slot staged native context；Center fixed/component frame adapter、persistent reset/step/pause/anchor/smoothing/scale-direction history、Move depth inertia与 animated step-basic scratch；单位正缩放world/limit/anchor/smoothing、正/零time scale、Fixed-derived Center与无parent/shear的X轴negative-scale teleport已由9个 Tier A Host对拍、native frame-shift/negative-matrix ABI、Blender显式第4至9帧、自动BasePose frame 12及独立Fixed World闭环；zero scale仅推进dynamic/candidate并应用shift，不执行native/Center step；gravity/damping prediction、Pin、ordered Distance、Bending fixed-point scratch、persistent velocity；公共Mesh result transaction与GN writeback闭环；双 ABI/soak。Bone/stats channel仍仅 planned | Center skip frame shift、configured teleport/negative-scale组合、父级/shear负缩放、baseline重建型 step-basic、Bone connection/topology/output、collision/self-collision |
...
2. MC2 按 `MC2_SOURCE_ALIGNMENT_EXECUTION_PLAN.md` 继续收口 configured teleport/negative-scale组合与 baseline重建型 step-basic；禁止按旧 solver 或未验证近似扩展 spec。
