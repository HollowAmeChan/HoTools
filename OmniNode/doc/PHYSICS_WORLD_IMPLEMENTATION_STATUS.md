...
| MC2 | Mesh native Point collision + public GN result；Bone Line native public result | 单一solver/三setup、staged native context、source-aligned static/N2/N3/Center/scheduler、Tether→Distance→Angle→Bending→Point→Distance→Motion→post、Mesh GN与Bone PoseBone writeback均已闭环；详细数值域和验收见MC2执行计划 | Edge/self collision、BoneSpring soft-collider、Bone negative scale、Bone triangle output、stats |
...
MC2 Collider补充：共享World current/previous collider snapshot已有独立不可变MC2帧契约，覆盖四类primitive、组mask、自身排除与moving pose；V0原子上传、Python owner和Mesh production非same-frame更新已接通。Mesh Point mode现消费radius curve/scale、DisableCollision、persistent friction/collision normal/real velocity与friction inverse mass，并按源码顺序在Bending后投影、第二次Distance后经Motion进入post。py313覆盖投影/post与完整native回归，Blender5.1覆盖production frame上传和既有Mesh/Bone链。Edge、BoneSpring soft-collider和self collision仍未接入。
...
- 当前新 MC2 step 已推进限定的 Mesh Point-collider 数值路径：节点从配置的 BasePose自动构建 N3并消费共享World collider snapshot，native readback先生成私有 candidate，再发布共享 `gn_attribute` 公共结果；不调用旧 MC2 package。
