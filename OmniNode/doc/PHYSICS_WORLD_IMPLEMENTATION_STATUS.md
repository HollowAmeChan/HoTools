...
| MC2 | V1-R替代资格审计 | 单一公开solver step处理全部active tasks；profile+task映射component，per-task slot/context隔离；三setup、外部collider、单cloth self、公共result/writeback与stats已成立 | 仍需Bone横向连接产品特化、跨物体self scope、半径模型、全隐式debug、代码/math整理、新旧性能与文件独立性审计；真实状态只看`MC2_ACCEPTANCE_MAP.md` |
