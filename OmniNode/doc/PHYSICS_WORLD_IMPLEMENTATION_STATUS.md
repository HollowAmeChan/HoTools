...
2. 按`MC2_ACCEPTANCE_MAP.md`先完成MC2替代资格审计；未取得“允许删除”结论前保留旧实现作为语义、性能和依赖审计输入。
   P-06d/P-06e已关闭：Bone生产静态链使用单一raw snapshot、连续数组、6次owner move与compact metadata；Mesh/Bone config分别复制五/六类native static并Center-only重建。topology/geometry/Pin必须全量生产；UV-onlylarge约20.50ms，仍比旧CPP约657.63ms快约32x。P-06f进行中：已删除`InitialState/ParticleBuffer/frame sync`平行host状态链，以及context完整Mesh static二次上传、Proxy/Baseline pack和Bone metadata fallback；生产static只允许staged owner路径。
