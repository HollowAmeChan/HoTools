...
| MC2 | Mesh/Bone native collider + public result | 单一solver/三setup、staged native context、Point/Edge、BoneSpring soft-sphere、self primitive/grid、EE/PT half contact及固定4轮fixed-point solve/sum、source-aligned constraint/post顺序、Mesh GN与Bone PoseBone writeback均已闭环；详细数值域见MC2执行计划 | self collision跨帧Intersect、Bone negative scale、Bone triangle output、wind、stats |
