...
当前 native binding 的 P0 semantic fixture 已达到 py311/py313 各 56/56，同进程双世界重放通过；旧式 Blender 后台集成测试在 2026-07-11 扩展后实跑为 31/31。S2 adapter runner 已对全部 56 个 P0 fixture 与 S1 完成零误差对拍；S3 Blender runner 已对同一套 56 个 P0 fixture 完成 RNA、scope、timeline command、contact/query、十一种约束、result、Quaternion writeback 和生命周期闭环，`repeat=2` 为 56/56。S3 扩展同时确认 `LOCAL_FRAMES` 约束的脏签名必须基于 authoring-local point/rotation，而不能基于随动态 body 变化的 world anchor，否则会每帧错误重建 Jolt 约束并丢失 spring/lambda 历史。
