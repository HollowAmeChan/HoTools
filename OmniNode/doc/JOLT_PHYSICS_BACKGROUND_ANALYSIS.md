...
首轮 Blender 性能基线已由 `benchmark_blender_rigid.py` 采集：每个 case 预热 10 帧、采样 30 帧，覆盖 1/128/1024 个持续 awake 的无接触动态刚体、32/256 个持续 awake 的 Distance 约束链，以及 32/256 个持续接触地面的动态刚体。七个 case 全部通过，报告同时记录 native step、完整 pipeline、Object delta writeback 的 P50/P95、接触事件数和 case 内工作集高水位。1024 body 的 P95 分别为 native `0.1054 ms`、pipeline `55.91 ms`、writeback `271.3 ms`；256 constraint 的 native P50/P95 为 `0.153/0.1895 ms`；256 contact 每帧事件数 min/P95 为 `474/479`，native/pipeline/writeback P95 为 `0.8672/22.5/19.81 ms`。本轮明确写入 `thresholds_frozen=false`，数据表明当前大规模瓶颈主要在 Blender 结果管线和写回，而不是 Jolt step；冻结门槛前必须在相同机器与构建条件下重复采样确认波动。

因此下一阶段不继续扩展 Path、ShapeCast、overlap、compound shape 或其它 feature surface。实现顺序以 `rigid/docs/JOLT_TEST_STRATEGY.md` 的 2026-07-11 决定为准：先用 `DET-003` 固定生产路径模拟顺序，再落 adapter fixture runner、Blender fixture runner、breakable/跨 ABI/soak/performance 门禁，最后恢复高级能力扩展。旧段落中的“后续重点”只描述能力缺口，不再表示当前施工优先级。
