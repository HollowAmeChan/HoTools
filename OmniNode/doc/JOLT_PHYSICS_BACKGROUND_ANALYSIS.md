...
当前 HoTools binding 已能在 Jolt update 后读取约束 current value 和 lambda；contact listener 与 sensor event 已接入轻量结果快照，query API 已接 closest-hit RayCast，ShapeCast/overlap/多命中仍未接入。
...
Jolt 现在已经足够作为“统一物理世界 vertical slice”的样板：它证明了 scope/spec/backend/result/writeback/debug/cache owner 这条链路能跑通。继续补 ShapeCast/overlap、advanced debug snapshot 和 advanced shape 会提升刚体能力，但不是 SpringBone/MC2/BoneCloth 迁移的前置条件。
...
- 已从本帧 contact result stream 输出有界 debug snapshot。
- HoTools overlay 已消费 contact point、normal、penetration、sensor 和 removed 状态 primitives。
...
后续重点：完善刚体属性/约束 spec、ShapeCast/overlap、完整 debug snapshot 和 advanced shape 能力。
...
`SWING_TWIST` 已进入持久属性、生成约束节点、`ConstraintSpec`、Jolt adapter、native binding、constraint state 和专用调试绘制。HoTools 本地 Z 映射为 twist axis，本地 X 映射为 plane axis；支持 `CONE` / `PYRAMID` 摆角、normal/plane half angle、twist min/max、最大摩擦力矩，以及独立 swing/twist 速度或位置 motor。motor 目标使用 HoTools frame，binding 负责映射为 Jolt 内部 `(Z, -Y, X)` constraint space。

语义夹具 `SWING_TWIST-001` 分别施加纯摆动和纯扭转初始角速度，检查 anchor、状态 schema、限制区间、最终收敛与非目标轴残差；`SWING_TWIST-002` 用不对称 Pyramid 半角确认本地 X/Y 分别受 normal/plane 半角限制，并按箱体惯量解析验证摩擦力矩；`SWING_TWIST-003` 验证 swing 位置 motor、twist 速度 motor、目标轴和 motor lambda。三者都要求同一 binary 的两个全新世界 physical trace 逐位一致。
...
验收状态：`hotools_jolt` Release 构建通过；native 测试 29/29、Blender 刚体后台集成测试 22/22 通过。contact debug snapshot 与 overlay 已接。

### Contact debug snapshot / overlay

`Jolt刚体可视化调试` 已增加“显示接触”和“显示传感器”开关。节点先调用 `build_rigid_debug_draw_snapshot()` 生成纯 tuple 线段，再由 draw handler 消费；handler 不读取 world、slot、spec 或 Blender 对象。普通接触、sensor、removed contact、removed sensor 使用独立批次和颜色。

为控制视口成本，每个 snapshot 最多处理 256 个 contact event、每个 event 最多 4 对接触点。snapshot 同时记录可见 contact/sensor 数和截断数。backend debug 摘要输出状态计数、overflow 和最多 16 条不含 native handle 的事件样本。

## 2026-07-10 追加：Closest-hit RayCast 查询

native `JoltWorld.cast_ray()` 已接 `NarrowPhaseQuery::CastRay`，返回最近命中的 body handle、位置、世界法线、fraction、sub-shape id 和 sensor 标记。native 支持排除 sensor 和忽略一个 body；handle 只到 adapter 为止，`JoltAdapter.ray_cast()` 会立即映射成 stable `slot_id`，并把方向归一化、fraction 转成世界距离。

新增普通函数节点 `刚体查询-RayCast`：输入 world、起点、方向、最大距离、是否包含传感器和可选忽略对象；输出命中对象、位置、法线、距离、比例、sensor 标记和纯结果 dict。节点不推进 Jolt 时间，查询结果发布到 `rigid_query_result`，公开结果不含 native handle。

验收状态：native 测试 30/30、旧 Jolt ABI 兼容测试 12/12、Blender 刚体后台集成测试 23/23。当前查询能力只覆盖 closest-hit RayCast；多命中 RayCast、ShapeCast、overlap 与 query collision-group filter 仍待后续。

## 2026-07-11 追加：下一阶段收口决定

当前 P0 semantic fixture 为 60 个：全部 fixture 在 py311/py313 的 S1 均通过同进程双世界重放，S2 adapter 与 S1 完成零误差对拍；S3 Blender runner 覆盖同一矩阵的 RNA、scope、timeline command、contact/query、十一种约束、result、Quaternion writeback 和生命周期闭环。新增 `BREAK-001/002` 以 S1/S2 raw lambda 为断裂前基线，并由 S3 独立验证 HoTools 冲量阈值、断裂后释放和 same-frame 稳定性。S3 扩展同时确认两条生命周期约束：`LOCAL_FRAMES` 脏签名必须基于 authoring-local point/rotation；restart 同步每个 `(generation, frame)` 只能消费一次，否则会重复创建约束并丢失 spring/lambda 或 broken 历史。

`DET-004` 已由 `run_cross_abi_semantics.py` 自动化：runner 发现 CPython 3.11/3.13，分别固定加载 `_Lib/py311` / `_Lib/py313` 的 native 模块，校验 fixture hash 和子运行状态，再对 canonical trace 做结构化容差差分。2026-07-11 使用 3.11.9 与 3.13.9 实跑全部 60 个 fixture、每套 ABI 重复两次，结果 `60/60`，最大绝对误差 `2.220446049250313e-16`，通过 `abs=2e-5`、`rel=1e-6` 门限；机器可读报告 schema 为 `hotools_jolt_cross_abi_report_v1`。

`CONE-003` 与 `SWING_TWIST-004` 关闭了旋转 frame 和独立 A/B frame 的剩余组合缺口。fixture 采用绕 Y 轴 60 度的非奇异 frame，避免 Quaternion→Euler→Quaternion 往返在 90 度万向节奇点附近触发离散限位分支；SwingTwist 的斜向纯 twist 使用各向同性球体，排除非均匀惯量引起的无外力进动。新增 oracle 直接从 body 姿态重建两侧 live twist 轴并验证 swing 边界，同时验证绕任意世界轴的旋转，不以 Jolt constraint readback 自证。S1/S2/S3 均为 `60/60 × repeat 2`，跨 ABI 最大误差未增加。

`CAP-001` 已覆盖 `max_bodies` 容量失败边界。native 创建超额刚体时通过 `nb::python_error` 稳定映射为带 `max_bodies` 上下文的 Python `RuntimeError`；失败不改变 `body_count` 或已接纳刚体状态，移除刚体释放容量后可以重新创建。生产 S3 链路按 `simulation_order_key` 接纳容量内刚体，只把超额 slot 标为 `_jolt_error`，其余刚体继续 step 和发布 transform，`rigid_solver_stats.sync_error_count` 准确反映失败数。

`EVENT-003` 使用 130 个完全重叠的动态 sensor 产生 `130 * 129 / 2 = 8385` 个 pair，native 和 Blender 生产链均确认固定缓存保留 8192 条、overflow 准确为 193，adapter 映射后的公开事件不含 native handle，contact/sensor result channel 与 solver stats 计数一致。测试同时发现并修复 clear 生命周期问题：Jolt 会把旧世界的 removed 回调延迟到下一次 update；listener 现在丢弃 active 快照不存在且两侧 handle 均已注销的 stale removed pair，避免污染新世界首帧。py311、py313 原生统一测试均为 `18 passed / 1 bpy skip / 0 failed`，Blender 后台集成为 `33/33`。

`SOAK-001` 已由 `run_native_soak.py` 自动化。每套 ABI 分别运行十二层箱体堆叠和八节 Distance 约束链各 10,000 帧；runner 每帧检查 position/rotation/velocity finite、body/constraint 数量恒定、位置边界和链残差，只稀疏写入报告样本。py311 与 py313 均完成两个场景，最大链残差同为 `0.00234770775`，低于 `0.02` 门限；统一 native discovery 另保留 300 帧 smoke，结果为 `19 passed / 1 bpy skip / 0 failed`。step P50/P95 只记录为本机观测，本轮不据此冻结性能门槛。

Blender 性能门禁已由 `benchmark_blender_rigid.py` 和 `performance_thresholds.json` 固定。冻结前使用 3 个独立 Blender 4.5 Release 进程，每 case 预热 10 帧、采样 60 帧，覆盖 1/128/1024 个持续 awake 的无接触动态刚体、32/256 个 Distance 约束链，以及 32/256 个持续接触地面的动态刚体。1024 body 三轮 P95 最大值为 native `0.1307 ms`、pipeline `56.19 ms`、writeback `281.2 ms`；最终门禁复跑为 `0.1177/48.31/242.6 ms`。256 contact 门禁复跑的 native/pipeline/writeback P95 为 `0.9918/24.36/25.34 ms`，进程工作集高水位为 `259.54 MiB`，7 个 case 的冻结阈值全部通过。报告现在写入 `thresholds_frozen=true` 和逐 case gate 结果；数据继续表明大规模瓶颈主要在 Blender 结果管线和写回，而不是 Jolt step。

首版 approved golden 由 `manage_goldens.py` 对完整 60-fixture canonical physical trace 生成，压缩基线总计约 111 KiB。普通命令只读比较，只有显式 `--approve-golden --reviewer --reason` 才能更新；approval report 包含旧/新关键帧 hash、最大误差、RMS 和事件数。py311 对初始基线 `60/60` 零误差，py313 `60/60` 通过，最大绝对误差仍为 `2.220446049250313e-16`。manifest 同时固定 fixture/native/spec/adapter SHA、Jolt 版本和 Release 构建标志。至此当前 P0 release 门禁完整闭环。

当前 P0 release 门禁已经完成。下一阶段可以恢复高级能力扩展，但仍按“公共 spec + binding/native + result/debug + 三层 fixture + golden/性能影响评估”作为不可拆分交付单元；优先项仍是 Path、ShapeCast/overlap、compound shape 等已记录缺口。旧段落中的“后续重点”只描述能力范围，不再表示未完成的门禁施工。
