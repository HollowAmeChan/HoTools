...
- gravity 默认 `(0, 0, -9.81)`；Python adapter 已接 `set_gravity()`，节点侧通过 `rigid_jolt.world_setting` 隐式对象热更新 Jolt 刚体世界 gravity。
...
rigid_jolt.world_setting          # Jolt 刚体世界 gravity、solver iteration 等 world 级对象
```
...
- 从 `rigid_jolt.world_setting` 隐式对象同步 Jolt 刚体世界 gravity。
- update kinematic。
- step。
...
`rigid_jolt.world_setting` 已接入第一项 Jolt 刚体世界级能力：`刚体世界-Jolt设置属性 -> 刚体世界-Jolt设置注册` 写入 gravity 设置，rigid solver 在同步 body/constraint 前调用 `JoltAdapter.set_gravity()`。这让零重力、横向重力和恢复默认重力都可以作为跨帧 Jolt 刚体世界配置表达，而不是临时 frame command。
