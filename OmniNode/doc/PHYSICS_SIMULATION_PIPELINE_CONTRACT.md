...
- MC2 Mesh/Bone substep顺序固定为：Center → prediction/baseline → Tether → Distance → Angle → Bending/Sum → Point或Edge collider → 第二次Distance → Motion → Self Collision → post。外部collider与BoneSpring规则不变。Self Collision在每frame首substep更新primitive/grid、生成EE/PT候选并构建half contact list；后续substep只更新既有contact。每个substep在post前固定执行4轮SolverContact→fixed-point SumContact并修改next position。contact/intersect容器归slot-owned native context；跨帧Intersect尚未进入生产数值域，因此FullMesh仍是受限能力。
