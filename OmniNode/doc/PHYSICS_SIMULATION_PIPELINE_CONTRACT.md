...
- MC2 Mesh/Bone substep顺序固定为：Center → prediction/baseline → Tether → Distance → Angle → Bending/Sum → Point或Edge collider → 第二次Distance → Motion → post。外部collider只消费公共World current/previous snapshot；Mesh mask来自对象属性，Bone mask来自task setup option。BoneSpring强制Point/Sphere，Fixed也参与，并以animated base与limit-distance curve执行soft clamp/反弹衰减及velocity-reference同步。第二次Distance即使没有collider也必须执行。Self Collision的Point→Edge→Triangle primitive已作为slot static state注册，contact/intersect容器也归slot-owned native context；broadphase、contact lifecycle、Intersect分帧与solve仍不属于当前生产域。
