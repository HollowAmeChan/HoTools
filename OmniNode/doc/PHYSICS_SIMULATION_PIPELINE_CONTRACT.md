...
- MC2 Mesh/Bone substep顺序固定为：Center → prediction/baseline → Tether → Distance → Angle → Bending/Sum → Point或Edge collider → 第二次Distance → Motion → Self Collision → post。外部collider只消费公共World current/previous snapshot；Mesh mask来自对象属性，Bone mask来自task setup option。BoneSpring强制Point/Sphere，Fixed也参与，并以animated base与limit-distance curve执行soft clamp/反弹衰减及velocity-reference同步。第二次Distance即使没有collider也必须执行。Self Collision的Point→Edge→Triangle primitive已作为slot static state注册，当前Self stage只在每frame首个实际substep更新inverse mass/thickness/swept AABB/Edge grid尺度、构建三类排序grid run并生成只读EE/PT broadphase候选；contact/intersect容器也归slot-owned native context。narrowphase contact lifecycle、Intersect分帧与solve仍不属于当前生产域。
