...
Rigid solver 也已消费 `rigid_jolt.world_setting`。`刚体世界-Jolt设置属性 -> 刚体世界-Jolt设置注册` 会把 Jolt 刚体世界级参数写入 `world.implicit_objects`；当前落地字段是 `gravity`，solver prepare 会在 body/constraint 同步前调用 `JoltAdapter.set_gravity()`。这类持久设置不走 `rigid_body_commands`，因为它不是单帧 impulse/force，而是直到被同 stable_id 的 disabled entry 或新签名覆盖前都持续生效的 Jolt 刚体世界配置。
