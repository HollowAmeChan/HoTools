...
- 共享 GN 组是受管持久资源。插件 register/re-register 必须按当前 builder 生成的结构
  contract 实检已有组并原位刷新，不能只信任整数 schema；逐帧路径只走 contract digest
  快路。刷新必须保持 modifier 指向稳定组、修复播放层顺序并显式 tag 使用对象。
