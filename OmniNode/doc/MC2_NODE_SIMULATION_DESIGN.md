...
5. **多 request 与失败**：同一 Physics World 同时运行 BoneCloth/BoneSpring；同一 Armature 的多链输入必须归并为一个可观察 request/domain，跨 Armature 才产生多个 request。验证 slot 复用、generation、参数热更新、结果事务和 deterministic 900 帧 soak；失败必须不写入 pose/output/receipt。
...
当前复核结果（仅 Blender 5.2 / Python 3.13）：基础 BoneCloth/BoneSpring product-only 集成、统一 Teleport/Center runner、约束 900 帧双跑、rotation controls 和 gravity axes/falloff 产品 runner 均已通过；旧 Bone constraint 文件只保留兼容门面，生产路径不再创建 V0 task 或读取 native context。下一门禁是把 self scope/cache/radius 与精确 target/rest 断言逐项落到 product request、DomainV1 readback 和公开事务合同，再登记 capability matrix 与删除签字表；在此之前不删除任何旧 Bone oracle 或 owner。
