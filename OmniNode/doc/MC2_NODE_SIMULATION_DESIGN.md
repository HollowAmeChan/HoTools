...
| `cpu_native_kernel.py` | E3 显式 native kernel adapter；只允许 `data_path_only=True`，把 compiled/frame POD 交给独立 C++ owner，并以独立开关暴露已验证的 Distance/Center inertia slice；完整 solver 未就绪时拒绝 product step。 |
