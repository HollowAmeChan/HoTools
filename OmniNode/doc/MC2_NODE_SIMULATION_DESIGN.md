...
计划状态（2026-07-23）：产品基础、多 request 事务、动态槽、Bone writeback 和同布局参数热更新已完成。`test_blender_mc2_bone_product_constraint_soak.py` 已使用两个公开 request 在同一 Physics World 同时运行 BoneCloth/BoneSpring；BoneCloth 锁定 Distance/Tether/Bending topology、Angle/Motion 参数 SoA、whole-domain self 和公共 external pass，BoneSpring 锁定 Distance/Tether、Angle 参数与 Sphere-only external 归一化。两套 ABI/Blender 均完成两次 900 帧，实际源码/pyd 路径正确且摘要同为 `a467fecbd5ad061f2e26926668de33fbbcbb9b7cae8deb33cac897c27435011c`。`test_blender_mc2_product_mixed_output_soak.py` 现在在第 301/601 帧对 MeshCloth、BoneCloth、BoneSpring 同时执行同布局参数热更新/恢复，并通过 native active-solve 计数锁定 Angle/Motion 在关闭区间冻结、恢复帧继续增长；owner、slot、scheduler 和 GN/Bone 两类 writeback 全程不替换。双版本重复 900 帧摘要同为 `7538202abf1026ea3a1b932d82fa38781cabc84f2c4752c4f808885d9adf0db8`。`test_blender_mc2_bone_product_collision_soak.py` 已完成外碰细粒度迁移：组屏蔽产生零碰撞法线，接受 Sphere 产生确定性非零法线与位移，自身 owner collider 被 collector 排除，BoneSpring Capsule 被 Sphere-only 合同排除；BoneCloth 平面切向驱动中，`friction 0.0 -> 0.5` 的平均 lag 在 5.2 为 `0.095117m -> 0.154738m`，4.5 为 `0.095114m -> 0.154742m`。`test_blender_mc2_bone_product_center_teleport.py` 又以公开 Bone request 锁定 world/anchor inertia `0/1` 端点、local inertia、depth 轨迹分离、Reset=`5`、Keep=`3`，并证明每帧三个子步只消费一次 Center frame shift。`test_blender_mc2_bone_product_failure_transaction.py` 对真实 BoneCloth/BoneSpring 两域注入第二域 step 故障，确认 owner/result/feedback 整批回滚、PoseBone 零写入并可同帧重试。
...
E7-CPU测试迁移首批（2026-07-23）：`acceptance_assets_v1.json`已把旧`mesh_cloth_world_self_interaction`并入产品`mesh_unified_domain_e5`，由统一域runner承担跨partition接触、穿插历史、互惠group-mask、动态source重配与多目标事务证据；纯旧`test_interaction_scope.py`、`test_shadow_pipeline.py`、`test_blender_mc2_interaction_v0.py`和`benchmark_blender_mc2_interaction_scope.py`删除。E3的Integration prediction、无碰撞完整混合pass与Post real-velocity history已从运行时V0对照迁到`test_domain_reference_golden.py`：JSON只冻结删除前已通过等价门禁的float32位置/速度与原容差，测试直接驱动DomainV1且不导入旧owner。`test_e3_v0_tolerance.py`尚余Angle/Motion、Center和碰撞七个测试函数，Mesh/Bone旧长帧soak也仍含尚未逐项落位的数值oracle，迁移完成前不得删除。当前只跑py313/Blender 5.2；py311/Blender 4.5冻结到旧代码删除与E7-S基本完成后的最终收尾。
