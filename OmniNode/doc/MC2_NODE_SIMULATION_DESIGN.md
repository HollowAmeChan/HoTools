...
状态修正（2026-07-22）：共享 `derive_center_world_pose_mc2` 已统一 V0 raw Center 与 DomainV1 的 Fixed+bind 姿态推导，无 Fixed 时回退 component pose；共享 `evaluate_center_step_mc2` 已由 V0 与 DomainV1 Center slice 共用。DomainV1 inspect 发布推导后的 Center frame pose，并保留前后 signed-scale history。py311/py313 的 native 总套件为 `27/27`，DomainV1 owner 为 `12/12`，adapter 为 `5/5`。这仍不代表 Center smoothing/anchor history、完整 substep 编排或端到端 V0 tolerance 已完成。
