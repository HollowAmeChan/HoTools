...
实现状态（2026-07-21）：E3 的 backend ABI/lifecycle 与独立 native owner 已完成，完整数值 kernel 尚未接入产品。`frame_compile.py` 先把按 partition 捕获的只读 frame snapshot 校验为同一 frame/generation，再按 compiled logical index view 生成统一 packet；`MC2CPUBackendDomainV1` 在 allocation 前调用 capability gate，创建后只持有 compiled program、私有 physical index map、每 partition history 和 kernel handle；`cpu_native_kernel.py` 只允许显式 `data_path_only=True`，普通 product step 会失败。独立 `_native/src/mc2_domain_cpu.cpp` 不 include V0/Python，复制 bind/frame POD、校验 signature、维护 frame/generation/step state，并通过 live-handle registry 保证重复 dispose 安全。frame ABI 已完整接收每 partition 的 world/anchor pose、signed scale/linear transform、Reset/Keep flags、velocity weight 与 gravity ratio；native 先验证全部分区和可逆 transform，再一次交换 particle frame 与各分区前后 pose/计数，任一分区失败不会发布半帧。animated base 与 persistent particle state 已分离：首次/代际变化初始化全域，连续 Move 保留数值状态，Fixed 每帧跟随动画，Reset 只把目标 partition 的位置和速度清到本帧基准；Keep 通过纯 `apply_partition_keep_transform_mc2` 以 `new_linear * inverse(old_linear)` 搬运目标 partition 的 Move state，并以 quaternion delta 只旋转物理速度，普通无 Keep 帧不分配 transform scratch。现有 `project_neighbor_constraints_mc2` 已作为显式 Distance slice 接入 `configure_distance/step_distance`；现有 `apply_substep_inertia_mc2` 已作为显式 Center inertia slice 接入 `configure_inertia/step_inertia`。后者从 compiled particle SoA 读取 depth，以 `particle_attribute_flags` 建立 Fixed=0、Move=1 的逆质量；`cloth_mass` 只属于跨 task 自碰修正分配，禁止作为粒子积分质量。V0 原有阻尼、velocity weight、重力和位置 prediction 循环已提取为纯 `integrate_particles_mc2`；V0 通过属性位和 16 点 damping 曲线调用，新 owner 通过 compiled per-particle damping SoA 与逆质量调用同一个 kernel，Python 仅在 `integration_slice=True` 时开放。py311/py313 的完整 native `run_all.py` 27/27、owner 9/9 与 adapter 5/5 均通过，覆盖 V0 完整 particle-frame oracle、Fixed 跟随、gravity、damping、velocity persistence、partition-local Reset/Keep、速度旋转、双分区独立计数和非法 frame 原子回滚。现有 `_native/src/mc2_context_*` 仍是 `PyObject*` 驱动的 `Mc2ContextV0` V0 ABI，不能作为新域 kernel 的隐式实现；下一步是 partition Center smoothing/anchor history、完整 substep 编排及单 source 端到端 V0 tolerance oracle，不得给 V0 context 叠加第二种输入。该 oracle 通过前，V0 solver、slot、Physics World 和 writeback 不导入该适配器。
