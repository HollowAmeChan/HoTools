...
计划状态（2026-07-23）：产品基础、多 request 事务、动态槽、Bone writeback 和同布局参数热更新已完成。`test_blender_mc2_bone_product_constraint_soak.py` 已使用两个公开 request 在同一 Physics World 同时运行 BoneCloth/BoneSpring；BoneCloth 锁定 Distance/Tether/Bending topology、Angle/Motion 参数 SoA、whole-domain self 和公共 external pass，BoneSpring 锁定 Distance/Tether、Angle 参数与 Sphere-only external 归一化。两套 ABI/Blender 均完成两次 900 帧，实际源码/pyd 路径正确且摘要同为 `a467fecbd5ad061f2e26926668de33fbbcbb9b7cae8deb33cac897c27435011c`。`test_blender_mc2_product_mixed_output_soak.py` 现在在第 301/601 帧对 MeshCloth、BoneCloth、BoneSpring 同时执行同布局参数热更新/恢复，并通过 native active-solve 计数锁定 Angle/Motion 在关闭区间冻结、恢复帧继续增长；owner、slot、scheduler 和 GN/Bone 两类 writeback 全程不替换。双版本重复 900 帧摘要同为 `7538202abf1026ea3a1b932d82fa38781cabc84f2c4752c4f808885d9adf0db8`。`test_blender_mc2_bone_product_collision_soak.py` 已完成外碰细粒度迁移：组屏蔽产生零碰撞法线，接受 Sphere 产生确定性非零法线与位移，自身 owner collider 被 collector 排除，BoneSpring Capsule 被 Sphere-only 合同排除；BoneCloth 平面切向驱动中，`friction 0.0 -> 0.5` 的平均 lag 在 5.2 为 `0.095117m -> 0.154738m`，4.5 为 `0.095114m -> 0.154742m`。`test_blender_mc2_bone_product_center_teleport.py` 又以公开 Bone request 锁定 world/anchor inertia `0/1` 端点、local inertia、depth 轨迹分离、Reset=`5`、Keep=`3`，并证明每帧三个子步只消费一次 Center frame shift；两版本均显式加载本工作树 ABI 产物并通过。产品 owner 只提供只读 domain inspect 与按请求触发的 `read_debug_state()`，产品门禁由源码审计禁止 V0 oracle、`MC2TaskSpec`、旧 context 和 interaction aggregate。仍待完成：产品故障语义，以及 Angle/Motion 的恢复目标、limit/backstop 数值边界。最终 import 审计前不允许删除 `_physicsMC2Bone*TaskV0Oracle` 或旧 Bone owner。
