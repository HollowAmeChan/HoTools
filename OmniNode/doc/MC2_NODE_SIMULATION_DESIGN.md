...
| 4 | P1-B（当前入口） | 建立保守失效的 source observation cache，消除普通热帧静态全量扫描。它不改变数值语义，可在 E4 前独立验收。 | 不把不可靠 depsgraph 标志伪装成完整 revision。 |
