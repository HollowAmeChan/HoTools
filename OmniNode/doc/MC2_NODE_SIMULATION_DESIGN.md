...
当前仍未关闭的 E3 门禁已收敛为 Center 跨帧 V0 全分支矩阵：normal shift、Keep/Reset 的两帧粒子提交已经通过；剩余是 paused/catch-up、zero-substep，以及这些状态下的 Center inertia、Distance/约束和 post/history。Angle Limit、MaxDistance、Backstop、point/edge/self 和无碰撞完整事务已经有 V0 tolerance；Blender 4.5 已验证 debug 关闭时 native readback 为零、默认 debug 节点惰性、稀疏请求只捕获声明状态；产品入口与通用计时测试也已验证热点计时关闭/模拟步禁用时不创建 session 且不读取时钟，因此 E3-E 的关闭态性能门禁已经关闭。scheduler/参数 handoff 和单 partition 最终 writeback 数学等价也已关闭。多 target 原子发布属于 E5，不得拿它继续阻塞 E3，也不得提前混入 E3。在剩余 Center 证据完成前不得切换 Physics World owner 或删除 V0。后续每完成一个大阶段，过程日志必须按本节方式归档，流水账不得继续追加到蓝本。
