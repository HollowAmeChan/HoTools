...
E3 evidence update (2026-07-22): same-source prediction and full-reference no-collision tolerance pass on py311/py313. The compiler keeps signed Distance rest values for the V0 horizontal-stiffness marker, and reference pipelines conditionally skip unavailable topology passes. StepBasic, integration-to-Angle, the two-frame Tether-to-Motion branch, and the Mesh point-collision branch now have V0 evidence; edge/self collision and full collision-order tolerance remain explicit open gates. Do not switch the product owner or delete V0 on the basis of slice availability alone.
