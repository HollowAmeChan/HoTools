...
E3-Angle/Motion contract gate（2026-07-22）：Angle 不能从现有 `constraint_tables["angle"]` 的二元 parent/child 行直接重建。生产 kernel 同时消费 parent index、每条 baseline line 的 `start/count/data`、StepBasic positions/rotations、restoration/limit 的逐粒子采样值，以及三轮交错迭代的状态。compiled domain IR 现在先落下可选但成组的 `baseline_parent_indices / baseline_line_start / baseline_line_count / baseline_line_data`，保留空 line 和跨 partition offset，旧 consumer 未提供时仍为 `None`；`MC2NativeCPUKernelV1` 已在 domain allocation transaction 中调用独立 `configure_baseline`，native 校验并持有这组静态 SoA，inspect 发布 ready/line/data count。独立 `angle_slice` 已接入 native `project_angle_constraints_mc2`，显式接收 StepBasic pose、restoration/limit 数组、衰减与开关；独立 `motion_slice` 也已接入 `project_motion_constraints_mc2`，显式接收 base pose、max-distance/backstop/stiffness 数组和两个开关。新增 `step_reference_pipeline` 已固定 `Tether -> Distance A -> Angle -> Bending -> Distance B -> Motion`，并用显式字段驱动每个 pass；debug-off 不复制隐式数组。该 pipeline 仍未包含 external collision/self，也未完成单 source V0 tolerance、三轮 Angle 顺序和 Motion 分支证据；只扩一个 angle endpoint 表或从当前 mesh bind 位置临时推断 baseline 都不合格。
...
| E3-D single-source tolerance | 同一冻结 frame/parameter fixture 同时喂 V0 和 DomainV1，按固定顺序比较 Center frame pose、shift、rotation、velocity history、integration、Tether StepBasic rest、Distance、Reset/Keep、paused/catch-up 和零子步结果。 | 固定 tolerance 摘要、有限性/确定性重复跑、端到端 V0 oracle；误差来源必须按 pass 标注，Tether 必须证明 rest 来源是 StepBasic。 | 不用更新后的产品差异 fixture 掩盖旧/新差异；不在 tolerance 之前删除 V0。 |
