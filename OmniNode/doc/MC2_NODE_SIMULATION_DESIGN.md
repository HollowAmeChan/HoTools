...
当前复核结果（仅 Blender 5.2 / Python 3.13）：基础 BoneCloth/BoneSpring product-only 集成、统一 Teleport/Center runner、约束 900 帧双跑和 rotation controls 产品 runner 均已通过；旧 Bone constraint 文件只保留兼容门面，生产路径不再创建 V0 task 或读取 native context。下一门禁是把 gravity axes/falloff、self scope/cache/radius 与精确 target/rest 断言逐项落到 product request、DomainV1 readback 和公开事务合同，再登记 capability matrix 与删除签字表；在此之前不删除任何旧 Bone oracle 或 owner。
