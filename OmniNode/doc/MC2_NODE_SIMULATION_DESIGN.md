...
| `cpu_native_kernel.py` | E3 显式 native data-path kernel adapter；只允许 `data_path_only=True`，把 compiled/frame POD 交给独立 C++ owner；数值 solver 未就绪时拒绝 product step。 |
...
实现状态（2026-07-21）：E3 的 backend ABI/lifecycle 与 native data-path owner slice 已完成，完整数值 kernel 尚未接入产品。`frame_compile.py` 先把按 partition 捕获的只读 frame snapshot 校验为同一 frame/generation，再按 compiled logical index view 生成统一 packet；`MC2CPUBackendDomainV1` 在 allocation 前调用 capability gate，创建后只持有 compiled program、私有 physical index map、每 partition history 和 kernel handle；`cpu_native_kernel.py` 只允许显式 `data_path_only=True`，普通 product step 会失败。独立 `_native/src/mc2_domain_cpu.cpp` 不 include V0/Python，复制 bind/frame POD、校验 signature、维护 frame/generation/step state，并通过 live-handle registry 保证重复 dispose 安全；现有 `project_neighbor_constraints_mc2` 已作为显式 Distance slice 接入 `configure_distance/step_distance`，Python 只有 `distance_slice=True` 才能调用。py311/py313 headless native/adapter 测试覆盖输入 ownership、签名失败不污染已发布 frame、未 update 禁止 step、资源计数、output 和 Distance slice。现有 `_native/src/mc2_context_*` 仍是 `PyObject*` 驱动的 `Mc2ContextV0` V0 ABI，不能作为新域 kernel 的隐式实现；下一步是 integration、Center/Teleport 及单 source V0 tolerance oracle，不得给 V0 context 叠加第二种输入。该 oracle 通过前，V0 solver、slot、Physics World 和 writeback 不导入该适配器。
