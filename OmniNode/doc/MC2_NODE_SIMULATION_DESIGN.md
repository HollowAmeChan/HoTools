...
E3-Angle contract gate（2026-07-22）：Angle 不能从现有 `constraint_tables["angle"]` 的二元 parent/child 行直接重建。生产 kernel 同时消费 parent index、每条 baseline line 的 `start/count/data`、StepBasic positions/rotations、restoration/limit 的逐粒子采样值，以及三轮交错迭代的状态。下一步必须在 domain IR 中新增 backend-neutral baseline line SoA（稳定 logical particle 索引、无跨 partition 行、显式空行），并把其 pose 输入与 parameter table signature 一起纳入 `ProgramViewV1`/`ParameterView`；只扩一个 angle endpoint 表或从当前 mesh bind 位置临时推断 baseline 都不合格。该 contract 通过前，reference prefix 保持在 Bending 结束，不接入 Angle，也不改变 Physics World 的 baseline owner。
