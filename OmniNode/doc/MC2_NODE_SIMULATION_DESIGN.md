...
- 开启 Angle Limit 的 full V0 tolerance 暴露了一个 handoff 误差：V0 只把 `powers.angle` 乘到 Angle Restoration 强度，Angle Limit 是按 depth 采样的即时 bound，不乘该 power。`reference_step.py` 已拆开两条路径；同一 fixture 同时覆盖 Angle Limit、MaxDistance 与 Backstop，py311/py313 均通过。
- E3 Center 两帧 fixture 发现 DomainV1 旧实现只保存 `center_shift_vectors/history`，没有把 shift 应用到粒子位置、velocity reference、velocity 和 rotation；这会让 inspect/Tier-A 全绿但跨帧结果错误。现已提取共享 `apply_particle_frame_shift_mc2`，V0 wrapper 与 Domain 按 partition 共用，并以无 Tether triangle source 的 V0/Domain 两帧对照锁定 normal shift；同一 fixture 的 Keep/Reset teleport、paused zero-substep 与 catch-up 也已分别对照 V0，Keep/Reset flags 为 `3/5`。Center inertia/full post 仍需继续验收。
...
当前仍未关闭的 E3 门禁只剩 Center 跨帧 full pipeline：normal、Keep/Reset、paused zero-substep 与 catch-up 的直接粒子提交已经通过；下一步必须把 Center inertia 串入 reference order，并在同一两帧 fixture 中继续比较 Distance/约束和 post/history。Angle Limit、MaxDistance、Backstop、point/edge/self 和无碰撞完整事务已经有 V0 tolerance；Blender 4.5 已验证 debug 关闭时 native readback 为零、默认 debug 节点惰性、稀疏请求只捕获声明状态；产品入口与通用计时测试也已验证热点计时关闭/模拟步禁用时不创建 session 且不读取时钟，因此 E3-E 的关闭态性能门禁已经关闭。scheduler/参数 handoff 和单 partition 最终 writeback 数学等价也已关闭。多 target 原子发布属于 E5，不得拿它继续阻塞 E3，也不得提前混入 E3。在剩余 Center full-pipeline 证据完成前不得切换 Physics World owner 或删除 V0。后续每完成一个大阶段，过程日志必须按本节方式归档，流水账不得继续追加到蓝本。
