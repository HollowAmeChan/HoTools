...
计划状态：产品基础、多 request 事务、动态槽和 Bone writeback 已完成；全约束/mixed-output 断言迁移、双 ABI 900 帧复验和最终 import 审计待执行。该计划完成前只允许继续迁移和审计，不允许删除 `_physicsMC2Bone*TaskV0Oracle` 或旧 Bone owner。
