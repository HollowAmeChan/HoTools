...
|---|---|---|
| BoneCloth | 同一个 Armature component 下的一个或多个中控骨链 partition | 支持 Line/Seq/SeqLoop；`hotools_product` 横向 triangle、旋转插值、根旋转及 triangle 最终覆盖顺序保持不变；不同 Armature 在 Require-Fusion 下明确失败。 |
| BoneSpring | 同一个 Armature component 下的一个或多个根骨链 partition | 只允许 Line 连接；保留当前只消费 SPHERE 外碰的产品限制、Line 方向输出及 connected/disconnected 写回语义；不同 Armature 不隐式拆分。 |
...
| 7 | E5-B：Bone统一域包装 | 把BoneCloth/BoneSpring capture、静态拓扑、帧输入和Bone输出接到同一product request、DomainV1与结果事务；旧节点改为薄collector。 | 不复制solver；不把多Armature输入静默拆成hidden task。 |
| 8 | E7-CPU | 在三种setup产品等价后，删除旧隐式拆task、aggregate常规路径、V0产品owner与binding。 | 不等待GPU，也不以Bone仍需迁移为理由保留双轨。 |
| 9 | E7-S：兼容层收敛审计 | 逐项review迁移中引入的legacy/compat/fallback/shadow/双schema处理，删除已无外部合同依据的分支并简化命名、所有权和事务层。 | 不把“测试仍通过”当作保留中间态适配器的理由。 |
| 10 | P6-B（贯穿 2-9，最终收口） | 把已落地的 SoA、pass 读写集、buffer 容量/溢出、增量上传、CPU tolerance 和单向 writeback 汇成 GPU implementation package。 | 不因此创建 GPU runtime，也不让 CPU 为 GPU 准备发生不可解释退化。 |
| 11 | E6（未来独立里程碑） | 在统一域产品路径、E7-S和规模基准稳定后，实现最小 GPU 原型并测 `2k/10k/50k/100k` 收益曲线，再决定完整覆盖顺序。 | 不以单个1760粒子样本或只报kernel时间宣称成功。 |
...
2. 为BoneCloth/BoneSpring建立基于现有`MC2PartitionEntry`的source/patch/implicit registry/collector，stable id包含Armature data、根/中控骨和setup type；同一Armature多链合为partition，跨Armature Require-Fusion直接失败。
...
退出条件：BoneCloth Line/Seq/SeqLoop与triangle覆盖、BoneSpring Line/SPHERE限制、动画反馈、Anchor/Reset/Keep、参数热更新、外碰、自碰/禁用分支、connected/disconnected写回和多Armature失败原子性均有Domain/V0对照；现有BoneCloth/BoneSpring 900帧代表soak在py311/py313与Blender 4.5/5.2通过；产品路径不再产生`MC2TaskSpec`、`MC2NativeContextV0`或hidden task。
...
E7 按所有权分三次收口。E5-B通过Bone长期资产回归后执行E7-CPU：同时删除MeshCloth、BoneCloth、BoneSpring旧隐式拆task路径、普通aggregate、V0产品owner与binding；这部分不等待E6。随后立即执行E7-S，对迁移中积累的兼容处理做专项review和简化。未来GPU产品化后再执行E7-GPU，删除原型适配、临时readback/fallback和已经被正式backend owner取代的资源路径。任何旧实现都只能在替代路径有长期证据时保留。
...
E3 原生复验（2026-07-22）：干净构建的 V0 native contract 与全部 E3 V0/Domain tolerance case 已通过。此前的 access violation 被确认是不一致的增量二进制产物，而不是 Domain tolerance 缺口。当前工作会话遵循用户占用约束，只构建 py313、只在 Blender 5.2 验收；py311 双 ABI 合同保留但不在本轮重编译。
...
E3 已关闭：单 source DomainV1 的完整 pass 顺序、Center 跨帧 normal/Keep/Reset/catch-up、零子步暂停、非零 inertia + Tether + Distance 组合、post/history、scheduler/参数 handoff、单 target writeback数学、debug-off readback和热点计时关闭态均已有固定证据。P0、P1-B、粒子级覆盖合同、partitioned StepBasic、whole-domain self/external、native-owned完整pass、单次domain collider capture、E5多target原子发布与Mesh产品collector也已关闭。当前执行入口是E5-B：把BoneCloth/BoneSpring已有capture/static/frame/output语义包装进setup-neutral product owner；三种setup自动化和Blender等价前不授权E7删除。后续每完成一个大阶段，只增加整理后的决策与踩坑结论，不记录逐提交过程。
