...
E7-CPU Angle/Motion 调试记录（2026-07-23）：产品 Domain 新增独立约束调试会话 ABI，只有最终真实 substep 且过滤器明确请求时才开启。Motion 的 MaxDistance 与 Backstop、Angle 的 Limit 与 Restoration 均从实际 kernel 分支发布 origin、target、correction、valid 和 partition identity；Angle target 是每轮父旋转级联后的真实目标，不允许由最终姿态反推。成功 capture 后释放全部缓冲，异常立即释放，暂停帧保留请求等待，debug-off 的 mask 与 retained state 都为零。双 ABI native `30/30`、DomainV1 `22/22`、产品槽 `13/13` 与三类 E7 reachability `0/0/0` 已通过；Blender 4.5/5.2 的 Mesh、BoneCloth、BoneSpring 和旧 renderer 均明确加载本工作树产物并通过。该组不包含 Distance/Tether/Bending、外碰或 whole-domain self；它们仍按混合执行顺序逐组关闭后才允许删除旧 owner。
