...
状态修正（2026-07-22）：共享 `derive_center_world_pose_mc2` 已统一 V0 raw Center 与 DomainV1 的 Fixed+bind 姿态推导，无 Fixed 时回退 component pose；共享 `evaluate_center_step_mc2` 已由 V0 与 DomainV1 Center slice 共用。DomainV1 inspect 发布推导后的 Center frame pose，并保留前后 signed-scale history。E3-B 的 `frame_delta_time/simulation_delta_time/time_scale/skip_count/is_running` 已贯通 domain IR、FrameView、binding 和只读 envelope，非法 timing 会整帧拒绝且不改变已发布状态。E3-C 的 `evaluate_center_frame_shift_mc2` 已按 Anchor → Teleport → smoothing → world inertia/限速 → frame history 顺序落地为显式 native slice；py311/py313 的 10 个 Tier-A frame-shift fixture、Center Anchor fixture、adapter slice 均通过。它尚未接入 DomainV1 完整 substep 或 product slot。py311/py313 的 native 总套件为 `27/27`，DomainV1 owner 为 `12/12`，adapter 为 `6/6`。这仍不代表完整 substep 编排或端到端 V0 tolerance 已完成。
