...
E4尚未关闭：native/adapter的compiled external和纯host的whole-domain collider capture子门槛已关闭。`MC2DomainColliderFrameSpec`拥有独立只读SoA，按collector draft的全部resolved source一次排除域内owner，不做partition mask预筛选，并可直接被compiled full endpoint消费；source重排不改变capture signature。`MC2MeshFragmentCacheV1`现按`static_signature + per-partition world_gravity_direction`复用完整Tier A fragment，stage期间的部分成功不会进入live cache，只有显式commit才批量发布并裁掉离域条目；stale/foreign batch均被拒绝。`MC2MeshFusedCPUOwnerV1`进一步把fragment stage、domain compile、native create和cache commit组成staged replacement：exact program/parameter命中复用同一handle；仅参数值改变虽然program可复用，但在原子参数热更新ABI落地前仍创建新handle；fragment或native创建失败均保留旧live domain/cache。成功同步后由owner自身承接domain frame、compiled full pipeline与logical output，不要求产品调用方穿透backend。`product_collect.py`现把现有节点产生的active单source Mesh task authoring intent按原顺序编为resolved draft，并逐partition只消费一次P1-B冻结raw snapshot生成E0静态快照，同时冻结BasePose topology identity；模块顶层不导入Blender adapter、不创建slot/backend/frame/writeback。`product_slot.py`用固定whole-domain slot id把owner挂到Physics World生命周期：同generation原子sync，跨generation先完整stage新slot再替换，失败保留旧slot，world dispose只释放一次handle。`product_frame.py`现按collector顺序一次捕获全部BasePose/Anchor，复用V0与Domain共同的无handle native Mesh旋转核心，并从fragment缓存直接消费native-ready triangles/ranges/records/corner UV，生成一个logical-domain frame packet；slot把该packet与一次whole-domain collider POD按frame identity共同发布给唯一owner。双ABI已锁定新旋转ABI与旧V0 raw Mesh输出的`1e-7`等价。
...
该slot当前仍明确`product_enabled=False`且未挂入普通V0 step，避免验收前双求解。scheduler frame timing、跨帧Anchor component-local历史和Center producer状态现由slot-owned staged/committed state正式拥有，并在frame/collider共同发布成功后提交。`Center frame shift`的native状态现按frame一次性消费：同一frame的后续substep仍执行全部7个solver pass，但不得重复应用component/Anchor位移；只有下一次成功`update_frame`才重新开放该事务。下一门禁是消费staged schedule、逐substep调用现有compiled settings compiler并在成功后advance scheduler，再完成Blender多source oracle与性能验收。在此之前不切普通产品求解，也不删除V0或aggregate路径。
