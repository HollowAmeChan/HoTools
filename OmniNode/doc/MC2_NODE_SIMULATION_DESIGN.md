...
E3 evidence update (2026-07-22): same-source prediction and full-reference no-collision tolerance pass on py311/py313. The compiler keeps signed Distance rest values for the V0 horizontal-stiffness marker, and reference pipelines conditionally skip unavailable topology passes. StepBasic and the isolated integration-to-Angle branch now have V0 evidence; Motion and collision/self full tolerance remain explicit open gates. Do not switch the product owner or delete V0 on the basis of slice availability alone.
