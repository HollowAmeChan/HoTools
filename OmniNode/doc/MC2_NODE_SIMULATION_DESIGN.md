...
计划状态（2026-07-23）：产品基础、多 request 事务、动态槽和 Bone writeback 已完成。`test_blender_mc2_bone_product_constraint_soak.py` 已使用两个公开 request 在同一 Physics World 同时运行 BoneCloth/BoneSpring；BoneCloth 锁定 Distance/Tether/Bending topology、Angle/Motion 参数 SoA、whole-domain self 和公共 external pass，BoneSpring 锁定 Distance/Tether、Angle 参数与 Sphere-only external 归一化。两套 ABI/Blender 均完成两次 900 帧，实际源码/pyd 路径正确且摘要同为 `a467fecbd5ad061f2e26926668de33fbbcbb9b7cae8deb33cac897c27435011c`。`test_blender_mc2_product_mixed_output_soak.py` 又以三个公开 request 同时运行 MeshCloth、BoneCloth、BoneSpring，统一发布 GN/Bone 两类结果并执行两类 writeback；双版本重复 900 帧摘要同为 `45bc14723e47eb9b80bf351f38bde701a83517e85d43f3dcfd983eb369a01a2a`。产品 owner 现显式提供只读 domain inspect 与按请求触发的 `read_debug_state()`，两份新门禁由源码审计禁止 V0 oracle、`MC2TaskSpec`、旧 context 和 interaction aggregate。仍待完成：把旧全约束和 mixed-output 脚本中 Angle、Motion、碰撞、Center/Teleport、参数热更新与故障语义等细粒度断言逐族迁移；最终 import 审计前不允许删除 `_physicsMC2Bone*TaskV0Oracle` 或旧 Bone owner。
