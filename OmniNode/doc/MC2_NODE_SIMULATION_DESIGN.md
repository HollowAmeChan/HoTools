...
当前复核结果（仅 Blender 5.2 / Python 3.13）：基础 BoneCloth/BoneSpring product-only 集成与统一 Teleport/Center runner 已通过；Bone constraint runner 已先完成 helper 与旧 mixed runner 的导入解耦，但其主体仍调用已被产品入口拒绝的 V0 task/native-context 路径，因此不能宣称约束 900 帧验收已通过。下一门禁是把该 runner 逐项迁移到 product request、DomainV1 readback 和公开事务合同，再登记 capability matrix 与删除签字表；在此之前不删除任何旧 Bone oracle 或 owner。
