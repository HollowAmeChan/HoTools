...
7. compile signature 覆盖 topology、参数布局、output mapping 和 backend requirement；任何一项变化都走 staged replacement。
...
| particle parameter SoA | mass/inertia、damping、radius、gravity、collision filters | CPU/GPU solver |
| constraint tables | type-specific indices、rest、stiffness、flags | constraint kernels |
| collision tables | primitive indices、group/mask、thickness | broadphase/narrowphase |
...
| `partition_parameters` | typed SoA[P] | Center/Teleport/collision policy 等。 |
...
仅数值变化不重建 topology/output map/backend domain；shape、类型、capability 或 logical membership 变化才产生新 layout/program。CPU/GPU 都必须报告 parameter upload/update 成本。
...
2. `CompiledDomainCache`：按有序 fragments、参数布局、output map 和 backend capability 复用；
