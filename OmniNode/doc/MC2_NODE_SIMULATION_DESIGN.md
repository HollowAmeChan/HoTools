...
实现状态（2026-07-22）：E3 的 backend ABI/lifecycle 与独立 native owner 已完成，完整数值 kernel 尚未接入产品。`frame_compile.py` 先把按 partition 捕获的只读 frame snapshot 校验为同一 frame/generation，再按 compiled logical index view 生成统一 packet；`MC2CPUBackendDomainV1` 在 allocation 前调用 capability gate，创建后只持有 compiled program、私有 physical index map、每 partition history 和 kernel handle；`cpu_native_kernel.py` 只允许显式 `data_path_only=True`，普通 product step 会失败。独立 `_native/src/mc2_domain_cpu.cpp` 不 include V0/Python，复制 bind/frame POD、校验 signature、维护 frame/generation/step state，并通过 live-handle registry 保证重复 dispose 安全。compiled program 现已显式携带每 partition 的 `center_local_position` 与 `initial_local_gravity_direction`；frame packet/ABI 现已携带每粒子 animated world rotation，不再要求 Center 或 Bone 从隐藏 V0 state 回查。frame ABI 同时接收每 partition 的 world/anchor pose、signed scale/linear transform、Reset/Keep flags、velocity weight 与 gravity ratio；native 先验证所有粒子旋转、全部分区和可逆 transform，再一次交换 particle frame 与各分区前后 pose/计数，任一输入失败不会发布半帧。animated base 与 persistent particle state 已分离：首次/代际变化初始化全域，连续 Move 保留数值状态，Fixed 每帧跟随动画位置/旋转，Reset 只把目标 partition 的位置、旋转和速度清到本帧基准；Keep 通过纯 `apply_partition_keep_transform_mc2` 以 `new_linear * inverse(old_linear)` 搬运目标 partition 的 Move position，并以同一 quaternion delta 旋转状态朝向与物理速度，普通无 Keep 帧不分配 transform scratch。现有 `project_neighbor_constraints_mc2` 已作为显式 Distance slice 接入 `configure_distance/step_distance`；现有 `apply_substep_inertia_mc2` 已作为显式 Center inertia slice 接入 `configure_inertia/step_inertia`。后者从 compiled particle SoA 读取 depth，以 `particle_attribute_flags` 建立 Fixed=0、Move=1 的逆质量；`cloth_mass` 只属于跨 task 自碰修正分配，禁止作为粒子积分质量。V0 原有阻尼、velocity weight、重力和位置 prediction 循环已提取为纯 `integrate_particles_mc2`；V0 通过属性位和 16 点 damping 曲线调用，新 owner 通过 compiled per-particle damping SoA 与逆质量调用同一个 kernel，Python 仅在 `integration_slice=True` 时开放。py311/py313 的完整 native `run_all.py` 27/27、owner 10/10 与 adapter 5/5 均通过，覆盖 V0 完整 particle-frame oracle、Fixed 跟随、gravity、damping、velocity persistence、partition-local Reset/Keep、状态/速度旋转、双分区独立计数、非单位粒子旋转拒绝和非法 frame 原子回滚。现有 `_native/src/mc2_context_*` 仍是 `PyObject*` 驱动的 `Mc2ContextV0` V0 ABI，不能作为新域 kernel 的隐式实现；下一步是共享纯 native Center evaluator、partition smoothing/anchor history、完整 substep 编排及单 source 端到端 V0 tolerance oracle，不得给 V0 context 叠加第二种输入。该 oracle 通过前，V0 solver、slot、Physics World 和 writeback 不导入该适配器。

状态修正（2026-07-22）：共享 `derive_center_world_pose_mc2` 已统一 V0 raw Center 与 DomainV1 的 Fixed+bind 姿态推导，无 Fixed 时回退 component pose；共享 `evaluate_center_step_mc2` 已由 V0 与 DomainV1 Center slice 共用。DomainV1 inspect 发布推导后的 Center frame pose，并保留前后 signed-scale history。E3-B 的 `frame_delta_time/simulation_delta_time/time_scale/skip_count/is_running` 已贯通 domain IR、FrameView、binding 和只读 envelope，非法 timing 会整帧拒绝且不改变已发布状态。E3-C 的 `evaluate_center_frame_shift_mc2` 已按 Anchor → Teleport → smoothing → world inertia/限速 → frame history 顺序落地，并由 DomainV1 显式 `step_center_frame_shift` 以 per-partition transaction 保存 shift/now/teleport history。E3-D 的 `step_reference_slices` 当前固定为 Center frame-shift → Center evaluator → integration/predict → Tether → Distance A → Bending；Tether 的 root topology 在 domain allocation 时配置，rest length 每次显式从 StepBasic positions 计算，避免把当前变形位置偷换成基准；Bending topology 保留 dihedral sign 与 volume marker `100`，native pass 分开提交两类四点记录并使用约束表聚合的 per-particle stiffness。它仍是可验证的 reference slice prefix，不是完整 product solver。py311/py313 的 10 个 Tier-A frame-shift fixture、Center Anchor fixture、DomainV1 transaction、Tether rest-length slice、Bending marker contract、reference prefix 和 adapter 均通过。py311/py313 的 native 总套件为 `27/27`，DomainV1 owner 为 `12/12`，adapter 为 `8/8`。这仍不代表 Angle/Motion、第二次 Distance、collision/self、完整 substep 编排或端到端 V0 tolerance 已完成。

E3-Angle/Motion/Collision contract gate（2026-07-22）：Angle 不能从现有 `constraint_tables["angle"]` 的二元 parent/child 行直接重建。生产 kernel 同时消费 parent index、每条 baseline line 的 `start/count/data`、StepBasic positions/rotations、restoration/limit 的逐粒子采样值，以及三轮交错迭代的状态。compiled domain IR 现在先落下可选但成组的 `baseline_parent_indices / baseline_line_start / baseline_line_count / baseline_line_data`，保留空 line 和跨 partition offset，旧 consumer 未提供时仍为 `None`；`MC2NativeCPUKernelV1` 已在 domain allocation transaction 中调用独立 `configure_baseline`，native 校验并持有这组静态 SoA，inspect 发布 ready/line/data count。独立 `angle_slice` 已接入 native `project_angle_constraints_mc2`，显式接收 StepBasic pose、restoration/limit 数组、衰减与开关；独立 `motion_slice` 也已接入 `project_motion_constraints_mc2`，显式接收 base pose、max-distance/backstop/stiffness 数组和两个开关。新增 `step_reference_pipeline` 已固定 `Tether -> Distance A -> Angle -> Bending -> Distance B -> Motion`，并用显式字段驱动每个 pass；独立 external point-collision、external edge-collision 和 self-collision slice 均已接入 typed SoA，native 只写 domain-owned normals/friction scratch，并把 particle move flags 转换为 native self-collision attributes。debug-off 不复制隐式数组。该 structural pipeline 仍未把 collision/self 自动串入 reference order，也未完成单 source V0 tolerance、三轮 Angle 顺序和 Motion/碰撞/self 分支证据；只扩一个 angle endpoint 表或从当前 mesh bind 位置临时推断 baseline 都不合格。
...
### E3 full reference transaction (2026-07-22)

V0 ordering is now recorded explicitly: `Tether -> Distance A -> Angle -> Bending -> point collision -> edge collision -> Distance B -> Motion -> self collision`. `MC2NativeCPUKernelV1.step_reference_pipeline_full` is a reference-only orchestration entry that calls the existing native slices at these positions. Its `point_collision`, `edge_collision`, and `self_collision` slots must each be an explicit mapping or `None`; self collision receives an independent `old_positions` array and must not reuse StepBasic. This does not change the product `step()` path. Collision/self branch tolerance and single-source V0 equivalence remain open E3 gates.

Distance contract correction: DomainV1 now stores V0-compatible per-particle distance inverse masses from `depth` and `collision_friction`, keeps Fixed particles at zero inverse mass (the kernel applies the fixed-neighbor fallback), and accepts a per-pass `simulation_power` without allocating a scaled stiffness buffer. The full reference pipeline exposes `distance_simulation_power` separately from prediction power; this is required before a V0 tolerance result can be considered valid.
...
E3 evidence update (2026-07-22): same-source prediction and full-reference no-collision tolerance pass on py311/py313. The compiler keeps signed Distance rest values for the V0 horizontal-stiffness marker, and reference pipelines conditionally skip unavailable topology passes. StepBasic, integration-to-Angle, the two-frame Tether-to-Motion branch, and the Mesh point/edge/self collision branches now have V0 evidence; combined full collision-order and final product-step tolerance remain explicit open gates. Do not switch the product owner or delete V0 on the basis of slice availability alone.

E3 StepBasic contract update (2026-07-22): an Angle mismatch initially looked like a solver discrepancy, but the native Angle kernel matched V0 when given V0's prepared StepBasic pose. The missing contract was baseline-local pose data and parent-first StepBasic reconstruction, not the Angle projection formula. The unified domain IR now carries baseline vertex local positions/rotations; DomainV1 configures them and exposes native `prepare_step_basic_pose`, implemented through the shared `update_step_basic_pose_mc2` kernel. A py311/py313 test compares both positions and rotations against V0's Angle debug target and then validates the native integration-to-Angle branch. Do not copy V0's private negative-scale/rebuild details into the product owner without a separate coordinate-space decision.

E3 open semantic gate (2026-07-22): a full-pipeline comparison can still diverge when V0 Tether is disabled while the compiled domain has Tether topology and the reference orchestrator executes it. This is an enablement/parameter contract issue, not evidence that the Angle kernel is wrong. Tether activation must be represented explicitly in the unified parameter/collector contract before full-pipeline equivalence is claimed.

E3 Motion evidence (2026-07-22): the enabled MaxDistance branch is validated across two continuous frames. The second frame translates the animated Fixed root, native StepBasic reconstruction feeds Tether first, and Motion consumes the resulting particle state plus compiled max-distance/backstop/stiffness fields. DomainV1 and V0 positions match on py311/py313. This test intentionally proves `integration -> Tether -> Motion`; a standalone Motion fixture is not sufficient evidence for the ordered reference pipeline.

E3 point-collision evidence (2026-07-22): MeshCloth point collision uses zero `base_positions`; the animated base pose is reserved for BoneSpring soft-sphere behavior. The V0 comparison fixture now also registers the source radius multipliers instead of silently falling back to one. With the same sphere SoA, collision groups, particle radius/multiplier, and integration prefix, DomainV1 and V0 match on py311/py313. A degenerate center case is retained because it detects both setup-specific base-pose mistakes and incomplete V0 static registration.

E3 edge/self evidence (2026-07-22): MeshCloth edge collision matches V0 with the compiled edge table and the same external sphere SoA. A disconnected overlapping-double-triangle fixture also matches V0's internal self-contact solve when Domain receives the total two-sided primitive thickness (`0.04` for two `0.02` sides) and the previous frame positions. Self tolerance is `1e-4` because V0 quantizes persistent contact parameters to half precision while Domain keeps the shared solve in float32. These are branch proofs; the final gate still requires the ordered full reference transaction with the selected collision mode and self slot.

E3 full transaction branch update (2026-07-22): point and edge cases now execute through `step_reference_pipeline_full` rather than calling the collision endpoint after a hand-written integration step; the self case already used the same full transaction. This closes the ordering evidence for each selectable Mesh collision mode. A single transaction cannot enable both V0 point and edge modes simultaneously; the remaining gate is the product scheduler's mode selection/parameter handoff and final post/writeback equivalence.
