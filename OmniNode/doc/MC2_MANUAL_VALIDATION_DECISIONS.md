...
| D-02 | **分段实施中** | Debug 一级信息架构 | 重力、速度、选中深度与约束 pass correction 已接通；约束记录身份/near-limit、Center 分层量与接触时间层仍待实施 |
...
这条链路当前是**逐pass、逐粒子聚合结果**，不是记录级结果。它能回答“这个pass把哪些粒子从哪里移动了多少”，还不能回答具体哪条Distance edge、Tether root record、Bending quad或Angle子分支负责，也没有near-limit、误差归一化和记录级颜色/宽度。Angle Restoration与Angle Limit目前共用真实Angle pass correction，不能仅凭该箭头归因到其中一个子分支。
...
当前绿/红/蓝仍表示最终边长误差；新增红箭头精确显示Distance A/B两次pass各自的逐粒子correction，已经能证明本帧实际移动量，但还不能归因到具体edge。待办：默认只突出超容差/实际 correction 的边，显示记录级归一化误差和correction；刚度曲线改变颜色/透明度；后续 pass 再次拉坏的边与本 pass 已钳制的边应可区分。

### Tether

当前从 root 到每个粒子画当前线和最小/最大圆环，形成截图中的大量放射线与圆，信息密度失控。

新增红箭头已经显示Tether pass的真实逐粒子correction。待办仍是把它归因到root record：默认只显示接近/超过最短或最长界限的粒子；范围改为沿 root→particle 方向的低亮度带/端帽，当前点与实际 correction 高亮；提供抽样/选中粒子模式。说明 Tether 限制的是相对 baseline root 的整体伸缩，不是相邻边，也不是弹簧力。
...
7. **Debug 基础数据合同（首段已实现）**：五类约束的六个有序pass已按独立请求位稀疏捕获真实pre-position/correction，debug-off零buffer/零readback；记录级active/near-limit和contact时间层继续实施。
