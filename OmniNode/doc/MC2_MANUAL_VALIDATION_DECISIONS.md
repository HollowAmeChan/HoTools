...
| D-08 | **已实现，待手测** | Baseline depth 审计 | 逐粒子深度、root边界和异常编码已接通；先肉眼验收，再决定父链代价或构建后修正 |
...
高级 `粒子深度` 模式的第一版已实现，当前进入人工视觉验收，而不是直接改算法：
...
当前实现从 C++ context 按需读取 solver 已有的 parent/root/depth：蓝到红表示归一化深度，粉色表示 Fixed，紫色表示无根 Move，黄色表示 ZeroDistance，白线表示实际拓扑边跨越 root，橙线表示局部深度突跳，红点/线表示 parent 非法、root 不一致或深度逆序。`max_items`只截断绘制，不参与拓扑有效性判定。选中粒子的 parent 路径、累计长度和归一化分母数值尚未实现，不得将 D-08 标记为人工关闭。
...
2. **粒子深度调试（已实现，待手测）**：以显式按需模式展示真实 depth/root/parent 和异常项，让用户肉眼判断远端异常是否来自深度。
