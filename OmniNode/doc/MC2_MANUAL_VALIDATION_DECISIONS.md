...
| D-02 | **分段实施中** | Debug 一级信息架构 | 重力、速度与选中深度路径已先改为结果导向；约束 correction、Center 分层量与接触时间层仍待实施 |
...
1. **已冻结决策**：D-04自碰质量与D-08深度已经人工关闭；D-05参数归属已经决策并完成owner迁移。D-01/D-03代码已关闭或接通，D-02正在分段实施，D-06方向已定，D-07已确认，D-09代码已实现待界面复验。
