...
| D-02 | **分段实施中** | Debug 一级信息架构 | 重力、速度、选中深度、Tether/Distance/Bending/Motion记录结果与Angle pass correction已接通；Angle迭代记录、Center分层量与接触时间层仍待实施 |
...
Motion也已进入逐粒子记录级：稳定身份为`branch + vertex`，branch固定区分MaxDistance与Backstop；记录保存各分支的pre-origin、边界目标、current/limit/error、active/near状态和刚度后的实际贡献，两个branch按vertex求和必须还原Motion pass correction。Angle当前仍是**逐pass、逐粒子聚合结果**。Restoration与Limit在三轮投影中交错执行，不能用两个独立重放的净差伪装实际归因；后续必须按迭代、baseline child/parent和子分支记录真实贡献。
...
当前每条记录已有稳定`record_index + quad四角色`，区分dihedral与volume，并冻结pass前四角色位置、有效rest/current、signed/normalized error、active状态和每个role的实际correction；红箭头已归因到具体quad，按vertex汇总与真实pass修正一致。紫/青/红整块线仍是主要几何表达。待办：角度记录补共享边、两面法线与rest/current角弧；volume记录使用更紧凑的体积误差表达；默认几何线进一步收缩为active/near-limit。弯曲刚度为0或topology没有triangle时必须明确无效原因。
...
- Motion记录已按稳定`branch + vertex`归因到MaxDistance或Backstop；两个分支分别保存边界目标、current/limit/error、active/near和刚度后的实际correction，红箭头不再使用无法归因的pass总量。现有BasePose、范围球与Backstop球继续表达边界。待办只剩更紧凑地联动画当前粒子、目标和边界，以及用曲线值编码颜色/透明度。
...
7. **Debug 基础数据合同（结构约束继续关闭）**：五类约束的六个有序pass已稀疏捕获；Tether/Distance/Bending/Motion已增加稳定记录identity、active状态与真实贡献，debug-off零buffer/零readback；Angle三轮交错子分支记录和contact时间层继续实施。
