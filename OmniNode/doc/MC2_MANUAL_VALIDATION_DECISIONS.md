...
| D-02 | **分段实施中** | Debug 一级信息架构 | 重力、速度、选中深度、Tether/Distance/Bending记录结果与其余约束pass correction已接通；Angle/Motion记录原因、Center分层量与接触时间层仍待实施 |
...
Angle与Motion当前仍是**逐pass、逐粒子聚合结果**。它们能回答“这个pass把哪些粒子从哪里移动了多少”，还不能回答Angle Restoration/Limit子分支或Motion MaxDistance/Backstop中的具体原因，也没有这些模式的记录级near-limit和误差归一化。Angle Restoration与Angle Limit目前共用真实Angle pass correction，不能仅凭该箭头归因到其中一个子分支。
...
7. **Debug 基础数据合同（结构约束继续关闭）**：五类约束的六个有序pass已稀疏捕获；Tether/Distance/Bending已增加稳定记录identity、active状态与真实贡献，debug-off零buffer/零readback；Angle/Motion记录原因和contact时间层继续实施。
