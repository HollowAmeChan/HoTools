...
当前九个能力族仍均为`gap`。BoneCloth/BoneSpring的Angle、外碰真实响应和外碰task过滤、BoneCloth Motion已经补入生产路径长时证据，且确定性均比较完整900帧数值轨迹；这些不能替代仍缺失的字段分支、Mesh外碰/Motion确定性以及各setup专项不变量。debug acceptance layer目前只表示待验收库存，绘制批次非空不证明几何语义正确。运行门禁仍按三层补齐：单能力静置/受力/边界切换、同context参数hot update与reset/rebuild、三setup与跨task交互混合soak；外碰摩擦、self interaction、Center/Teleport和最终输出映射必须由各自行为断言闭环。
