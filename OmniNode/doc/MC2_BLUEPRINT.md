...
E3 仍是迁移 reference，不是当前产品 owner。现有 `_native/src/mc2_context_*` 继续承载 `Mc2ContextV0` 产品 ABI，Physics World 生产路径在 E4/E5 完成并通过迁移门禁前继续走 V0；不得把 compiled domain 伪装成 V0 输入，也不得把 `center_state.py` 数值公式复制进新的 Python owner。P0、P1-B、粒子级覆盖、whole-domain self/external、多partition frame/collider发布合同和A5-04分区full settings已经闭环；Mesh帧旋转由V0与Domain共用同一native核心。当前硬门禁是产品slot对scheduler timing、Anchor component-local历史和Center producer状态的正式所有权、真实子步执行、Blender多source oracle、性能门禁与E5统一输出事务，不得把“frame ready”或“settings可编译”误报为E4产品切换完成。
...
A5-04已关闭whole-domain参数隔离：Tether、Angle、Motion和Post不再读取partition 0标量，而是按`particle_partition_index`消费分区SoA展开；Integration和Post的velocity weight/gravity继续由同一native owner的Center输出驱动。旧标量endpoint只作为E3单partition oracle保留，Motion的六个有向轴`0..5`与V0公开参数合同一致。显式`product_enabled=False`且尚未进入普通V0 step，因此不产生双求解回归。它仍不代表产品迁移完成：slot尚未拥有frame timing、Anchor component-local历史和scheduler/Center producer状态，真实产品子步、Blender oracle、性能门禁与E5多目标事务仍未关闭，旧V0 task与普通aggregate暂时继续拥有产品路径。只有E5自动化与Blender验收完成后才允许切换owner并执行E7-CPU删除。
