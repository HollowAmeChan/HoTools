...
| BoneCloth | 每个Bone socket中控骨 -> 一个task；其直接子骨组成独立HoTools product ordered-chain组 | Point/Edge外部碰撞，单/跨物体self | Bone transform batch | 中控骨不入粒子；不同中控骨禁止横连；imported triangle拒绝；同Armature组件骨名不得重叠 |
...
| Blender 4.5约束专项soak | Angle Restoration production frame零力静置/热更新900帧；Motion BasePosition/max-distance移动边界900帧；双task外碰scope 600帧 |
