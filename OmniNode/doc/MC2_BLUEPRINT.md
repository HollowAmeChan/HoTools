...
Angle 的下一步不是直接绑定一个二元约束表。compiled domain IR 已记录可选的 `baseline_parent_indices / baseline_line_start / baseline_line_count / baseline_line_data` backend-neutral SoA，DomainV1 已通过独立静态配置事务校验并持有它。下一步必须再接入 StepBasic pose、restoration/limit 逐粒子参数并锁定三轮 Limit/Restoration 交错顺序；在该 contract 与单 source V0 tolerance 通过前，不能由 bind 位置猜 baseline，也不能把 Angle 的 owner 移到 Python 或 Physics World。
