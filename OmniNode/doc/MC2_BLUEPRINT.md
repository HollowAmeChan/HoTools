...
E7-CPU 尚未完成：capability matrix 仍有 3 条证据直接指向旧 Mesh constraint runner；`specs.py` 仍承载部分待迁出的中立合同；Python V0 owner、测试门面、普通 aggregate、68 个 V0 binding 和 5 个 `mc2_context_*` 翻译单元仍在树中。它们只能服务隔离迁移 oracle，不得被产品路径调用。
...
统一产品域已经成立，当前任务是迁移最后的 oracle 并物理删除旧面。代码级能力矩阵共有 9 个能力族，其中 3 个保持 `gap`；另有 3 条证据仍直接引用旧 Mesh constraint runner，即使所属能力暂写 `verified`，也不具备删除签字资格。
...
1. 用产品 request、DomainV1 或共享 kernel 迁移 3 条旧 Mesh constraint 证据；data-path 记录不能代替真实响应。
