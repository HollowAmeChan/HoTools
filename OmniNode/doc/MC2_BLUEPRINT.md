...
Angle 的下一步不是直接绑定一个二元约束表。compiled domain IR 已记录可选的 `baseline_parent_indices / baseline_line_start / baseline_line_count / baseline_line_data` backend-neutral SoA，DomainV1 已通过独立静态配置事务校验并持有它；独立 `angle_slice`、`motion_slice`、external point-collision 和 self-collision slice 已接入现有 native kernel，`step_reference_pipeline` 也已固定 `Tether -> Distance A -> Angle -> Bending -> Distance B -> Motion`。下一步必须锁定 StepBasic/base pose、restoration/limit 与 max-distance/backstop 的逐粒子参数、Angle 三轮 Limit/Restoration 交错顺序、Motion 两分支、collider frame 过滤和 self primitive 语义，并完成单 source V0 tolerance；edge collision 尚未进入 pipeline，在此之前不能由 bind 位置猜 baseline，也不能把这些 owner 移到 Python 或 Physics World。
