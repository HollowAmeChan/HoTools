...
E3 的 CPU backend 生命周期边界已冻结为独立适配器：`cpu_backend.py` 先执行无资源 capability gate，再由显式 kernel 协议承接 domain allocation、frame、step、output 和 dispose；适配器拥有 physical index map 与每 partition history，但不读取 Blender、不导入 V0 context、不承担 Physics World slot 或 writeback。独立 C++ owner 已接入共享 Center world-pose 推导、共享 Center evaluator，以及显式 Distance/Center inertia 数值 slice；双 ABI 已验收，但仍不是完整 solver。现有 `_native/src/mc2_context_*` 是 `PyObject*` 驱动的 `Mc2ContextV0` V0 ABI；新 CPU 实现不得把 compiled domain 伪装成 V0 输入。旧/新单 source tolerance 对照通过前，生产路径继续只走 V0。
...
E3 native owner 已真实编译并通过 py311/py313 headless smoke：`mc2_domain_cpu.cpp` 不依赖 V0/Python，复制 bind/frame POD、校验 domain/layout identity、保留 owner state，并用 live-handle registry 保证重复 dispose 安全。compiled program 显式拥有每 partition 的 Center 本地基准与初始局部重力；frame ABI 显式拥有每粒子 animated world rotation，以及每 partition 的 world/anchor pose、signed transform、Reset/Keep flags 和逐帧权重。任一粒子旋转或分区输入非法时 particle frame、pose 与计数全部保持上一发布值。animated base 与 persistent state 已分开，连续 Move 保留、Fixed 跟随位置/旋转、Reset 只重置目标 partition；Keep 用 full linear pose delta 搬运目标 Move position，并用同一 quaternion delta 旋转状态朝向与物理速度，未触发 Keep 的普通帧不分配该 scratch。现有 `project_neighbor_constraints_mc2` 通过 `configure_distance/step_distance` 接入显式 Distance slice，现有 `apply_substep_inertia_mc2` 通过 `configure_inertia/step_inertia` 接入 depth-weighted Center inertia slice。后者按 compiled `particle_attribute_flags` 使用 Fixed=0、Move=1 的逆质量；自碰交互参数 `cloth_mass` 不参与粒子积分。V0 原有 damping/velocity weight/gravity/position prediction 循环已提取为纯 `integrate_particles_mc2`，V0 与新 owner 分别用 curve view 和 compiled particle SoA 调用同一个 kernel；`cpu_native_kernel.py` 只有在 `data_path_only=True` 且明确请求对应 slice 时才调用数值路径。双 ABI 完整 native 27/27、owner 10/10 和 adapter 5/5 通过。该阶段证明 ABI、frame ownership、生命周期、partition particle history/Reset/Keep 和三段共享/复用既有 kernel 的数值路径，不代表 Center smoothing/anchor history、完整 substep 编排或端到端 V0 tolerance oracle 已完成。
...
状态修正（2026-07-22）：上段历史记录中的 owner `10/10` 已因新增 Center frame-pose 专项提升为 DomainV1 `12/12`；py311/py313 的 native 总套件仍为 `27/27`，adapter 为 `5/5`。共享 Center pose/evaluator 已落地，但 smoothing、anchor history、完整 substep 和端到端 V0 tolerance 仍属于 E3 未完成项。

E3 后续执行顺序固定为：E3-B 已把 domain IR/frame packet 的 scheduler timing contract 接通，E3-C 已把 Anchor、task Teleport、smoothing、world inertia/限速与 frame history 收敛为独立 native slice，并由 DomainV1 的 per-partition transaction 保存；E3-D 当前已固定 Center frame-shift → Center evaluator → integration/predict → Tether（每次从显式 StepBasic positions 计算 root rest）→ Distance A → Bending 的 reference prefix。Bending topology 必须保留 dihedral sign 与 volume marker `100`，并把 stiffness 作为约束记录归并到粒子 SoA；下一步再接入 Angle、第二次 Distance、Motion、collision/self pass，并以同一 prefix/完整 pass 顺序做 V0/DomainV1 tolerance 和 debug-off 性能门禁。每一步都必须保留旧/新 history 的原子回滚证据；在 E3-D tolerance 通过前，不得删除 V0 owner、把 Python `center_state.py` 公式复制到新 owner，或切换 Physics World product slot。

Angle 的下一步不是直接绑定一个二元约束表。compiled domain IR 已记录可选的 `baseline_parent_indices / baseline_line_start / baseline_line_count / baseline_line_data` backend-neutral SoA，DomainV1 已通过独立静态配置事务校验并持有它；独立 `angle_slice`、`motion_slice`、external point/edge-collision 和 self-collision slice 已接入现有 native kernel，`step_reference_pipeline` 也已固定 `Tether -> Distance A -> Angle -> Bending -> Distance B -> Motion`。下一步必须锁定 StepBasic/base pose、restoration/limit 与 max-distance/backstop 的逐粒子参数、Angle 三轮 Limit/Restoration 交错顺序、Motion 两分支、collider frame 过滤和 self primitive 语义，并完成单 source V0 tolerance；collision/self 仍需按 V0 时序接入 pipeline，在此之前不能由 bind 位置猜 baseline，也不能把这些 owner 移到 Python 或 Physics World。
...
V0 ordering is fixed as `Tether -> Distance A -> Angle -> Bending -> point collision -> edge collision -> Distance B -> Motion -> self collision`. `step_reference_pipeline_full` only orchestrates the already landed native slices and requires explicit `mapping` or `None` values for point/edge/self collision slots. The self slot carries its own previous-state positions. This is not the product solver path; full V0 tolerance, collision/self branch evidence, and later removal of V0 remain pending.
...
E3 evidence update (2026-07-22): same-source prediction and full-reference no-collision tolerance pass on py311/py313. Domain compilation preserves signed Distance rest values so V0 horizontal constraints retain their reduced-stiffness marker, and reference orchestration skips Tether, Distance, Angle, and Bending only when the compiled domain has no corresponding topology/baseline. StepBasic, integration-to-Angle, the two-frame Tether-to-Motion branch, and the Mesh point/edge/self collision branches now have V0 evidence; combined full collision-order and final product-step tolerance remain open. These tests must use the exact final-proxy/StepBasic coordinate source before any product-owner switch.

E3 StepBasic preparation (2026-07-22): the Angle solver was checked against V0 with the exact native StepBasic target. The numerical kernel matched; the missing part was the input preparation stage. Compiled domains therefore carry baseline vertex-local positions/rotations, DomainV1 configures those arrays, and `prepare_step_basic_pose` calls the shared native parent-first reconstruction kernel. The new py311/py313 evidence compares native StepBasic positions/rotations to V0 Angle debug output and validates integration followed by Angle. V0's private negative-scale details remain an explicit coordinate-space decision, not an automatic product requirement.

E3 semantic gap (2026-07-22): full reference ordering must not infer enablement from topology alone. A V0 run with Tether disabled can diverge from a domain that owns Tether topology but executes it unconditionally. Tether activation and its parameter provenance must be explicit in the unified collector/parameter packet before full-pipeline equivalence is closed.

E3 Motion evidence (2026-07-22): a continuous two-frame MaxDistance case now compares V0 and DomainV1 after the required native StepBasic and Tether prefix. The second frame moves the Fixed root, Tether produces a non-zero correction, and Motion consumes compiled max-distance/backstop/stiffness parameters. py311/py313 match within the E3 tolerance. Standalone Motion output without its preceding passes is not accepted as full-pipeline evidence.

E3 point-collision evidence (2026-07-22): MeshCloth point collision is compared with zero base-pose positions, matching V0's hard-sphere path; only BoneSpring soft-sphere uses animated base positions. The V0 fixture explicitly uploads source radius multipliers, avoiding the old test harness's implicit all-ones fallback. A degenerate sphere-center case matches DomainV1 and V0 on py311/py313 and remains a guard for setup-specific base-pose and static-radius contracts.

E3 edge/self evidence (2026-07-22): the edge branch matches V0 using compiled edge topology and the external sphere SoA. A disconnected overlapping-double-triangle source matches V0 internal self contacts when the previous positions and total two-sided primitive thickness are passed explicitly; self tolerance is `1e-4` for V0 half-precision contact quantization versus Domain float32 state. Branch evidence does not yet authorize product owner replacement until the ordered full transaction gate is closed.

E3 full transaction branch update (2026-07-22): point and edge comparisons now run through `step_reference_pipeline_full`, and the self comparison already runs through that same ordered transaction. Each selectable Mesh collision mode therefore has an ordered full-transaction branch proof; V0 cannot enable point and edge modes simultaneously. The remaining E3 gate is scheduler mode/parameter handoff plus final post/writeback equivalence, not another standalone collision endpoint.
