...
Angle 的下一步不是直接绑定一个二元约束表。它必须先把 parent、baseline line 的 `start/count/data`、StepBasic pose 和 restoration/limit 逐粒子参数作为 backend-neutral SoA 纳入 domain IR，并锁定三轮 Limit/Restoration 交错顺序；在该 contract 与单 source V0 tolerance 通过前，不能由 bind 位置猜 baseline，也不能把 Angle 的 owner 移到 Python 或 Physics World。
