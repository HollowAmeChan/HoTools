...
E3 native owner 已真实编译并通过 py311/py313 headless smoke：`mc2_domain_cpu.cpp` 不依赖 V0/Python，复制 bind/frame POD、校验 domain/layout identity、保留 owner state，并用 live-handle registry 保证重复 dispose 安全；frame ABI 已原子接收每 partition 的 world/anchor pose、signed transform、Reset/Keep flags 和逐帧权重，任一分区非法时 particle frame、pose 与计数全部保持上一发布值。animated base 与 persistent state 已分开，连续 Move 保留、Fixed 跟随、Reset 只重置目标 partition；Keep 用 full linear pose delta 搬运目标 Move state，并只用 quaternion delta 旋转物理速度，未触发 Keep 的普通帧不分配该 scratch。现有 `project_neighbor_constraints_mc2` 通过 `configure_distance/step_distance` 接入显式 Distance slice，现有 `apply_substep_inertia_mc2` 通过 `configure_inertia/step_inertia` 接入 depth-weighted Center inertia slice。后者按 compiled `particle_attribute_flags` 使用 Fixed=0、Move=1 的逆质量；自碰交互参数 `cloth_mass` 不参与粒子积分。V0 原有 damping/velocity weight/gravity/position prediction 循环已提取为纯 `integrate_particles_mc2`，V0 与新 owner 分别用 curve view 和 compiled particle SoA 调用同一个 kernel；`cpu_native_kernel.py` 只有在 `data_path_only=True` 且明确请求对应 slice 时才调用数值路径。双 ABI 完整 native 27/27、owner 9/9 和 adapter 5/5 通过。该阶段证明 ABI、frame ownership、生命周期、partition particle history/Reset/Keep 和三段共享/复用既有 kernel 的数值路径，不代表 Center smoothing/anchor history、完整 substep 编排或端到端 V0 tolerance oracle 已完成。
