...
E7-CPU 尚未完成：capability matrix 已清除全部旧 Mesh/Bone constraint runner 引用，但 Mesh Distance/Tether 与 whole-domain self 仍是 `gap`；`specs.py` 仍承载部分待迁出的中立合同；Python V0 owner、测试门面、普通 aggregate、68 个 V0 binding 和 5 个 `mc2_context_*` 翻译单元仍在树中。它们只能服务隔离迁移 oracle，不得被产品路径调用。
...
统一产品域已经成立，当前任务是关闭最后两个产品数值缺口并物理删除旧面。代码级能力矩阵共有 9 个能力族，其中 2 个保持 `gap`；全部证据已清除旧 Mesh/Bone constraint runner 引用。
...
1. 关闭 Mesh Distance/Tether 与 whole-domain self 两个能力缺口；data-path 记录不能代替真实响应。
2. Bone 删除前计划与全部旧 constraint runner 迁移已经签字关闭，不再重复实施。
