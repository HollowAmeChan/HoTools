...
E7-CPU 尚未完成：capability matrix 已清除全部旧 Mesh/Bone constraint runner 引用，9 个能力族均已由产品数值闸门接管；`specs.py` 仍承载部分待迁出的中立合同；Python V0 owner、测试门面、普通 aggregate、68 个 V0 binding 和 5 个 `mc2_context_*` 翻译单元仍在树中。它们只能服务隔离迁移 oracle，不得被产品路径调用。
...
统一产品域已经成立，当前任务是物理删除旧面并进行 E7-S 结构复核。代码级能力矩阵共有 9 个能力族，全部 `verified`；全部证据已清除旧 Mesh/Bone constraint runner 引用。
...
2. 将中立 topology/setup/frame 合同迁出 `specs.py`，删除测试 helper 对 mixed/V0 runner 的依赖。
3. 删除 Python V0 oracle、旧 solver/context/interaction owner、hidden task、普通 aggregate 和 resource bridge。
4. 删除 68 个 V0 binding、5 个 `mc2_context_*` 翻译单元及 API/CMake/required-symbol 残留。
