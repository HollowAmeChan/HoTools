...
旧节点 package、旧数组 solve、旧 BoneCloth IO、`MC2TaskSpec`、Python V0 solver/context/interaction owner、普通 aggregate 入口和兼容构建选项已经删除。V0 native ABI 与 5 个 context 翻译单元仍在树中，是当前 E7-CPU 的删除对象，不是可保留的 fallback。
...
E7-CPU 的 Python 批次已经完成，native 批次尚未完成：capability matrix 已清除全部旧 Mesh/Bone constraint runner 引用，9 个能力族均由产品数值闸门接管；topology/setup/frame 中立合同已归入真实职责模块；剩余删除面是 62 个 `mc2_context_v0_*`、6 个 `mc2_interaction_v0_*` binding、5 个 `mc2_context_*` 翻译单元、API/CMake 残留和直接 V0 native tests。
...
3. 当前批次删除 68 个 V0 binding、5 个 `mc2_context_*` 翻译单元、`mc2_api.hpp` 声明、CMake/object dependency、required-symbol 和直接 V0 native tests；4 个中立 frame/static API 与产品 DomainV1 ABI 保留。
4. native 产物重编译后运行 py313 静态符号、纯 native、Python 产品事务和 Blender 5.2 产品验收；不得用旧已编译 pyd 掩盖源码缺失。
