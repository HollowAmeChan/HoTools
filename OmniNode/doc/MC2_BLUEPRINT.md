...
这关闭了E4的compiled external、whole-domain self与native-owned完整pass子门槛。纯host边界也已能从collector draft的全部resolved source一次消费Physics World公共snapshot，生成拥有独立只读数组的whole-domain collider POD：域内全部owner统一排除，外部group不在Python端按任一partition预筛选，moving collider previous pose与四种shape继续复用公共打包规则。它不代表产品迁移完成；既有source observation/fragment cache尚未接到fused owner，多目标原子事务和产品collector也未关闭，因此旧V0 task与普通aggregate暂时继续拥有产品路径。只有E5自动化与Blender验收完成后才允许切换owner并执行E7-CPU删除。
