...
10. self求交半径判定与contact geometry仍是后续重大优化点：Edge-Edge/Point-Triangle closest、预测位移阈值、法线/符号、half量化以及四轮contact update必须独立计时和保持CPU reference等价。后续可验证保守平方距离早退、SoA/SIMD和成熟C++几何内核，但外部库不获得产品所有权，也不得改变MC2退化行为、确定性顺序或单一半径模型。
