...
E3 后续执行顺序固定为：先扩展 domain IR/frame packet 的 frame-shift 输入，再把 Anchor、task Teleport、smoothing、world inertia/限速与 Center evaluator 收敛为同一 native 顺序，最后做 V0/DomainV1 single-source tolerance 和 debug-off 性能门禁。每一步都必须保留旧/新 history 的原子回滚证据；在 E3-D tolerance 通过前，不得删除 V0 owner、把 Python `center_state.py` 公式复制到新 owner，或切换 Physics World product slot。
