...
BoneCloth横向连接只是distance/bending topology的额外producer：每个中控骨独立生成横向边，不改变上述粒子参数含义，也不会把多个中控骨合成同一task。
...
| `碰撞模式` | `0:None`关闭，`1:Point`按粒子点碰撞，`2:Edge`按结构边连续碰撞 | 外部collider上传后由Point或Edge pass消费；`M/C`可调，`S`固定Point。 |
