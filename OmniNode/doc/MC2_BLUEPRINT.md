...
当前架构审计基线为：4 个中立 C++ PyObject API 定义无所有权违规；102 个注册 binding、21 个加载器必需 MC2 symbol 无缺失；产品/公开节点/debug 到旧模块的可达性为零；native 旧面为 0 binding / 0 翻译单元 / 0 专用头文件。运行时 py313 产物导出 72 个 `mc2_*` symbol，旧 context/interaction symbol 为零。数字变化必须由明确产品 ABI 变更解释。
...
- Mesh final-proxy/BasePose 两份兼容 runner 也已删除，验收资产直接指向 Mesh 产品 runner；5 个旧 runner 统一由不存在性门禁禁止回流。
