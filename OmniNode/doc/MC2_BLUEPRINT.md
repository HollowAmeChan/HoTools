...
| 属性 | 用户看到的功能 | 当前实现与setup |
|---|---|---|
| `Tether压缩` | 限制可移动粒子相对所属baseline root的最短距离，防止整片向根部塌缩 | 第一次结构solve中使用StepBasic root rest length投影；`M/C`有效，`S`使用固定压缩值`0.8`。stretch limit固定为`0.03`，不公开。 |
...
| `弯曲刚度` | 抵抗三角面或骨链跨边折叠；0关闭 | runtime把大于0映射为MC2 bending method 2，native triangle/chain bending kernel执行；三个setup有效。 |
| `角度恢复`、`角度恢复刚度`、`角度恢复曲线` | 把父子方向向StepBasic参考方向拉回，形成姿态记忆 | Angle kernel逐baseline投影；target来自StepBasic父子向量与当前parent position，不来自Motion BasePosition。三个setup有效。 |
| `恢复速度衰减` | 角度恢复修正后，控制有多少修正同步进velocity reference，抑制持续摆动 | Angle kernel更新位置时同步修正velocity reference；三个setup有效。 |
| `恢复重力衰减` | 组件旋转使初始重力方向偏离world重力时，降低角度恢复 | 使用`value * (1 - center_gravity_dot)`调节恢复；三个setup进入Angle kernel。 |
| `角度限制`、`限制角度`、`限制角度曲线` | 限制相邻粒子相对StepBasic父方向的最大弯折角 | 与角度恢复共用Angle pass，但使用独立enable和深度角度数组；三个setup有效。 |
| `限制刚度` | 控制超出角度上限后的投影强度 | Angle kernel只在角度限制启用时消费；三个setup有效。 |
...
| `跨物体自碰撞` | 让同一Physics World中启用交互的MeshCloth task自动互碰，不要求指定对象列表 | 转换为`self_collision_sync_mode=2`；world coordinator当前只接纳`setup_kind == mesh_cloth`，因此`M`有效，`C`虽显示并打包但没有跨task效果，`S`强制关闭。 |
| `布料质量` | 改变self primitive的相对修正权重 | self primitive构建时进入inverse-mass计算；`M/C`的task内self都消费，但只有`M`当前能在跨task接触中形成不同布料之间的质量比例。 |

产品只公开一个半径模型：`particle_radius = profile.radius(depth) * object_radius_weight`，Mesh self thickness再由该半径按`0.25`派生。独立`self_collision_thickness`仅是统一profile/source-oracle兼容字段，产品setup不允许用户同时调两套半径。
...
Spring/wind字段目前只为源码数据结构和预设解析兼容而保留；三个产品节点都写入`spring_enabled=False`，native也没有生产Spring/wind kernel consumer。BoneSpring的“弹簧感”来自固定distance、bending/angle、惯性和soft-sphere组合，不等于启用了这些`spring_*`字段。
...
| Blender 4.5混合输出soak | MeshCloth、BoneCloth、BoneSpring同world锁步900帧；三context热更新；Mesh local offset与Bone connected/disconnected写回掩码；完整场景重复两次并比较最终粒子/写回数组摘要以验证确定性 |
| Blender 4.5 Bone角度soak | BoneCloth/BoneSpring各900帧并重复两次；Restoration/Limit在301/601帧关闭与重开，context不重建且关闭区间solve count冻结；BoneCloth强制同时包含connected旋转写回与disconnected位移旋转写回。默认阻尼下静置漂移预算分别为`0.00005m`与`0.0016m`，因此只记为bounded drift，不冒充严格zero-force rest |
| Blender 4.5 Bone Motion soak | BoneCloth 900帧并重复两次；同时包含connected/disconnected写回，前半程MaxDistance、451帧同context热开Backstop，逐帧相对动画基准距离不超过`0.031m`，最终debug Motion BasePosition逐点等于动画输入 |
| Blender 4.5 Bone外碰soak | BoneCloth Edge与BoneSpring固定Point各900帧并重复两次；真实Sphere轻微压入并产生非零响应，451帧同context热更新半径/soft limit。每帧另注入source自有、mask不匹配和BoneSpring不支持的碰撞体，native与最终debug都只允许目标Sphere。BoneCloth逐帧覆盖connected/disconnected写回并以链总长约束跨度和回转范围；BoneSpring最大偏移`0.0102m`且受`collision_limit_distance`约束 |
...
当前九个能力族仍均为`gap`。BoneCloth/BoneSpring的Angle、外碰真实响应和外碰task过滤、BoneCloth Motion已经补入生产路径长时证据，但不能替代仍缺失的字段分支、Mesh外碰响应/确定性、Bone Motion确定性以及各setup专项不变量。debug acceptance layer目前只表示待验收库存，绘制批次非空不证明几何语义正确。运行门禁仍按三层补齐：单能力静置/受力/边界切换、同context参数hot update与reset/rebuild、三setup与跨task交互混合soak；外碰摩擦、self interaction、Center/Teleport和最终输出映射必须由各自行为断言闭环。
