...
- capability matrix 仍有 5 条旧约束 runner 引用，需迁移为共享 kernel、DomainV1 或 product runner 的独立断言。
- Mesh 外碰/摩擦目前只有产品 data-path，数值 kernel 尚未形成真实响应，不能宣称等价。
- BoneCloth/BoneSpring 仍需完成跨 source self scope、contact cache/radius、target/rest 等删除前精确门禁。
...
1. 先补齐 BoneCloth/BoneSpring plan 和 capability matrix 的剩余产品证据，逐条保留原有数值不变量。
2. 把 topology/setup 必需的中立合同迁出 mc2/specs.py，再移除旧 oracle、hidden task、普通 aggregate 和旧 runner bridge。
3. 删除旧 Python owner 与 interaction/shadow 兼容面，随后删除 native V0 binding 和 5 个 context 翻译单元。
4. 删除后立即运行架构、可达性、双 ABI 符号和产品事务审计；任何兼容分支都必须说明保留理由，否则一并简化。
5. 最后只在旧面完成删除后恢复 4.5/py311，执行最终 native、Blender 和持久化回归。
