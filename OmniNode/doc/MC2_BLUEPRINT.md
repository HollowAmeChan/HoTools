...
E3 后续执行顺序固定为：E3-B 已把 domain IR/frame packet 的 scheduler timing contract 接通，E3-C 已把 Anchor、task Teleport、smoothing、world inertia/限速与 frame history 收敛为独立 native slice，并由 DomainV1 的 per-partition transaction 保存。下一步是把该 transaction 与 Center evaluator、完整 substep pass 顺序组成单 source reference，再做 V0/DomainV1 tolerance 和 debug-off 性能门禁。每一步都必须保留旧/新 history 的原子回滚证据；在 E3-D tolerance 通过前，不得删除 V0 owner、把 Python `center_state.py` 公式复制到新 owner，或切换 Physics World product slot。
