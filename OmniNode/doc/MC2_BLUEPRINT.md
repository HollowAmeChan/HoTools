...
人工验收已确认一个待关闭反例：无拓扑交叉、无非流形的单层Mesh中，红色self contact箭头主要聚集在高顶点密度区域，并伴随持续微动。该现象不能与洋红intersection record混为一谈。完成一环过滤后，实际模型中的洋红record和红色contact箭头都明显减少，边缘持续跳动且不收敛的现象几乎消失，确认一环误碰是主要扰动源之一。后续仍必须用同一world-space曲面的低/中/高密度重拓扑，联合记录self thickness/局部一环边长比例、单位面积candidate/contact、contact churn和RMS速度；在定量证据关闭前，自碰静置与密度无关性不视为完全验收。
