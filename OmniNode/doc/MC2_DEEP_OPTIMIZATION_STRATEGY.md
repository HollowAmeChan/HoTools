...
可靠测量
  ...
  -> native 纯计算边界与释放 GIL
  -> task 内 Split Job / 多 context Batch Job 混合调度
  -> 数据布局、SIMD和GPU验证
```
...
单独释放 GIL不会让一个单线程 step 自动变快；它是安全 worker pool、并发独立 context 和避免阻塞其他 Python 线程的必要边界。

### P4：混合 Job 策略

原版 MagicaCloth2 v2.14+ 的公开性能说明采用混合调度：小 cloth 用一个组件一个 Batch Job；重 cloth 使用阶段内 Split Job；超过约 300 proxy vertex 或启用 self/mutual collision 时倾向 Split Job。可借鉴的是划分原则，不是照搬 Unity Job API。

OmniMC2 的目标调度：

- 多个互不交互的小 context：作为 Batch Job 并行。
- 一个较大的 fused MeshCloth context：在 context 内按阶段和数据范围拆 Split Job。
- self collision 或未来仍存在的跨 context barrier：显式依赖栅栏，不能让下一约束阶段读取未完成数据。
- 小数组低于 grain threshold 时保持单线程，避免调度成本大于计算。

使用 native 持久 worker pool，线程数量按物理核心、平台和场景阈值配置。禁止每帧 `std::async`/创建线程；禁止 worker 存活后异步访问已释放 context；world dispose、undo/load 和 addon unregister 必须先排空队列。
...
fused context、连续 partition ranges、显式 particle/constraint SoA 和无 Python inner-loop 是 GPU 化的前置条件，但不代表应立即实现 GPU solver。
...
只有 CPU fused + Split Job 后仍无法达到目标资产预算，且 profiling 显示足够大的可并行 kernel 占比，才进入 GPU 原型。否则 PCIe/driver 同步、双实现维护和调试成本可能高于收益。
...
| P3/P4 | 大 fused task 随 2/4/8 worker 有实际扩展，小任务不会因调度退化；Blender/Python线程边界无违规。 |
| P5 | 每项优化由内部阶段计时证明，输出合同不变。 |
| P6 | 仅在 CPU 路线仍不达标且可并行占比支持时立项。 |
...
7. `refactor(mc2): isolate pure native step from Python errors`
8. `perf(mc2): add persistent native job system`
9. `perf(mc2): parallelize particle and self broadphase stages`
10. `perf(mc2): color and parallelize constraint batches`
