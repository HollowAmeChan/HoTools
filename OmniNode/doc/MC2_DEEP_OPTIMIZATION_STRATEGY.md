...
5. 粒子级隐式/显式resolved intent、provenance、partition filter与domain draft编译入口已闭环；E4/P2的partitioned StepBasic、compiled external、compiled whole-domain self以及native-owned substep snapshot到post的完整pass已由双ABI关闭。当前继续Physics World单次collider capture和多source fragment cache的产品host接线；每个子交付独立做单/双source oracle，产品slot仍保持V0。
