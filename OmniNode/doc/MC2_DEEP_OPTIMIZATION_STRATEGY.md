...
3. 执行 E7-S：六个顶层 setup 产品钩子先按真实 owner/lifecycle 归位为四个 setup 模块，生产模块由 72 个变为 70 个；不以文件数量为 KPI。可选 `native_context` 参数和两个纯内存 `v0` 资源键已经清除；生产 `V0/_v0` 门禁只豁免一个内容签名和一个活动结果 schema，精确迁移词违规也已归零。后续又删除 1 个 Mesh domain 类型别名、3 个无逻辑产品 wrapper 和 1 个测试专用 compiler wrapper，分类 forwarder 降为 80，继续审计重复 owner 和误导命名。
