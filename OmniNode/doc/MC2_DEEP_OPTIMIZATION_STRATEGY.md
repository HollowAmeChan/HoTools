...
5. 粒子级隐式/显式resolved intent、provenance、partition filter与domain draft编译入口已闭环；E4/P2已先让StepBasic按partition消费差异化`animation_pose_ratio`，当前继续把compiled primitive/filter/particle参数接入一次whole-domain self，再完成多source capture/fragment cache与统一context。每个子交付独立做单/双source oracle。
