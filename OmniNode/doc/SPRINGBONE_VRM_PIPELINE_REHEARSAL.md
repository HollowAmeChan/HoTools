...
- `Bone.hotools_collision` 是保持 `.blend` 数据兼容的显式存储名，字段定义与语义归 SpringBone capability 所有。
- 新隐式对象 `bone_collision.override` 是同一能力表的 override interface。
- `spring_vrm/properties.py` 从能力表生成 `PG_Hotools_BoneCollision`；`physicsWorld.registry` 负责注册/注销 class 与 `Bone.hotools_collision` binding；`PhysicsTools` 只保留面板、操作器和预览。
...
2. 修改外部物理面板。
   ...
   - 字段、默认值、范围和说明由 `BONE_COLLISION_CAPABILITY` 持有，`spring_vrm/properties.py` 据此生成 RNA。
