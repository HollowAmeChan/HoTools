...
本文最初用旧 VRM SpringBone solver 预演 `PHYSICS_SIMULATION_PIPELINE_CONTRACT.md`；截至 2026-07-10，预演目标已经全部落地并完成旧路径删除。文中保留的旧结构与性能数据只作为迁移依据和验收历史，不表示发布包仍含兼容入口。

参考源码：

```text
OmniNode/NodeTree/Function/physicsWorld/registry.py
OmniNode/NodeTree/Function/physicsWorld/spring_vrm/
  capabilities.py  -> 显式/隐式字段 schema 唯一事实源
  properties.py    -> capability 生成的 Blender RNA 注册声明
  implicit_objects.py / specs.py / solver.py / native.py

_native/include/hotools_spring_bone_vrm.hpp       -> context-only C++ API
_native/src/spring_bone_vrm.cpp                   -> context 与私有 step kernel
_native/src/spring_bone_vrm_bindings.cpp          -> context-only Python bindings
```
...
这些用途已经完成：旧/context 参数矩阵对拍误差通过 `2e-5` 门槛后，旧 Python 节点/runtime、35 参数 native binding 与公开旧 View 已全部删除。
...
OmniNode/NodeTree/Function/physicsWorld/spring_vrm/
  ...
  declaration.py  -> solver declaration + removed surfaces 记录
  properties.py   -> solver-owned Blender RNA 声明
```
...
- old node socket/cache/payload 不兼容，发布运行时不存在旧入口。
...
- 它必须引用 `BONE_COLLISION_CAPABILITY`，和 solver 显式 `Bone.hotools_collision` RNA 使用同一份能力表；不能在隐式对象里重新手写一套字段 schema。
...
- `Bone.hotools_collision` 是保持 `.blend` 数据兼容的显式存储名，字段定义与语义归 Physics World 共享 collision capability 所有。
- 新隐式对象 `bone_collision.override` 是同一能力表的 override interface。
- `physicsWorld.collision.properties` 从共享能力表生成 `PG_Hotools_BoneCollision`；`physicsWorld.registry` 负责注册/注销 class 与 `Bone.hotools_collision` binding；面板、操作器和预览位于 `physicsWorld.ui`。
...
**最终落地状态（2026-07-10）：** resolver、显式 RNA 生成、隐式 override 和统一注册均已闭环。解析优先级为 override > solver 显式 RNA > capability 默认值；Blender 后台测试覆盖 override 优先、禁用后回退显式 RNA，以及 native 消费完整字段。
...
- 每根模拟骨统一通过 BoneCollisionProfile resolver 读取 `radius`、`collided_by_groups` 与 `pin`。resolver 三层为：`bone_collision.override` 隐式对象、solver 生成的 `Bone.hotools_collision` 显式 RNA、capability 默认值。
...
### 阶段 5：native context 单一路径

目标：

- 采用"静态上传 + 每帧 step"的 context 生命周期（见 CONTRACT.md「Native Context 生命周期协议」）。
- static arrays 常驻 C++ context，topology dirty 才重传。
- 每帧只传 animated arrays + collider arrays + 标量参数。

最终落地状态（2026-07-10）：

- Python slot 侧的 `SpringVRMNativeContext` 只走 context 路径：`spring_vrm_create_context` 常驻 static topology arrays，`spring_vrm_update_dynamic` 每帧上传 pose/collider arrays，`spring_vrm_step` 推进，`spring_vrm_read_results` 读回 target matrices / quaternions。
...
- 35 参数 `solve_spring_bone_vrm_cpp` Python binding、公开 `SpringBoneVrmChainView`、C++ wrapper 和所有旧节点均已删除；数值 kernel 是 context 翻译单元内的私有 `SpringVrmStepView` / `step_spring_vrm_context`，不构成第二 ABI。
...
| `hotools_native.solve_spring_bone_vrm_cpp(...)` | 已删除 | 不存在兼容入口 |
...
hit radius、collided_by_groups、pin 等字段统一视为 BoneCollisionProfile resolver 的输出，而不是各消费端直接读取 Bone 属性。当前更新频率以 capability 表和 slot dirty policy 为准：

```text
restart_only:
  pin 等静态拓扑字段在 slot/context 重建时生效。

live / dirty:
  hit/collider arrays 按显式 RNA hash、override signature/version 和 collider snapshot key 失效刷新。
```

### 多骨架批量仍是 Python 串行

当前 world-aware solver 按 armature slot 串行推进；旧 `_run_spring_bone_vrm_node` / `_SpringBoneVRM.run()` 已删除。未来若做跨 armature 批处理，应直接扩展 context/backend，不恢复旧 wrapper。
...
`spring_vrm` 声明由 `spring_vrm/declaration.py` 持有，中央 `physicsWorld/declarations.py` 通过 registry 惰性汇总；删除记录位于 `SPRING_VRM_REMOVED_SURFACES`。
...
后续优先级已转向 MC2/BoneCloth 原子化和显式 exchange channel；SpringBone 不再保留旧 `_SpringBoneVRM` 审查代码。

2026-07-09 实现补充：`spring_vrm/debug.py` 已作为 SpringBone 自有调试边界落地，负责 slot debug snapshot、native context 统计和 SpringBone debug draw mode 声明；`spring_vrm/debug_draw.py` 负责 SpringBone 骨链/尾端可视化采样与 draw store。公共绘制函数下沉到 `physicsWorld/utils/debug_draw.py`，SpringBone 只复用线段/GPU/基础几何工具，不再走世界级可视化节点。

## 2026-07-10 完成：能力表同源与旧 API 删除

“能力表同源”验证与迁移已经完成：

1. 测试对齐。
   - 以 `BONE_COLLISION_CAPABILITY` 为权威字段表，写 resolver 对照测试。
   - 对比迁移前 `Bone.hotools_collision` 直读路径与 BoneCollisionProfile resolver 的输出。
   - 覆盖 `pin`、`radius`、`collided_by_groups`、`collision_type`、`length`、`offset`、`primary_collision_group`。
   - 删除前先保证原显式数据行为完全不变，再加入 `bone_collision.override` 覆写测试。

2. 修改外部物理面板。
   - 外部物理面板不再把字段定义当作自己的私有事实源。
   - 字段、默认值、范围和说明由共享 `BONE_COLLISION_CAPABILITY` 持有，`physicsWorld.collision.properties` 据此生成 RNA。
   - `Bone.hotools_collision` 只保留为稳定存储名；class 与 binding 生命周期由 physicsWorld registry 接管。

3. 旧实现 API 与兼容层已删除。
   - `_SpringBoneVRM`、旧 VRM/Base 节点、`_BonePhysics` 的 VRM 专用 helper 与 35 参数 native ABI 均不存在。
   - 新 solver 只通过 capability resolver、`world.implicit_objects`、`world.collider_snapshot`、context API 和 result stream 交互。
   - native 双 Python ABI 重建通过；Blender 35/35、py311/py313 context-only 各 10/10、RNA 生命周期 smoke 1/1。

## 2026-07-10 追加：性能验收推进

`slot.data.writeback_plan` 已从空声明落地为跨帧复用的批次计划。SpringBone 每个 slot 每帧只向 `bone_transform` result stream 发布一个 `bone_transform_batch` envelope；正常写回直接消费计划中的 PoseBone 引用和 `mathutils.Matrix`，按 armature 通过一次 `foreach_set("matrix_basis")` 提交。debug、测试和导出仍可通过 `iter_spring_vrm_pose_results()` 按需展开为原逐骨快照 schema，正常播放不再承担逐骨 dict、16-float matrix 二次序列化和按名称重新解析目标的成本。

连续帧同时复用 native context 中的 PoseBone records；未进入 `spring_vrm_update_dynamic` ABI 的 Python `current_tails/prev_tails` 不再每帧重复维护，只在 reset 或显式 debug readback 时更新。

Blender 4.5 / Release py311 实测：

- 128 骨、无 collider，最终纯 envelope 实现下三轮独立进程新/旧倍率 `1.079x-1.099x`，通过 `<= 1.15x` 门槛。
