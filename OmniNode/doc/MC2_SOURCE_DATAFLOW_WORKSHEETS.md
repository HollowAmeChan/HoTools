...
- HoTools当前不执行源码固定4次`SolverContact -> SumContact`，因此contact list只可用于检测/lifecycle验收，不修改粒子。Edge最大尺寸不大于epsilon时grid未就绪并保持0个run，不执行除零坐标转换。
...
当前可信覆盖除既有static/Center/constraint/output fixture外，已增加共享collider typed pack、Point/Edge V0 production-order、BoneSpring soft-sphere数值case、自碰撞primitive事务、first-step dynamic逐数组数值、交错双岛grid/hash/run、EE/PT broadphase候选、half contact数值与后续substep更新计数，以及Blender5.1 Mesh零contact过滤和Bone/BoneSpring真实注册。Bone负/零scale仍为显式拒绝边界；self-collision Intersect与SolverContact/Sum仍未接入。
