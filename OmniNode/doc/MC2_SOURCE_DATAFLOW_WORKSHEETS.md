...
13. Distance的vertical/horizontal分类、共享edge全部triangle pair、`0.9396926f` normal-dot、`0.3f`长度比、shear逆序插入、float32 rest与`+0.0`编码，由`mc2_static_build` C++结果唯一生产。staged context在静态构建前创建，capsule-owned ndarray直接注册一次；随后另建immutable spec仅供尚未迁移的host/debug消费，不再tuple→pack后第二次上传。最终由native Final Proxy/Baseline直接生产并move到同一context。Python不得恢复triangle normal、edge map、shear循环或第二份阈值。
