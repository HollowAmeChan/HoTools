...
- 同frame后续substep不重建primitive/AABB；源码改为更新首substep检测出的contact。HoTools当前只完成前者，尚未建立grid坐标/排序和contact list，因此不会进入后续update/solve。
...
当前可信覆盖除既有static/Center/constraint/output fixture外，已增加共享collider typed pack、Point/Edge V0 production-order、BoneSpring soft-sphere fixed/max-length/velocity-reference数值case、自碰撞primitive host/native事务与first-step dynamic逐数组数值，以及Blender5.1 Mesh动态启停和Bone/BoneSpring真实注册。Bone负/零scale仍为显式拒绝边界；self-collision grid/contact/intersect与solve仍未接入。
