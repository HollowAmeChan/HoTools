...
13. Distance的vertical/horizontal分类、共享edge全部triangle pair、`0.9396926f` normal-dot、`0.3f`长度比、shear逆序插入、float32 rest与`+0.0`编码，由`mc2_static_build` C++结果唯一生产。capsule-owned ndarray回读、immutable spec与`update_distance_static`是P-06c内的过渡链；最终由native Final Proxy/Baseline直接生产并move到同一staged context。Python不得恢复triangle normal、edge map、shear循环或第二份阈值。
14. Bending的反向triangle bucket顺序、ordered quad role、float32 dihedral/sign、`<120°`弯曲、`90°..179°` world-volume、sorted membership first-wins与marker `100`，由`mc2_static_build` C++结果唯一生产。capsule-owned ndarray回读、immutable spec与`update_bending_static`是P-06c内的过渡链；最终直接move到消费它的staged context。Python不得恢复angle/volume数值循环、edge map或第二份角度阈值。
