...
11. Mesh Final Proxy的triangle-direction连通层/80°阈值/整层翻转，以及edge union、vertex adjacency、每顶点triangle/flip、final normal/binormal和bind pose，由`mc2_static_build` C++结果唯一生产。staged context在构建前创建，预分配输出直接初始化proxy/frame-producer且revision只增加一次；Finalizer adjacency/triangle/bind数组仅以ndarray transient存活到同次Distance/Center构建结束，生产slot随后只保留coverage/count metadata。Final Proxy local position/normal/tangent/UV仅以同次构建transient供六类producer消费，结束后压缩为identity及debug/writeback所需只读attribute/edge/triangle metadata，不构造immutable Proxy tuple/spec；no-context与staged共用固定dtype流式内容签名。Proxy/Baseline预分配复制边界仍由P-06b/P-06c继续收口。Python不得恢复同名数值实现、第二份阈值、完整生产shadow或派生循环。
12. Mesh Baseline的Fixed并行frontier传播、parent cost/tie-break、children/baseline展平、ZeroDistance、root/depth与local pose，以及Bone共用的ZeroDistance/root/depth/local pose，由`mc2_static_build` C++结果唯一生产。Mesh staged输出注册context并供同次Distance消费后立即丢弃parent/child/baseline/local-pose ndarray，生产slot不再构造完整immutable baseline spec，只保留count/signature与全隐式debug曲线采样所需的只读`float32 depths`；无context Tier A/Bone兼容入口仍返回完整spec。窄`finalize_proxy_attributes` ABI只补写ZeroDistance后的最终attribute且不制造第二次proxy revision；P-06b/P-06c继续删除预分配传递数组，P-06d收口Bone过渡输出。不得恢复已删除的Python producer、完整生产shadow或Bone私有转发。
...
|---|---|---|---|---|
| MeshCloth | `task -> raw mesh arrays -> native四类fingerprint/change mask -> native Final Proxy/Baseline/Distance/Bending/Center/Self producer -> staged native context`；Distance/Bending/Center/Self已move，Finalizer/Baseline/Final Proxy仅在同次构建保留transient，生产slot只留必要metadata；P-06b/P-06c剩余工作是删除Proxy/Baseline预分配复制并由context直接接管，再把Blender bulk snapshot接入native producer | BasePose只上传world positions/component pose；native context内部生产triangle orientation与Center，collider/runtime/scheduler同步到slot context | stable Mesh vertex identity -> GN delta result | prepare失败保留旧slot/result；重建先staging后替换；slot prune/world dispose释放context与BasePose cache owner |
