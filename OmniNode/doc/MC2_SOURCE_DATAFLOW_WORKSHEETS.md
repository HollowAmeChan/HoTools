...
如果 HoTools 按 D-03 把用户 mesh 限定为“最终 proxy 且无需 render mapping”，直接发布 object-local position offset 可以成立，但这是明确的受限输入域和 intentional deviation。它不能用来声称已经迁移 MC2 的 mapping/reduction/render mesh 输出。
...
5. identity mapping mesh 与非 identity weighted mapping mesh 分开验证。
...
- Mesh final-proxy 输入域与 MC2 mapping mesh 输出之间的产品边界；
...
已落盘的 `proxy_static_triangle_contract_001` 是 Tier B **contract-shape fixture**：它手工记录单 fixed 三角形的 proxy/baseline 数组，用于冻结分层、dtype、shape、range 和 validation；它没有运行 MC2 builder，不能关闭上述任何 source parity case，也不能从 Tier B 静默升级为 Tier A。
...
1. Tier A 尚无可信运行宿主；废弃 HoClothUnity 已明确排除，需独立建立最小 Unity 验证工程。
