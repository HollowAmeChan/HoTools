...
当前可信覆盖除既有static/Center/constraint/output fixture外，已增加共享collider typed pack、Point/Edge V0 production-order、BoneSpring soft-sphere fixed/max-length/velocity-reference数值case，以及Blender5.1 Bone task mask/Sphere过滤/真实子步集成。Bone负/零scale仍为显式拒绝边界；self-collision仍未接入。
