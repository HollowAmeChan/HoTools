...
当前可信覆盖：Mesh final proxy、9 个 Mesh baseline case、7 个 Distance build + 2 个 runtime-order case、13 个 Bending static + 3 个 runtime-scratch case、2 个 N2完整参数 dump、1 个 N3 rotation/reset dump、1 个 Center static dump、1 个 Center substep dump、3 个 Center frame-shift dump（world inertia、speed limits、anchor）和 2 个 particle-step dump（gravity/damping、Center inertia）。尚缺 smoothing/time-scale/skip/negative-scale完整 frame shift、Bone connection、connection-aware Bone rotation、collider/self-collision和含 constraint/post commit 的完整粒子/Center frame Tier A闭环。
