...
11. Mesh Final Proxy的triangle-direction连通层/80°阈值/整层翻转，以及edge union、vertex adjacency、每顶点triangle/flip、final normal/binormal和bind pose，由`mc2_static_build` C++结果唯一生产。staged context在构建前创建，预分配输出在转immutable spec前直接初始化proxy/frame-producer且revision只增加一次，不再tuple→pack后二次上传；当前仍存在C++结果→ndarray→context vector复制和供host/debug使用的平行spec。P-06b/P-06c收口时由context直接move结构并删除readback/tuple pack。Python不得恢复同名数值实现、第二份阈值或派生循环。
12. Mesh Baseline的Fixed并行frontier传播、parent cost/tie-break、children/baseline展平、ZeroDistance、root/depth与local pose，以及Bone共用的ZeroDistance/root/depth/local pose，由`mc2_static_build` C++结果唯一生产。Mesh输出在转tuple前直接注册baseline，窄`finalize_proxy_attributes` ABI只补写ZeroDistance后的最终attribute且不制造第二次proxy revision；Bone仍使用共用pose/depth过渡输出。staged context最终接管后必须删除readback、tuple pack及Bone对Mesh数值实现的私有转发，不得恢复已删除的Python producer。
13. Distance的vertical/horizontal分类、共享edge全部triangle pair、`0.9396926f` normal-dot、`0.3f`长度比、shear逆序插入、float32 rest与`+0.0`编码，由`mc2_static_build` C++结果唯一生产。capsule-owned ndarray直接注册staged context一次；Mesh生产slot用固定dtype内容哈希建立稳定签名后只保留record count/signature metadata，不构造或长期持有ranges/targets/rest tuple。Bone与无context Tier A兼容入口仍返回完整spec供后续P-06d上传/oracle；下一步由native Final Proxy/Baseline直接生产并move到同一context，消除ndarray复制。Python不得恢复triangle normal、edge map、shear循环或第二份阈值。
14. Bending的反向triangle bucket顺序、ordered quad role、float32 dihedral/sign、`<120°`弯曲、`90°..179°` world-volume、sorted membership first-wins与marker `100`，由`mc2_static_build` C++结果唯一生产。capsule-owned ndarray直接注册staged context且revision只能增加一次；生产slot用固定dtype内容哈希建立稳定签名后只保留record count/signature metadata，不构造或长期持有quad/rest/marker tuple。无context的Tier A兼容入口仍返回完整oracle spec；下一步直接move到消费context并消除ndarray复制。Python不得恢复angle/volume数值循环、edge map或第二份角度阈值。
15. Self primitive registration的Point/Edge/Triangle source顺序、Fix0/1/2、AllFix、Ignore、int3 particle role与平均baseline depth，由`mc2_static_build` C++结果唯一生产；Mesh staged context直接注册一次，随后生产slot只保留签名/count metadata并立即释放capsule arrays，不再构造或长期持有host primitive tuple。无context的Tier A/Bone兼容入口仍返回完整oracle spec；跨物体membership、半径/厚度采样、grid/contact/intersection由world/native runtime按K-06/K-07合同处理。下一步须消除derived ndarray到context vector复制；Python不得恢复`_primitive_flag`或primitive生产循环。
16. Center static的Fixed边界筛选（排除Move及完全被非Move邻点包围的内部点）、fixed local中心、bind修正姿态平均与initial local gravity，由`mc2_static_build` C++结果唯一生产。Mesh staged context在host spec构造前直接注册一次且`center_static_revision`只能增加一次；逐帧Center pose/inertia继续由native context自产自用。host `MC2CenterStaticSpec`与packer当前仅为过渡兼容、签名和测试保留，P-06c直接move后删除平行数组；Python不得恢复邻接筛选或姿态平均数值循环。
...
|---|---|---|---|---|
| MeshCloth | `task -> raw mesh arrays -> native四类fingerprint/change mask -> native Final Proxy/Baseline/Distance/Bending/Center/Self producer -> 过渡host immutable spec -> native context`；P-06b/P-06c剩余工作是删除平行spec/readback与ndarray复制并由staged context直接move接管 | BasePose只上传world positions/component pose；native context内部生产triangle orientation与Center，collider/runtime/scheduler同步到slot context | stable Mesh vertex identity -> GN delta result | prepare失败保留旧slot/result；重建先staging后替换；slot prune/world dispose释放context与BasePose cache owner |
