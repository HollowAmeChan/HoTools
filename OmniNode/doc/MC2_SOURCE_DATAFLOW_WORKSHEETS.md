...
当前 13 个 static fixture与 3 个 runtime fixture已冻结阈值、double record、volume first-wins、Fixed/Move writeback、scratch average/clear和 negative scale消费。`bending_static.py` 已按 raw order实现 host builder、完整签名和只读 packer，并接入 Mesh slot static bundle；ordered role quad/rest/marker 已上传 native context，合法空数组与未上传状态明确分离，kernel/scratch仍待实现。
...
native V0 已在该限定域内持久化 velocity：reset清零，prediction按上述顺序更新，Distance correction按 `distance_velocity_attenuation` 同步移动 velocity-reference，post再由 `(next - velocityReference) / dt` 提交下一步 velocity。当前 `frameInterpolation=1`，未实现 inertia/wind/collision/friction；这些字段不得用默认零值伪装成完整 parity。
...
当前可信覆盖：Mesh final proxy、9 个 Mesh baseline case、7 个 Distance build + 2 个 runtime-order case、13 个 Bending static + 3 个 runtime-scratch case、2 个 N2完整参数 dump、1 个 N3 rotation/reset dump和 1 个 Center static dump。尚缺 Bone connection、connection-aware Bone rotation、collider/self-collision和完整 frame step的 Tier A闭环。
