...
固定 `center_frame_shift_world_inertia_001` Tier A case 直调 `SimulationCalcCenterAndInertiaAndWind()`，在正缩放、无 fixed list、无 anchor、无 smoothing/速度限制/teleport/sync/culling/skip/stabilization 的域内隔离 `worldInertia`。component从原点移动到 `(10,0,0)`并绕Y旋转90度，`worldInertia=0.25`产生75%的 frame shift：`frameComponentShiftVector=(7.5,0,0)`、rotation为67.5度；oldFrame/now pose按旧 component pivot同时平移旋转。该 fixture当前只属于 verified contract，尚未证明 host/native生产消费。
