...
当前可信覆盖除既有static/Center/constraint/output fixture外，已增加共享collider typed pack、Point/Edge V0 production-order、BoneSpring soft-sphere fixed/max-length/velocity-reference数值case、自碰撞primitive host/native事务，以及Blender5.1 Mesh/Bone/BoneSpring真实注册。Bone负/零scale仍为显式拒绝边界；self-collision动态contact与solve仍未接入。
