...
11. Mesh Final Proxy的triangle-direction连通层/80°阈值/整层翻转，以及edge union、vertex adjacency、每顶点triangle/flip、final normal/binormal和bind pose，由`mc2_static_build` C++结果唯一生产。Blender co/normal/edge必须用`foreach_get`批量读取，normal规范化与fallback tangent由native bulk kernel生产；不得恢复逐vertex Python math。Mesh topology token由读取Final Proxy的owner一次验证并随结果上交，外层不得重复扫描。staged context在构建前创建；Proxy七组vectors与frame producer三组vectors使用受限命名capsule，在完整校验后直接move且revision只增加一次，ZeroDistance最终attribute在move前写入同一owner。Finalizer adjacency/triangle/bind数组仅以ndarray transient存活到同次Distance/Center构建结束，生产slot随后只保留coverage/count metadata。Final Proxy local position/normal/tangent/UV仅以同次构建transient供六类producer消费，结束后压缩为identity及debug/writeback所需只读attribute/edge/triangle metadata，不构造immutable Proxy tuple/spec；no-context与staged共用固定dtype流式内容签名。下一步单一raw snapshot必须供fingerprint/topology/Final Proxy共享，删除`_mesh_payload/_freeze`树物化。
