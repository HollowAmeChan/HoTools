...
15. Self primitive registration的Point/Edge/Triangle source顺序、Fix0/1/2、AllFix、Ignore、int3 particle role与平均baseline depth，由`mc2_static_build` C++结果唯一生产；Mesh staged context在host spec构造前直接注册一次，跨物体membership、半径/厚度采样、grid/contact/intersection仍由world/native runtime按K-06/K-07合同处理。host spec当前只保留签名/count/测试旁路，最终须删除平行primitive arrays；Python不得恢复`_primitive_flag`或primitive生产循环。
