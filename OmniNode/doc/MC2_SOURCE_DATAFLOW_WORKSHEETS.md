...
当前 7 个 build fixture和 2 个 ordered runtime fixture已覆盖这些边界；`distance_static.py` 是生产 host builder。保序 `ranges/targets/rest_signed` 已上传 native context并再次校验 packed source limits、index、finite与 `+0.0` 编码；当前只持有，尚未进入数值 step。
