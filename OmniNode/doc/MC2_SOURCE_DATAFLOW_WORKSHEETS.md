...
native V0 已在该限定域内持久化 velocity/friction/collision state：reset清零，world/anchor frame-shift预步均绕旧component pivot同步变换state position/rotation、velocity-reference与velocity；每个substep按源码执行Tether→Distance→Angle→Bending/Sum→Point/Edge collider→第二次Distance→Motion→post。Distance使用depth与friction inverse mass及animation-pose rest length，post消费collision normal、dynamic/static friction、speed limit和velocityWeight。BoneSpring setup-kind令Point阶段同时接受Fixed，base position取本substep animated base，limit-distance采样curve槽7乘scale ratio；Sphere投影依次执行max-length clamp、按`limitedLength/radius`插值到0.85的反弹衰减、摩擦距离乘3，并将平均push correction同步加入velocity-reference。Team options保留粒子/Center history。wind/self collision仍未实现，不得用外部collider能力伪装成完整collision parity。
...
外部Collider已在受限生产域启用；Self Collision仅完成N0/N1 primitive注册，求解启用前仍需独立worksheet/oracle。不得直接复用旧full-core即宣称完成。
...
- Unity contact queue并行入队顺序不是identity。HoTools把通过broadphase过滤的`(contactType, primitive0, primitive1)`排序去重后保存为frame候选集；这只canonicalize queue顺序，不执行`UpdateContactInfo`，不创建contact cache，也不修改粒子。
- 同frame后续substep不重建primitive/AABB/grid；源码改为更新首substepnarrowphase生成的contact。HoTools当前只有broadphase candidate，没有contact list，因此不会进入后续update/solve。Edge最大尺寸不大于epsilon时grid未就绪并保持0个run，不执行除零坐标转换。
- native context预留slot-owned contact key与intersect record持久容器，static替换和reset会清空；当前没有producer写入它们，inspect计数必须为0。
...
当前可信覆盖除既有static/Center/constraint/output fixture外，已增加共享collider typed pack、Point/Edge V0 production-order、BoneSpring soft-sphere fixed/max-length/velocity-reference数值case、自碰撞primitive host/native事务、first-step dynamic逐数组数值、交错双岛grid排序与Unity hash/run布局、EE/PT broadphase候选正例，以及Blender5.1 Mesh零候选过滤和Bone/BoneSpring真实注册。Bone负/零scale仍为显式拒绝边界；self-collision narrowphase contact/intersect与solve仍未接入。
