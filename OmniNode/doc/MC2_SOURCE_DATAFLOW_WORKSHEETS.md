...
当前可信覆盖：Mesh final proxy、9 个 Mesh baseline case、7 个 Distance build + 2 个 runtime-order case、13 个 Bending static + 3 个 runtime-scratch case、2 个 N2完整参数 dump、1 个 N3 rotation/reset dump、1 个 Center static dump和 1 个 Center substep dump。尚缺 Bone connection、connection-aware Bone rotation、collider/self-collision和含粒子/Center联动的完整 frame Tier A闭环。
