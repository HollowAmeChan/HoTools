...
| range builder | `Runtime/Utility/Data/MultiDataBuilder.cs::ToArray()` |
| range packing | `Runtime/Utility/Data/DataUtility.cs::Pack12_20()` / `Unpack12_20()` |
...
输出表示是逐 vertex 压缩邻接表。以下是 source dump representation，不是 HoTools ABI：

| Array | 类型 | 语义 |
|---|---|---|
| `indexArray` | `uint[vertexCount]` | 12-bit count + 20-bit start。每个 vertex 一项。 |
| `dataArray` | `ushort[N]` | 该 vertex 的目标 local vertex index。 |
| `distanceArray` | `float[N]` | rest distance；正数=vertical，负数=horizontal，`abs(distance) < 1e-8` 时统一写 `+0.0`。 |

源码字段注释仍写“10-22”，但 `CreateData()`、`MultiDataBuilder.ToArray()`、`Pack12_20()` 与 solver 的 `Unpack12_20()` 实际一致使用 12-bit count + 20-bit start。`Pack12_20()` 只截位，不做溢出检查；`dataArray` 又把 target 强制转成 `ushort`。因此 source representation 的硬上限是每 vertex 4095 条记录、data start 1048575、target index 65535。HoTools 展开 ABI 不能继承静默截断，创建时必须显式拒绝超出 source-aligned 输入域的 mesh/记录。

生成步骤：

1. 从 proxy vertex adjacency 遍历每个有向邻接。普通 edge 记录的遍历顺序来自 `NativeParallelMultiHashMap.GetValuesForKey()`，不具备可跨运行冻结的 sibling 顺序保证。
2. 普通 edge 两端都不 Move 时排除；任一端 Invalid 时排除。
3. 若 `target == parent[current]` 或 `current == parent[target]`，归入 vertical，否则归入 horizontal。
4. 每个保留的普通 edge 以 `Pack32Sort(i,target)` 进入 `connectSet`，供 shear 去重。`connectSet` 不去重普通 adjacency 本身。由于 final proxy adjacency 是双向的，同一无向普通 edge 通常在两个 source vertex range 各出现一次。
5. 对每条 canonical edge 的所有相邻 triangle 两两组合，取共享边两侧的 opposite vertices；相邻 triangle 超过两个时不是只取第一对。共享边长度 `< 1e-8` 时跳过；两面法线点积绝对值至少为 `0.9396926`，且 `abs(oppositeDistance/sharedEdgeLength - 1) <= 0.3` 时形成 horizontal shear 候选。
6. shear 只检查 opposite 两端是否都不 Move，并用同一个 `connectSet` 去重；源码没有在 shear 分支重复普通 edge 的 `Invalid` 过滤。若一端 Invalid、一端 Move，记录仍会创建，但运行时 Invalid source vertex 被跳过，Move source vertex仍会读取 Invalid target。此行为必须由 oracle 固定；在没有数值证据前不能擅自“修复”为普通 edge 过滤规则。
7. 新 shear 以 `e3 -> e4` 和 `e4 -> e3` 双向加入 horizontal builder。普通 horizontal 和 shear 都先进入同一个 hash map，最终不能假设普通 edge 一定排在 shear 前；同一 bucket 的 sibling 顺序不稳定。
8. 最终每个 vertex 的输出严格先拼 vertical range，再拼 horizontal range。普通 horizontal 与 shear 不分 kind，均用负 rest distance编码。
9. rest distance `< 1e-8` 时无论原类型都写 `+0.0`。因此 horizontal zero-distance 在 source 数据和 solver 中退化为 zero-distance special case，不再应用 horizontal stiffness；不能仅靠 `distance_rest_signed` 的符号恢复原始类型。
...
`Register()` 把三个数组追加到 manager 全局数组，并在 TeamData 保存 `distanceStartChunk` 与 `distanceDataChunk`；`Exit()` 按 chunk 释放。solver 每个 step 对每个 vertex 读取自己的 count/start，遍历 target，并只把平均 correction 写回当前 vertex。非 BoneSpring 的 Fixed source vertex直接跳过；BoneSpring 允许 Fixed source vertex参与。

`CreateData(proxyMesh, parameters)` 的 `parameters` 形参当前未被读取；Distance static 完全由 final proxy/baseline 派生。非零 runtime 记录按 rest sign 选择 stiffness：vertical 使用 `curve(depth) * simulationPower.y`，horizontal 再乘固定 `0.5`，之后 saturate；rest length 在 `abs(rest) * (InitScale * scaleRatio)` 与当前 `basePos` 距离之间按 `animationPoseRatio` 插值。source/target inverse mass 由 friction、depth、DontMove 与 cloth type 共同决定。当前长度 `< 1e-8` 时该记录不进入平均计数。

zero-distance 记录不使用 stiffness、inverse mass、scale 或 `animationPoseRatio`。源码执行 `addPos = (targetNext - sourceNext) * 0.5`，是覆盖已有累计值而不是 `+=`，随后 `addCnt++`；后续 nonzero 记录仍会在覆盖值上累加。最终 `addPos / addCnt` 同时更新 `nextPos[source]` 和 `velocityPos[source] += averagedAddPos * 0.3`。因此混合 zero/nonzero 的结果可能受同 bucket hash 枚举顺序影响，不能在没有数值 oracle 时直接宣称 target 排序等价。per-vertex range、单边写回、覆盖顺序与平均分母都属于数值语义，而不是可任意换成无向 pair solver 的存储细节。

HoTools 不复制全局 manager chunk，也不把 packed `uint/ushort` 当 ABI。`MC2DistanceStaticV0` 展开为 `int32[N,2] ranges + int32[D] targets + float32[D] rest_signed`；创建时校验 range 连续覆盖、target 越界、finite rest、target 的 source `ushort` 上限和 source packed 上限。zero-distance 只保留 source 的 `+0.0` 语义，不新增一个会影响 kernel 分支的 synthetic kind。用于 debug 的 `distance_kinds` 若存在，只能由 builder 在阈值化前记录，native v0 不消费且不进入 source parity 断言。

static signature 覆盖展开后的完整数组及 schema version。N0 positions、attributes、edges/triangles 或 baseline parent 变化都会重新构建 N1；任一 N1 数组变化导致 context rebuild + reset。Distance stiffness 是 runtime-change parameter，`animationPoseRatio`、scale、friction、depth 和当前 base pose 也是 runtime consumer input，均不进入 N1 signature；固定 `velocityAttenuation=0.3` 与 `horizontalStiffness=0.5` 属于 N2/kernel 常量，不是 static builder 输入。
...
1. parent edge 与非-parent edge 的正负 rest encoding；比较逐 vertex range，不能只比较无向 pair set。
2. 全 Fixed 普通 edge、普通 edge 任一 Invalid、shear 的 Invalid+Move 非对称边界，以及 BoneSpring Fixed source runtime 分支。
...
当前已落盘 13 个 TriangleBending Tier A static fixture：flat dihedral、89.9/100/119.9/120.1/130/178.9/179.5° 阈值分支、bending+volume 双 record、all-Fixed/Invalid 过滤、uniform initial world scale 对 volume 的 8 倍影响、tetra 多 role 的 volume first-wins，以及无 triangle 的 null 返回。fixture 保存 raw Pack64、ordered quads、rest、marker，并把 write arrays 明确标为 diagnostic/runtime-unconsumed。

另有 3 个直接 reflection 调用固定 source `SolverConstraint()`/`SumConstraint()` 的 Tier A runtime fixture，关闭单 record fixed-point scratch、Fixed 粒子不写回、Move contribution average、全 scratch 无条件 clear，以及同一 bending+volume 双 record 在正/负 `negativeScaleSign` 下的不同结果。`test_bending_tier_a.py` 同时校验 static 与 runtime facts；尚未实现 host builder/native consumer。
