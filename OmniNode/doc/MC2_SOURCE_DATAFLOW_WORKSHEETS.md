...
当前可信覆盖：Mesh final proxy、9 个 Mesh baseline case、7 个 Distance build + 2 个 runtime-order case、13 个 Bending static + 3 个 runtime-scratch case，以及 2 个 N2完整参数 dump。尚缺 Bone connection、Center/reset、output rotation、collider/self-collision和完整 frame step的 Tier A闭环。
