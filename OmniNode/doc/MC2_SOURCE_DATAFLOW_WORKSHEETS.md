...
|---|---|---|---|---|
| MeshCloth | `task -> raw mesh arrays -> native四类fingerprint/change mask -> native Final Proxy/Baseline/Distance/Bending/Center/Self producer -> staged native context`；Proxy/frame/Distance/Bending/Center/Self已move，Finalizer/Baseline/Final Proxy仅在同次构建保留transient，生产slot只留必要metadata；P-06b/P-06c剩余工作是删除Baseline预分配复制并由context直接接管，再把Blender bulk snapshot接入native producer | BasePose只上传world positions/component pose；native context内部生产triangle orientation与Center，collider/runtime/scheduler同步到slot context | stable Mesh vertex identity -> GN delta result | prepare失败保留旧slot/result；重建先staging后替换；slot prune/world dispose释放context与BasePose cache owner |
