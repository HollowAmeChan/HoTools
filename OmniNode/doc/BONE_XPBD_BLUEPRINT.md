...
### 4.2 Moving Pin 不改变 rest

Bone XPBD 复用 `MeshXpbdContextV1`。共享 native context 已把两类位置拆开：

- `rest_positions`：建立 stretch/bend rest length，只在静态 reference 更新时改变；
- `pin_positions`：Fixed 粒子本帧动画目标，通过 `update_pin_targets()` 更新。

`update_pin_targets()` 只覆盖 inverse mass 为 `0` 的 position/previous position，并递增 `pin_target_update_count`；它不调用 `update_reference()`，不重建 constraint rest length。Bone adapter 在正常帧先提交当前逻辑动画姿态的 moving Pin，再推进 native；restart 则从当前逻辑姿态 reset，并再次提交 Pin 目标。
...
PoseBone 当前 RNA 可能仍包含 Bone XPBD 上一帧写回。如果把它当成新动画输入，会形成“读自己的输出再积分”的反馈。`bone_xpbd.frame_state` 因此记录 source、pending 与 confirmed basis：

- solver 发布 result 时只建立 pending 指纹；公共 Bone 写回整组成功后产生 `bone_writeback_receipt_v1`，下一次采集只有匹配该 receipt 才把 pending 提升为 confirmed；
- 当前 basis 与上一份 confirmed 写回相同时，采集逻辑输入姿态而不是把 solver 输出当动画；
- 用户动画真正改变 basis 时，以新输入为准；
- frame jump 可以只转移轻量 feedback 状态，不复制 slot/native/result；
- restart 清理后只从当前 RNA `matrix_basis` 重建输入；单位矩阵也是合法输入，不能用“是否为单位矩阵”猜测并恢复旧 source；
...
solver 先把同一 Armature 上所有 task 的目标 Pose matrix 合并，再统一反算每根 `PoseBone.matrix_basis`。这样子骨反算能看到同批父骨的目标矩阵，而不是依赖 Blender 中尚未写入的中间状态。
...
| Pin 双端 | 已实现 | 公共 Bone Pin；根不自动 fixed |
| Moving Pin | 已实现 | 共享 native `update_pin_targets()`，不改 rest length |
| Tail 吸附 | 已实现 | 默认开；只控制输出旋转吸附 |
