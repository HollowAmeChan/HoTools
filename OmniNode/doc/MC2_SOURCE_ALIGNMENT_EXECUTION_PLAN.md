...
1. Blender component/BasePose frame snapshot 到 Center dynamic、再到 Move particle inertia 与 animated step-basic scratch 的纵切已完成；正缩放、无 anchor/smoothing/limit/teleport 的 world-inertia frame shift Tier A oracle已冻结，下一步接 host/native，再扩展 anchor shift、negative-scale teleport 和 baseline重建型 step-basic边界。
