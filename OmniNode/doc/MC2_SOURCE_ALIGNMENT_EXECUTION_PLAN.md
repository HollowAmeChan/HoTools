...
| 顺序 | 对应项 | 工作 | 退出条件 |
|---|---|---|---|
| 1 | P-06b | 收口首次构建的Final Proxy与Baseline所有权：Final Proxy/Baseline staged生产slot均已删除完整immutable spec，Proxy/frame/Baseline vectors已由staged context直接接管，Mesh static注册复制边界已清零；Blender co/normal/edge已bulk读取、fallback tangent已进入C++、topology token已单owner读取，生产Topology已用native fingerprint compact metadata替代`_mesh_payload/_freeze`树。下一步建立单一Mesh raw snapshot，让fingerprint与Final Proxy复用同一批arrays，并将triangle/line staged入口改为全ndarray消费。 | Tier A/static fixture逐数组等价且no-context/staged Proxy signature一致；首次构建小/中/大profile下降；生产ABI不把派生Final Proxy/Baseline大数组回传Python再传回C++。Python实现只可作为明确test oracle保留，生产import为零；同提交清理旧packer/转发/重复校验。 |
