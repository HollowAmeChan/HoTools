...
1. N1 DistanceConstraint static：已完成。`MC2DistanceStaticV0`、保序纯 host builder、显式 dtype packer、7 个 raw packed/expanded build fixture、2 个 ordered runtime fixture及 Blender slot bundle 回归均已落地；all-Fixed source null arrays 在 host 规范化为 N 个 zero ranges。仍不代表 native solver capability。
2. N1 TriangleBending static：`MC2BendingStaticV0` 三数组字段与 consumer 已冻结，排除 runtime 未消费的 write arrays；13 个独立 Unity static fixture已关闭 raw traversal、90/120/179 阈值、bending-then-volume 双 record、volume first-wins、过滤/空态和 initial local-to-world volume；3 个 runtime fixture已关闭 fixed-point scratch sum/average/clear、Fixed/Move 写回和 negative-scale 消费。下一步实现保序 host builder/float32 packer，并把 initial local-to-world 纳入 Mesh static dirty key；不接 native solver。
