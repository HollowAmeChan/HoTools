...
| 2 | P-06c | 收口Mesh约束静态所有权：让Distance/Bending直接消费native Final Proxy/Baseline并move进staged context；迁移Center与Self primitive registration的剩余producer和consumer。 | 对应Tier A、Blender static dirty、self与debug门禁通过；native内部签名可检查；Python不再生产或pack这些数组。逐项迁移、逐项提交，每项完成后审查并删除死代码。 |
