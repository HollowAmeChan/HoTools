...
| Tether N1 | landed gated native kernel | Tether无需新增static数组，复用Move attribute、root/depth与step-basic pose；V0已按prediction后/首次Distance前接入现有native kernel，compression/stretch消费N2槽24/25。显式gate默认关闭，py313 Tier B隔离case与inspect count通过；独立Tier A substep fixture完成前production solver不得开启。 |
