...
| 4 | P-06e | 进行中：收口变化重建。Bone `config=8`已在新staged context内复制Proxy/Baseline/Bone/Distance/Bending/Self六类native static并只重建Center；Python只接收fixed count/signature metadata并组织事务。 | cold build与clone rebuild的Center/整体static signature完全一致，`static_clone_count=6 / center_static_rebuild_count=1 / owned_static_take_count=0`。small/medium/large config由`2.09/5.14/12.24ms`降至`1.75/2.99/6.20ms`；剩余Mesh config及geometry/surface/topology复用矩阵。 |
