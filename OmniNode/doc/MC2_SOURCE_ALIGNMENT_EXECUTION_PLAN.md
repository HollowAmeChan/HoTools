...
| 4 | P-06e | 进行中：收口变化重建。Mesh/Bone `config=8`均在新staged context内复制不变native static并只重建Center；Python只接收fixed count/signature metadata并组织事务。 | cold build与clone rebuild的Center/整体metadata一致；Bone `clone=6`、Mesh `clone=5`，两者`center rebuild=1 / owner take=0`。small/medium/large Mesh config约`1.57/2.56/5.08ms`，large较同轮surface全量重建`20.96ms`快`4.12x`；剩余geometry/surface/topology复用矩阵。 |
