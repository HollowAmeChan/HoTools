...
1. Blender component/BasePose frame snapshot 到 Center dynamic、world-inertia/speed-limit frame shift、anchor cancellation及其组合、movement smoothing、正/零time scale、Fixed-derived Center、单轴negative-scale teleport、Move particle inertia、animated step-basic scratch与正缩放 Mesh baseline step-basic重建的纵切已在受限精确域闭环；anchor域要求稳定同一 identity，负缩放域要求无parent/shear且不与world-inertia shift组合，整体仍要求无configured teleport且 stabilization 完成。scheduler producer/step ratios/skip consumer、slot owner与native step-interpolation复用原语已冻结，下一步让solver驱动plan/advance及多子步循环，并扩展configured teleport/负缩放组合及baseline与负缩放组合。
