...
| Inertia/Center | landed restricted production slice | `center_state.py` 已冻结 center fixed list/local center、initial local gravity、component/anchor frame pose、persistent reset与 substep derived分层；Center static、`center_step_inertia_001`、`particle_step_inertia_001` 与正缩放 `particle_step_baseline_pose_001` 均由 Tier A oracle覆盖。world-inertia、movement/rotation speed-limit、movement smoothing与time-scale shift 已在单位正缩放、无 configured teleport 域内接入 Host persistent、native粒子预步和solver。9个frame-shift fixture覆盖anchor/组合/smoothing/正与零time scale/Fixed Center及X轴scale-sign transition；负缩放路径按源码构建component/Center TRS delta matrix，先变换Center persistent与native粒子history/velocity，再进入inertia shift/step。zero scale使用World `raw_dt`计算100% shift，只更新dynamic/candidate与paused Center history，不执行native/Center step。Blender第4至9帧、自动BasePose负缩放frame 12及独立Fixed World覆盖生产组合；Fixed World同时验证正缩放baseline重建启用与animation pose ratio热关闭。Mesh BasePose frame snapshot 已携带 evaluated component pose；无parent/shear的Mesh负缩放会保留真实axis sign并重建无反射rotation。skip count、configured teleport、负缩放与world-inertia同帧组合、baseline与负缩放组合及父级/shear负缩放仍未完成。 |
