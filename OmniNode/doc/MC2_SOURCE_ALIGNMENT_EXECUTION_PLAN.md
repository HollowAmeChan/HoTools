...
| Inertia/Center | landed restricted production slice | `center_state.py` 已冻结 center fixed list/local center、initial local gravity、component/anchor frame pose、persistent reset与 substep derived分层；Center static、`center_step_inertia_001` 与 `particle_step_inertia_001` 均由 Tier A oracle覆盖。world-inertia、movement/rotation speed-limit、movement smoothing与正time-scale shift 已在无 fixed list、单位正缩放、无 teleport 域内接入 Host persistent、native粒子预步和solver。`center_frame_shift_anchor_001` 冻结纯 `anchorInertia` cancellation，`center_frame_shift_anchor_world_limit_001` 冻结 anchor预变换后组合world inertia与双限速，`center_frame_shift_smoothing_001` 冻结 persistent smoothing velocity，`center_frame_shift_time_scale_001` 冻结缩放simulation dt、raw frame dt与额外shift/moving-speed归一化；Blender第4至7帧覆盖上述生产组合。Mesh BasePose frame snapshot 已携带 evaluated component pose；solver 会按固定点平均/绑定旋转或 component pose 构建 Center world pose，在 reset/连续帧上传、读取并提交 native Center history。fixed-list frame shift、zero-time-scale pause、skip count、完整 negative-scale teleport matrix与 baseline重建型 step-basic仍未完成。 |
...
1. Blender component/BasePose frame snapshot 到 Center dynamic、world-inertia/speed-limit frame shift、anchor cancellation及其组合、movement smoothing、正time scale、Move particle inertia 与 animated step-basic scratch 的纵切已在受限精确域闭环；anchor域要求稳定同一 identity，整体仍要求无 fixed list、单位正缩放、无 teleport且 stabilization 完成。下一步逐项冻结并扩展 fixed-list、zero-time-scale pause、skip count、negative-scale teleport 和 baseline重建型 step-basic边界。
