...
| 5 | P-06f | 进行中：关闭总体性能与边界审计。已复跑同资产benchmark并开始生产可达性清理；早期`InitialState/ParticleBuffer/frame sync`平行host状态链、setup builder、lazy exports与虚假声明已删除。 | Mesh/Bone逐帧均不劣于旧CPP full-core并有明确优势；首次构建与变化重建的剩余差异有实测结论；所有热点都有最终owner，仓库搜索不存在生产Python派生静态、双份数组或仅转发函数，P-06才可关闭。 |
