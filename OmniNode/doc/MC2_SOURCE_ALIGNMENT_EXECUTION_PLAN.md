...
| 3 | P-06d | 进行中：第一步已建立单一`MC2BoneRawSnapshot`，让fingerprint、Line/product connection和Bone static共享names/parents/head-tail/rest matrices；生产Topology只保留`bone_names`与轻量身份，static/frame不再解冻每骨dict。下一步批量化Armature rest读取，再把Bone orientation/transform mapping、Distance、Center与Self直接进入native staged context。 | 显式oracle与生产compact topology signature一致；26/26纯MC2、Blender Bone static/product/frame通过。large Bone profile step约`28.9 -> 20.9ms`，frame input约`11.6 -> 4.45ms`，`_thaw`退出profile；外层build样本本轮为120.5ms且仍需后续稳定收口。最终退出条件仍是不产生第二套Python派生静态树，并在每个producer迁移后删除对应builder/packer/adapter转发。 |
