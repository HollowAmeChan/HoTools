...
| 2 | P-06c | 收口Mesh约束静态所有权：六类数值producer已进入C++；Distance/Bending/Self在校验后直接move命名capsule vector进staged context，生产slot只保留签名/count metadata并删除host array shadow。下一步直接接管Final Proxy/Baseline，处理Center小数组与host oracle兼容，再删除其余平行host arrays、过渡ndarray复制及对应packer。 | 对应Tier A、Blender static dirty、self与debug门禁通过；`owned_static_take_count`固定真实move数量；native内部签名可检查；Python不再生产、长期持有或pack这些数组。逐项迁移、逐项提交，每项完成后审查并删除死代码。 |
