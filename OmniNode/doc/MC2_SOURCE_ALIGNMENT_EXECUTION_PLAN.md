...
| Self Collision N0/N1 registration | landed static registration | `self_collision_static.py` 按源码 Point→Edge→Triangle 顺序从final proxy构建primitive：仅有triangle时生成逐顶点Point、始终生成proxy Edge、随后生成Triangle；冻结kind、Fix0/1/2、AllFix、Ignore、particle indices与平均depth。Mesh/Bone staged bundle将只读`uint32/int32/float32`数组原子上传到slot-owned V0；native校验拓扑计数、kind分段、arity与depth范围，失败不推进revision。context已持有随reset清空的contact/intersect容器，但目前保持为空；broadphase、contact convert/update/cache、Intersect分帧和projection尚未接step，不能声明self-collision可用。py313覆盖host逐项与native事务，Blender5.1覆盖三角Mesh的3/3/1和Line Bone/BoneSpring的0/2/0生产注册。 |
...
| 新 native context/step | landed foundation | 新 V0 已完成 `create -> set setup-kind -> inspect -> update N0/N1/self primitives/parameters/dynamic/center dynamic/colliders -> update step interpolation -> reset -> step -> read/read bone output -> free`，由slot独占并支持staged replacement、输入先验证、幂等释放、双ABI与soak。step执行Center/Move inertia、frame interpolation、animated step-basic、prediction、Tether→Distance→Angle→Bending→Point/Edge collider→第二次Distance→Motion→source-aligned post；BoneSpring soft-sphere复用同一Point stage并将Fixed/post语义切换为spring。两子步Tier A fixture闭环no-collision顺序，Point/Edge/soft-sphere另由raw V0 case覆盖。self primitive仅完成静态注册，仍无self-collision step、wind、Bone triangle output与stats。旧`_native`full-core不计入此项。 |
