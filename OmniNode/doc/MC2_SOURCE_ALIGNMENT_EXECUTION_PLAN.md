...
| Self Collision N0/N1 contact detection stage | landed broadphase + narrowphase lifecycle, no solve | `self_collision_static.py` 按源码 Point→Edge→Triangle 顺序构建并原子上传primitive；native复算proxy顺序、完整flag与平均depth。self mode启用时，首个实际substep在Motion后/post前更新inverse mass、16采样thickness×scale、old/current swept AABB、Edge grid尺度、三类排序grid run及精确Unity.Mathematics 1.2.6 `int3` hash。hash二分搜索建立Edge–Edge唯一pair及Point–Triangle broadphase候选，执行AABB、Ignore、双方AllFix与共享particle过滤，并将并行queue顺序canonicalize为type/index有序去重集。候选随后按源码`UpdateContactInfo`使用old pose最近点/法线和双方位移投影生成contact；thickness、EE s/t/normal与PT sign均量化为binary16。首substep只保留enable contact，后续同frame substep不重建primitive/grid/candidate，而是原地更新contact enable与EE参数并保留PT初始sign。Edge最大尺寸接近0时grid/contact显式未就绪。Intersect分帧、4次SolverContact/Sum和粒子projection尚未接step，不能声明self-collision可用。py313覆盖host/static事务、primitive/grid/hash、EE/PT候选与half contact数值及substep lifecycle，Blender5.1覆盖三角Mesh零contact生产过滤和Line Bone/BoneSpring静态注册。 |
