...
| Self Collision N0/N1 primitive/grid/candidate stage | landed first-step broadphase candidate set | `self_collision_static.py` 按源码 Point→Edge→Triangle 顺序从final proxy构建primitive：仅有triangle时生成逐顶点Point、始终生成proxy Edge、随后生成Triangle；冻结kind、Fix0/1/2、AllFix、Ignore、particle indices与平均depth。Mesh/Bone staged bundle将只读`uint32/int32/float32`数组原子上传到slot-owned V0；native复算proxy顺序、完整flag与平均depth，失败不推进revision。self mode启用时，首个实际substep在Motion后/post前按源码更新fixed/move inverse mass、16采样thickness×scale、old/current swept AABB、Edge max primitive size与3倍grid size；随后按AABB center计算grid，Point/Edge/Triangle三段分别按X→Y→Z稳定排序，提取连续run，并用Unity.Mathematics 1.2.6 `int3` hash按有符号值重排run buffer。当前已用同一hash二分搜索建立Edge–Edge唯一pair及Point–Triangle broadphase候选，执行AABB、Ignore、双方AllFix与共享particle过滤；并行queue顺序canonicalize为type/index有序去重集。同帧后续substep不重建，下一frame才更新；Edge最大尺寸接近0时grid/candidate显式保持未就绪。contact/intersect容器目前保持为空；`UpdateContactInfo` narrowphase、contact convert/update/cache、Intersect分帧和projection尚未接step，不能声明self-collision可用。py313覆盖host/static事务、primitive逐数组数值、双岛grid/hash/run布局及EE/PT候选正例，Blender5.1覆盖三角Mesh零候选生产过滤和Line Bone/BoneSpring静态注册。 |
...
| Runtime parameters N2 | landed foundation | `runtime_parameters.py` 已冻结 V0 value ABI：47 个 `float32`、11 个 `int32`、9x16 个 curve samples；task/slot parameter signature已改用该运行时块，scheduler保持独立签名。solver settings显式提供源码全局 `simulation_frequency=90`与每帧上限3；旧`substeps/iterations`接口保留兼容。Mesh非线性曲线与BoneSpring完整覆写由2个固定commit Tier A dump逐数组验证；prediction、Tether/Distance/Angle/Motion、Point/Edge及BoneSpring collision-limit curve已进入对应consumer，其余尚未实现的wind/self-collision值只保存不伪装消费。 |
...
| 新 native context/step | landed foundation | 新 V0 已完成 `create -> set setup-kind -> inspect -> update N0/N1/self primitives/parameters/dynamic/center dynamic/colliders -> update step interpolation -> reset -> step -> read/read bone/self primitive output -> free`，由slot独占并支持staged replacement、输入先验证、幂等释放、双ABI与soak。step执行Center/Move inertia、frame interpolation、animated step-basic、prediction、Tether→Distance→Angle→Bending→Point/Edge collider→第二次Distance→Motion→首substepself primitive update→source-aligned post；BoneSpring soft-sphere复用同一Point stage并将Fixed/post语义切换为spring。两子步Tier A fixture闭环no-collision顺序，Point/Edge/soft-sphere另由raw V0 case覆盖。self contact/solve仍未进入step，另无wind、Bone triangle output与stats。旧`_native`full-core不计入此项。 |
