...
| 顺序 | 对应项 | 工作 | 退出条件 |
|---|---|---|---|
| 1 | P-06b | 收口首次构建的Final Proxy与Baseline所有权：Final Proxy/Baseline staged生产slot均已删除完整immutable spec，Proxy/frame/Baseline vectors已由staged context直接接管，Mesh static注册复制边界已清零；solver prepare的单一`MC2MeshRawSnapshot`已供fingerprint/Final Proxy共享，fallback tangent已进入C++，生产Topology已compact，staged triangle/line保持ndarray。下一步审计并迁移BasePose兼容mesh topology token，消除最后的逐element JSON签名扫描；随后关闭P-06b。 | Tier A/static fixture逐数组等价且no-context/staged Proxy signature一致；首次构建小/中/大profile下降；生产ABI不把派生Final Proxy/Baseline大数组回传Python再传回C++。Python实现只可作为明确test oracle保留，生产import为零；同提交清理旧packer/转发/重复校验。 |
