...
| 3 | P-06d | 进行中：单一`MC2BoneRawSnapshot`已供fingerprint、Line/product connection和Bone static共享；rest/finalizer/baseline/constraint派生已进入native producer。Proxy/Baseline/Distance/Center/Self/Bone registration共6次直接move，生产packer/copy fallback已删除；slot仅保留必要metadata。下一步建立staged Bone native data，消除同次prepare仍构造的完整Proxy/Finalizer/Baseline/Bone tuple transient。 | 显式oracle与生产compact topology signature一致；`owned_static_take_count=6`；raw native Bone ABI、26/26纯MC2、Blender Mesh BasePose/Bone static/product/frame/全隐式debug通过。large Bone首建约`65.12ms`，为旧CPP的`5.25x`；热帧本轮约`6.60ms`，仍为旧CPP的`2.95x`。native target只增量构建`hotools_native`，无clean/Jolt。最终退出条件仍是不产生第二套Python派生静态树，并在每个producer迁移后删除对应builder/packer/adapter转发。 |
