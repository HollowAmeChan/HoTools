...
| 2 | P-06c | 收口Mesh约束静态所有权：六类数值producer已进入C++；Distance/Bending/Center/Self已直接move；Finalizer staged路径只在同次构建持有ndarray transient并于结束压缩为coverage/count metadata。下一步直接接管Proxy/Baseline，再删除其平行host arrays、预分配ndarray复制及对应packer。 | 对应Tier A、Blender static dirty、self与debug门禁通过；`owned_static_take_count=4`固定真实move数量；生产slot无Finalizer adjacency/bind arrays；native内部签名可检查。逐项迁移、逐项提交，每项完成后审查并删除死代码。 |
