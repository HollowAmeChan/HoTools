...
下一交付是 **Bone Line native rotation/output 的最小纵切**：

1. Bone connection import membership已由8个 Tier A fixture、mode 0..3参数面和不可变host builder闭环，包含119°/121°的严格120°拒绝边界；正缩放Line与triangle override各由3个Tier A fixture闭环到world/local。新增3个Bone Line static fixture已闭环final proxy、transform baseline、bind pose、normal adjustment与写回旋转；Line限定域现已接Blender rest snapshot、slot static signature和staged native N0上传。下一步把已有Line rotation contract移入native post/display阶段，生成private Bone candidate；不得同时扩张Automatic/Sequential或提前启用PoseBone写回。
