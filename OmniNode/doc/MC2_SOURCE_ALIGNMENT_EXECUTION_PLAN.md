...
| 3 | P-06d | 进行中：单一`MC2BoneRawSnapshot`已供fingerprint、Line/product connection和Bone static共享；rest/finalizer/baseline/constraint派生已进入native producer。Proxy/Baseline/Distance/Center/Self/Bone registration共6次直接move，生产packer/copy fallback已删除；`MC2BoneNativeData`已取代四层完整spec树，slot只留metadata。下一步清理static assembly剩余transform/UV/position小型list/tuple转发并做生产可达性复查。 | 显式oracle与生产compact topology/static signature一致；`owned_static_take_count=6`；raw native Bone ABI、26/26纯MC2、Blender Mesh BasePose/Bone static/product/frame/全隐式debug通过。large Bone首建约`21.05ms`，为旧CPP的`16.17x`；热帧约`6.25ms`，为旧CPP的`3.10x`。最终退出条件仍是不产生第二套Python派生静态树，并在每个producer迁移后删除对应builder/packer/adapter转发。 |
