...
| 3 | P-06d | 进行中：单一`MC2BoneRawSnapshot`已供fingerprint、Line/product connection和Bone static共享；rest frame、vertex-to-transform及transform baseline/pose-depth已进入native bulk producer。Baseline/Distance/Center/Self共4次直接move，完整constraint spec与packer退出生产上传；native注册后完整Bone tuple bundle压缩为必要identity/debug/count/signature metadata。下一步把Bone registration剩余8组数组改为native owner直接消费并删除最小packer。 | 显式oracle与生产compact topology signature一致；`owned_static_take_count=4`；26/26纯MC2、Blender Bone static/product/frame/全隐式debug通过。large Bone首建约`65.89ms`，为旧CPP的`5.18x`；热帧约`6.11ms`，为旧CPP的`3.25x`。当前约`72.9KB` host NumPy包含必要debug数组，不能与旧tuple未计入口径直接比较。最终退出条件仍是不产生第二套Python派生静态树，并在每个producer迁移后删除对应builder/packer/adapter转发。 |
