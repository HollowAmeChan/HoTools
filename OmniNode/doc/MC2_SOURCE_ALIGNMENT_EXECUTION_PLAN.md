...
| Collider N0/N1 Point+Edge | production native restricted | `collider_frame.py`将共享World current/previous snapshot转换为MC2不可变帧数组，覆盖Sphere/Capsule/Plane/Box、16组mask、自身owner排除、moving center/segment回退和box signed Z；不读取旧MC2模块、不重扫Blender scene。V0 `update_colliders`逐数组校验并原子持有，Python owner与Mesh production solver在reset/dynamic前按非same-frame上传。Mesh Point按逐粒子、Edge按proxy edge投影，均消费radius curve/scale并持久化friction、collision normal与real velocity，在第二次Distance前更新friction mass、Motion后执行source-aligned post/velocityWeight。Edge view显式区分固定源码`Move=0x02`与旧full-core`0x04` ABI；py313覆盖typed-empty、四primitive、失败不污染、Point/Edge投影/post和完整旧回归，Blender5.1覆盖两帧revision与production链。BoneSpring soft-collider limit-distance及self collision仍未接入。 |
