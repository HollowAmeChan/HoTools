...
1. Blender component/BasePose frame snapshot 到 Center dynamic、world-inertia/speed-limit frame shift、Move particle inertia 与 animated step-basic scratch 的纵切已在无 fixed list、单位正缩放、无 anchor/smoothing/teleport 且 stabilization 完成的精确域闭环；下一步逐项冻结并扩展 anchor、smoothing、fixed-list、time-scale/skip、negative-scale teleport 和 baseline重建型 step-basic边界。
