...
| Self Collision N0/N1 primitive/grid stage | landed registration + first-step broadphase inputs | `self_collision_static.py` 按源码 Point→Edge→Triangle 顺序从final proxy构建primitive：仅有triangle时生成逐顶点Point、始终生成proxy Edge、随后生成Triangle；冻结kind、Fix0/1/2、AllFix、Ignore、particle indices与平均depth。Mesh/Bone staged bundle将只读`uint32/int32/float32`数组原子上传到slot-owned V0；native复算proxy顺序、完整flag与平均depth，失败不推进revision。self mode启用时，首个实际substep在Motion后/post前按源码更新fixed/move inverse mass、16采样thickness×scale、old/current swept AABB、Edge max primitive size与3倍grid size；随后按AABB center计算grid，Point/Edge/Triangle三段分别按X→Y→Z稳定排序，提取连续run，并用Unity.Mathematics 1.2.6 `int3` hash按有符号值重排run buffer。同帧后续substep不重建，下一frame才更新；Edge最大尺寸接近0时grid显式保持未就绪。context已持有随reset清空的contact/intersect容器，但目前保持为空；broadphase candidate/contact detection、contact convert/update/cache、Intersect分帧和projection尚未接step，不能声明self-collision可用。py313覆盖host/static事务、primitive逐数组数值和双岛grid/hash/run布局，Blender5.1覆盖三角Mesh动态启停和Line Bone/BoneSpring静态注册。 |
