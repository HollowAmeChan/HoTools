...
| 顺序 | 对应项 | 工作 | 退出条件 |
|---|---|---|---|
| 1 | P-06b | 收口首次构建的Final Proxy与Baseline所有权：Baseline staged生产slot已删除完整immutable spec，只保留count/signature与隐式debug采样所需depth；下一步让native staged context直接接管Proxy/Baseline现有C++派生vectors，连接Mesh与Bone共用消费端，删除剩余预分配readback、Proxy tuple重打包及仅为过渡ABI存在的host转发。 | Tier A/static fixture逐数组等价；首次构建小/中/大profile下降；生产ABI不把派生Final Proxy/Baseline大数组回传Python再传回C++。Python实现只可作为明确test oracle保留，生产import为零；同提交清理旧packer/转发/重复校验。 |
| 2 | P-06c | 收口Mesh约束静态所有权：六类数值producer已进入C++；Distance/Bending/Center/Self已直接move；Finalizer与Baseline staged路径只在同次构建持有ndarray transient并分别压缩为coverage/count及count/signature/debug-depth metadata。下一步直接接管Proxy vectors与Baseline传递数组，再删除Proxy平行host spec、预分配ndarray复制及对应packer。 | 对应Tier A、Blender static dirty、self与全隐式debug门禁通过；`owned_static_take_count=4`固定真实move数量；生产slot无Finalizer adjacency/bind arrays或Baseline parent/child/local-pose arrays；native内部签名可检查。逐项迁移、逐项提交，每项完成后审查并删除死代码。 |
