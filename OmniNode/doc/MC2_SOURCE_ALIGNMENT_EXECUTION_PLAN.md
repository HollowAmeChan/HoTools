...
| 2 | P-06c | 收口Mesh约束静态所有权：让Final Proxy/Baseline/Distance/Bending/Self派生结构直接move进staged context并删除平行host arrays；迁移Center剩余static producer/consumer。 | 对应Tier A、Blender static dirty、self与debug门禁通过；native内部签名可检查；Python不再生产或pack这些数组。逐项迁移、逐项提交，每项完成后审查并删除死代码。 |
