...
1. N1 DistanceConstraint static：字段、生成规则、zero-distance、shear Invalid 边界、ordering 风险、signature/dirty policy 已冻结为 `MC2DistanceStaticV0`；独立 Unity oracle 已落地 7 个 raw packed/expanded build fixture。下一步先补最小 runtime 数值 case，关闭 mixed zero/nonzero 的覆盖与顺序语义，再实现纯 host builder。不得使用旧 solver 对拍。
