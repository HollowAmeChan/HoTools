...
| 4 | P-06e | 进行中：收口变化重建。C++根据change mask复用不变输入并在新staged context内完成完整静态生产；Python只组织事务、失败回滚与slot原子替换，不复制native static。已补Bone真实rest geometry与仅gravity direction配置变化基准。 | Pin/UV/拓扑/Bone rest变化的重建结果与首次构建一致；失败保留旧context/result；无变化帧零静态分配。当前small/medium/large Bone完整重建约`3.03/8.03/18.97ms`，config全量重建约`2.09/5.14/12.24ms`；下一步config只重建Center并复用其余native static。 |
