...
| Inertia/Center | landed restricted production slice | `center_state.py` 已冻结 center fixed list/local center、initial local gravity、component/anchor frame pose、persistent reset与 substep derived分层；Center static、`center_step_inertia_001`、`particle_step_inertia_001` 与正缩放 `particle_step_baseline_pose_001` 均由 Tier A oracle覆盖。world-inertia、movement/rotation speed-limit、movement smoothing、time-scale shift与configured Keep teleport已在单位正缩放域接入 Host persistent、native粒子预步和solver。11个frame-shift fixture覆盖anchor/组合/smoothing/正与零time scale/scheduler skip count/Fixed Center、X轴scale-sign transition及Keep teleport；Keep fixture冻结源码在smoothing前检测阈值、跳过smoothing并强制100%平移/旋转shift的顺序。`MC2TimeSchedulerState` 已对拍并由slot持有；生产solver现按每slot scheduler执行plan/advance，一次应用negative-scale/frame shift、一次上传完整Center frame，再按ratio仅刷新interpolation并执行多子步，最后只发布一次candidate。Blender 50Hz、0.1秒、每帧上限3已闭环planned/update/skip=`5/3/2`、三个native step、两次interpolation update与4/36度skip shift；独立Keep World验证三native子步仅应用一次10单位/Y90度frame shift、保留smoothing velocity且moving speed归零；reset/same-frame/pause/idle均不伪造step。负缩放路径按源码构建component/Center TRS delta matrix，先变换Center persistent与native粒子history/velocity，再进入inertia shift/step。zero scale使用World `raw_dt`计算100% shift，只更新dynamic/candidate并应用shift，不执行native/Center step。Blender第4至9帧、Keep frame 41、自动BasePose负缩放frame 12及独立Fixed World覆盖生产组合；Fixed World同时验证正缩放baseline parent-first重建、三子步animated blend与animation pose ratio热关闭。Mesh BasePose frame snapshot 已携带 evaluated component pose；无parent/shear的Mesh负缩放会保留真实axis sign并重建无反射rotation。configured Reset teleport、Keep与负缩放同帧组合、负缩放与world-inertia同帧组合、baseline与负缩放组合及父级/shear负缩放仍未完成。 |
...
1. Blender component/BasePose frame snapshot 到 Center dynamic、world-inertia/speed-limit frame shift、anchor cancellation及其组合、movement smoothing、正/零time scale、Fixed-derived Center、configured Keep teleport、单轴negative-scale teleport、Move particle inertia、animated step-basic scratch、正缩放 Mesh baseline step-basic重建与production scheduler/native多子步纵切已在受限精确域闭环；anchor域要求稳定同一 identity，Keep域要求单位正缩放，负缩放域要求无parent/shear且不与world-inertia shift组合，整体仍要求 stabilization 完成。下一步扩展configured Reset teleport、Keep/negative-scale组合及baseline与负缩放组合。
