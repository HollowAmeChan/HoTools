...
| P-10 | 删除后正确性基线 | 待执行 | P-08门禁是删除前基线 | 删除legacy源码、节点、测试和ABI后，native-only编译、raw V0/static、26/26纯MC2、8资产/7脚本、180帧soak、debug及生命周期全通过；生产和测试不再引用删除项 | 是 |
...
`V1-R` 的产品替代资格已由P-08放行，但“可删除旧实现”不等于“代码已经进入维护态”。当前进入 **P-09..P-16删除后工程收尾**：先删除并复验，再完成Python/C++职责、依赖、原子边界、稳定蓝本和热点基线；全部关闭前`solver_acceptance_blocker=True`保持正确。

当前开放阻塞：

1. `P-09/P-10`：按冻结清单移除旧实现，并建立不借助legacy测试/ABI的正确性基线。
2. `P-11..P-14`：代码事实与职责审计、Python/C++重组及依赖洁净度终审。
3. `P-15/P-16`：单一稳定蓝本、热点性能基线与维护态总门禁。
