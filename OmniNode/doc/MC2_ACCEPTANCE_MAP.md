...
| P-06 | 新旧性能对比与C++边界审计 | 待审计 | 同资产小/中/大benchmark已覆盖旧CPP full-core与新Physics World的构建、真实重建、逐帧、debug、host内存和Python分配；逐帧Mesh/Bone保持快于旧CPP。Frame orientation、Bone pose rotation、Center已由native context自产自用；静态raw数组由C++生成并分类`1/2/4/8` change mask，旧JSON签名与重复key已删除。P-06b/P-06c已把Mesh Final Proxy、Baseline、Distance、Bending及Self primitive派生迁入可由context move接管的C++ static-build结果；Bone共用pose/depth与Distance消费相同kernel，旧Python数值producer、阈值与私有函数转发已删除。staged context已前移到只读prepare，五类Mesh派生输出均在转immutable spec前直接注册；窄attributes finalize ABI保留ZeroDistance语义，Blender门禁固定各static revision均为1。大Mesh首构约1188→838→804.6→631.2→300.4→281.7ms、重建约288.5ms，现为旧CPP的2.26x；大Bone首构约114.8ms，现为旧CPP的3.13x；raw ABI/context、Mesh/Bone Tier A、Blender Final Proxy/BasePose/Bone static/product/frame/debug/soak及跨物体Self交互通过 | 五类Mesh派生仍平行构造host immutable spec，且context注册会从过渡ndarray复制native vectors；large Mesh主成本仍为Final Proxy host immutable spec/topology路径，profile约325.9ms。P-06b/P-06c须由context直接move派生结构并删除过渡spec/readback，再按P-06d..P-06f迁移Bone static和变化重建后关闭 | 是 |
