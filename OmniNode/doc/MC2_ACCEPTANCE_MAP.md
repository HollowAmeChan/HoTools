...
| P-13 | C++原子化与文件重组 | 执行中 | MC2 binding与fingerprint已分别抽出唯一owner；`mc2_context_internal.hpp`已固定context/interaction唯一state布局，无Python对象或shadow。3.11 native-only增量构建无Jolt，native `26/26`与MC2 raw `18/18`通过 | 尚需将约7.1k行context按lifecycle、static、frame/step、interaction、debug/readback继续拆分；不得恢复host round-trip | 是 |
