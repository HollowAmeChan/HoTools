...
| P-06 | 新旧性能对比与C++边界审计 | 待审计 | 同资产小/中/大benchmark已覆盖旧CPP full-core与新Physics World的构建、真实重建、逐帧、debug、host内存和Python分配；逐帧Mesh/Bone保持快于旧CPP。Frame orientation、Bone pose rotation、Center已由native context自产自用；静态raw数组由C++生成并分类`1/2/4/8` change mask，旧JSON签名与重复key已删除。P-06b/P-06c已把Mesh Final Proxy、Baseline和Distance派生迁入可由context move接管的C++ static-build结果；Bone共用pose/depth与Distance消费相同kernel，旧Python数值producer、阈值与私有函数转发已删除。大Mesh首构约1188→838→804.6→631.2ms，现为旧CPP的1.26x；大Bone首构284.9→142.9ms，现为旧CPP的3.04x；Baseline/Distance已退出首构profile前列；raw ABI、Mesh/Bone Tier A、Blender Final Proxy/BasePose/Bone static/product/frame/debug通过 | Final Proxy/Baseline/Distance仍经host ndarray/immutable spec后再上传context；large Mesh剩余首构热点为Bending约556.8ms及Final Proxy host spec路径约353.3ms。P-06b/P-06c须删除过渡readback/packer并由staged context直接消费，再按P-06d..P-06f迁移Bone static和变化重建后关闭 | 是 |
