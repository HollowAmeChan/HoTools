...
| P-06 | 新旧性能对比与C++边界审计 | 待审计 | 同资产小/中/大benchmark已覆盖旧CPP full-core与新Physics World的构建、真实重建、逐帧、debug、host内存和Python分配；当前逐帧Mesh约1.26..1.80x、Bone约1.86..2.34x，大Bone首次构建约1.00x。Frame orientation、Bone pose rotation、Center已改为native context自产自用；Mesh/Bone静态raw数组由C++生成topology/geometry/surface fingerprint，slot context持有并分类`1/2/4/8` change mask；大Mesh热路径fingerprint约3.33ms；Pin/UV/geometry/gravity config/Bone rest/hierarchy/same-frame/纯Pose均有Blender 5.1回归，旧JSON签名producer及重复signature/key状态已删除 | Mesh首次构建仍只有旧实现的约0.58..0.60x，剩余热点集中在Final Proxy/Baseline、Distance/Bending/Bone static的Python生产。按执行计划P-06b..P-06f逐项迁入native并在每步删除旧producer/packer/转发后，才允许关闭 | 是 |
