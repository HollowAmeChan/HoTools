...
| P-06 | 新旧性能对比与C++边界审计 | 待审计 | 同资产小/中/大benchmark已覆盖旧CPP full-core与新Physics World的构建、真实重建、逐帧、debug、host内存和Python分配；逐帧Mesh/Bone保持快于旧CPP。Frame orientation、Bone pose rotation、Center已由native context自产自用；静态raw数组由C++生成并分类`1/2/4/8` change mask，旧JSON签名与重复key已删除。P-06b已把Mesh Final Proxy派生及Fixed/Move frontier、parent/children、baseline/root/depth/local pose完整迁入可由context move接管的C++ static-build结果；Bone共用pose/depth也消费同一kernel，旧Python数值producer与私有函数转发已删除。大Mesh首构约1188→838→804.6ms、相对旧实现约0.60x→0.76x→0.946x；Baseline已退出首构profile前列；raw ABI、Mesh/Bone Tier A、Blender Final Proxy/BasePose/Bone static/product/frame/debug通过 | Final Proxy/Baseline仍经host预分配buffer转为immutable spec供Distance/Bending等下游使用；large Mesh剩余首构热点为Bending约559ms、Distance约526ms及Final Proxy host spec转换。P-06b须先删除过渡readback/packer并由staged context直接消费，再按P-06c..P-06f迁移约束/Bone static和变化重建后关闭 | 是 |
