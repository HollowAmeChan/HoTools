...
| P-06 | 新旧性能对比与C++边界审计 | 待审计 | 同资产小/中/大benchmark已覆盖旧CPP full-core与新Physics World的构建、真实重建、逐帧、debug、host内存和Python分配；逐帧Mesh/Bone保持快于旧CPP。Frame orientation、Bone pose rotation、Center已由native context自产自用；静态raw数组由C++生成并分类`1/2/4/8` change mask，旧JSON签名与重复key已删除。P-06b/P-06c已把Mesh Final Proxy、Baseline、Distance、Bending、Center static及Self primitive六类派生迁入C++ static-build结果；Bone共用pose/depth与Distance消费相同kernel，旧Python数值producer、阈值与私有函数转发已删除。staged context已前移到只读prepare；Distance/Bending/Self通过受限命名capsule在完整校验后直接move vector进context，生产slot只保留签名/count metadata，不再构造或持有host constraint/primitive tuple，`owned_static_take_count=3`由Blender BasePose固定；窄attributes finalize ABI保留ZeroDistance语义，各static revision均为1。大Mesh首构约1188→838→804.6→631.2→300.4→281.7ms、重建约288.5ms，现为旧CPP的2.26x；大Bone首构约114.8ms，现为旧CPP的3.13x；raw ABI/context、Mesh/Bone Tier A、Blender Final Proxy/BasePose/Bone static/product/frame/debug/soak及跨物体Self交互通过 | Final Proxy/Baseline/Center三类Mesh派生仍平行构造host immutable spec并从过渡ndarray复制native vectors；large Mesh主成本仍为Final Proxy host immutable spec/topology路径，profile约325.9ms。P-06b/P-06c须直接接管Final Proxy/Baseline、评估Center host oracle兼容并删除其余过渡spec/readback，再按P-06d..P-06f迁移Bone static和变化重建后关闭 | 是 |
