...
| P-06 | 新旧性能对比与C++边界审计 | 待审计 | 同资产小/中/大benchmark已覆盖旧CPP full-core与新Physics World的构建、真实重建、逐帧、debug、host内存和Python分配；逐帧Mesh/Bone保持快于旧CPP。Frame orientation、Bone pose rotation、Center已由native context自产自用；静态raw数组由C++生成并分类`1/2/4/8` change mask，旧JSON签名与重复key已删除。P-06b/P-06c已把Mesh Final Proxy、Baseline、Distance和Bending派生迁入可由context move接管的C++ static-build结果；Bone共用pose/depth与Distance消费相同kernel，旧Python数值producer、阈值与私有函数转发已删除。staged context已前移到只读prepare，Distance/Bending capsule arrays直接注册且Blender门禁固定revision各为1，不再经tuple/spec重打包后二次上传。大Mesh首构约1188→838→804.6→631.2→300.4ms，现为旧CPP的2.21x；大Bone首构142.9→118.9ms，现为旧CPP的2.89x；Baseline/Distance/Bending均已退出首构profile前列；raw ABI、Mesh/Bone Tier A、Blender Final Proxy/BasePose/Bone static/product/frame/debug/soak通过 | Final Proxy/Baseline仍经host预分配readback与immutable spec上传；Distance/Bending仍平行构造host spec且context注册会复制native vectors。large Mesh剩余主成本为Final Proxy host immutable spec/topology路径，profile约322.9ms。P-06b/P-06c须由context直接move派生结构并删除过渡spec/readback，再按P-06d..P-06f迁移Bone static和变化重建后关闭 | 是 |
