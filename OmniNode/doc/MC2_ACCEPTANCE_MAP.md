...
| P-06 | 新旧性能对比与C++边界审计 | 待审计 | 同资产小/中/大benchmark已覆盖旧CPP full-core与新Physics World的构建、真实重建、逐帧、debug、host内存和Python分配；逐帧Mesh/Bone保持快于旧CPP。Frame orientation、Bone pose rotation、Center已由native context自产自用；静态raw数组由C++生成并分类`1/2/4/8` change mask，旧JSON签名与重复key已删除。P-06b已把Mesh triangle-direction、edge union、vertex adjacency、triangle flip records、final normal/binormal和bind pose迁入可由context move接管的C++ static-build结果并删除Python数值producer；大Mesh首构约1188→838ms、相对旧实现约0.60x→0.76x；raw ABI、Tier A逐数组、Blender Final Proxy/BasePose/debug通过 | Final Proxy当前仍经host预分配buffer转为immutable spec供下游使用；Baseline、Distance/Bending/Bone static仍由Python生产。按执行计划P-06b..P-06f继续迁入native并删除过渡readback/packer后，才允许关闭 | 是 |
