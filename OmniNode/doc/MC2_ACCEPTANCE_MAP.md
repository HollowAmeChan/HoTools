...
| P-13 | C++原子化与文件重组 | 执行中 | MC2 74个nanobind注册已抽出`mc2_bindings.cpp/.hpp`单一owner；通用shell只保留PropertyCurve/SpringBone 15个binding和`bind_mc2`。3.11 native-only增量构建无Jolt，全量native `26/26`通过 | 尚需将约7.5k行context按state/lifecycle、static、frame/step、interaction、debug/readback建立内部owner边界；不得恢复host round-trip | 是 |
