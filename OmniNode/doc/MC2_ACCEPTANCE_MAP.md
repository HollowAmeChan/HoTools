...
| P-06 | 新旧性能对比与C++边界审计 | 待审计 | 同资产小/中/大benchmark已覆盖旧CPP full-core与新Physics World的构建、真实重建、逐帧、debug、host内存和Python分配；逐帧Mesh/Bone保持快于旧CPP。Frame orientation、Bone pose rotation、Center已由native context自产自用；静态raw数组由C++生成并分类`1/2/4/8` change mask，旧JSON签名与重复key已删除。P-06b首项已把Mesh triangle-direction迁为可由context复用的C++ static-build kernel并删除Python算法，大Mesh首构约1188→969ms、相对旧实现约0.60x→0.66x；Tier A逐数组、Blender Final Proxy/BasePose通过 | Final Proxy其余orientation/adjacency/bind、Baseline、Distance/Bending/Bone static仍由Python生产；Mesh首次构建仍慢于旧实现。按执行计划P-06b..P-06f逐项迁入native并在每步删除旧producer/packer/转发后，才允许关闭 | 是 |
