...
| P-06 | 新旧性能对比与C++边界审计 | 待审计 | 同资产小/中/大benchmark已覆盖旧CPP full-core与新Physics World的构建、真实重建、逐帧、debug、host内存和Python分配；逐帧Mesh/Bone保持快于旧CPP。Frame orientation、Bone pose rotation、Center已由native context自产自用；静态raw数组由C++生成并分类`1/2/4/8` change mask，旧JSON签名与重复key已删除。P-06b/P-06c已把Mesh六类静态派生迁入C++ static-build结果；Bone共用pose/depth与Distance消费相同kernel，旧Python数值producer、阈值与私有函数转发已删除。staged context已前移到只读prepare；Distance/Bending/Center/Self通过受限命名capsule在完整校验后直接move vector进context，生产slot只保留签名/count metadata；host-rotation frame也调用同一native Center producer。Finalizer、Baseline与Final Proxy的staged大数组只在同次构建内作为ndarray transient消费，结束后分别压缩为coverage/count、count/signature/debug-depth，以及identity/attributes/edges/triangles metadata；生产slot不再构造完整immutable Proxy/Baseline/Finalizer spec。Proxy的no-context/staged签名统一使用固定dtype流式内容哈希。`owned_static_take_count=4`、各static revision=1及无local pose shadow由Blender BasePose固定。大Mesh首构序列约1188→838→804.6→631.2→300.4→281.7→217.3→177.8→154.9→118.3ms；最新独立profile中Proxy spec约81.4ms与Baseline约50.5ms均退出顶部。host NumPy约471KB，较前次上升来自debug/writeback所需edge/triangle metadata由tuple改为可计量的只读ndarray，不代表完整shadow恢复。大Bone本轮受全局负载影响不作为新基线；raw ABI/context、26/26纯MC2、Blender Final Proxy/BasePose/Bone static/product/frame/全隐式debug/180帧soak及跨物体Self交互通过 | Proxy/Baseline native输出仍经过Python预分配ndarray注册/复制边界；当前profile热点转为Blender Mesh source topology冻结、逐vertex normal/fallback tangent提取以及两次topology signature。P-06b/P-06c须让staged context直接接管剩余Proxy/Baseline vectors，并把首帧/变化重建的Blender bulk snapshot交给C++生产消费，再按P-06d..P-06f迁移Bone static和变化重建后关闭 | 是 |
