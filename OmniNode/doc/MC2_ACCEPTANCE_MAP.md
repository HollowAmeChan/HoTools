...
| P-13 | C++原子化与文件重组 | 执行中 | MC2 74个nanobind注册已抽出`mc2_bindings.cpp/.hpp`单一owner；通用shell只保留其他15个binding和`bind_mc2`。无context state的Mesh/Bone fingerprint已抽出`mc2_fingerprint.cpp`。3.11 native-only增量构建无Jolt，native `26/26`与MC2 raw `18/18`通过 | 尚需将约7.3k行context按state/lifecycle、static、frame/step、interaction、debug/readback建立内部owner边界；不得恢复host round-trip | 是 |
...
| P-15 | MC2稳定蓝本合一 | 待执行 | 当前验收表、执行计划、worksheet仍含迁移流水 | 仿照`SPRINGBONE_VRM_BLUEPRINT.md`生成单一`MC2_BLUEPRINT.md`，只保留当前事实、职责表、产品决策、数据流、性能边界与扩展检查表；删除三份专项迁移文档并更新全部路由 | 是 |
...
`V1-R` 的产品替代资格已由P-08放行，P-09/P-10删除与基线、P-11职责审计和P-12 Python重组均已关闭。当前进入 **P-13 C++原子化与文件重组**，随后完成依赖洁净度、稳定蓝本和热点基线；全部关闭前`solver_acceptance_blocker=True`保持正确。

当前开放阻塞：

1. `P-13/P-14`：C++原子化与文件重组、依赖洁净度终审。
2. `P-15/P-16`：单一稳定蓝本、热点性能基线与维护态总门禁。
