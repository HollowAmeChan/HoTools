...
| 参数曲线 | 已接入 “基础值 + 浮点曲线” 形式，覆盖 distance、bend、angle restoration、angle limit、max distance、backstop distance 等当前热路径；但还不是完整 MC2 参数面，damping/radius/tether compression/motion stiffness/use flags 等仍需补齐。 | 部分完成 |
...
| `angleRestorationConstraint` | `useAngleRestoration`、stiffness 曲线、velocityAttenuation、gravityFalloff。 | stiffness 曲线已接；启用用“值是否大于 0”隐式表达；velocityAttenuation/gravityFalloff 仍用常量。 | 需要补显式 use、velocityAttenuation、gravityFalloff。 |
| `angleLimitConstraint` | `useAngleLimit`、limitAngle 曲线、stiffness。 | limitAngle 曲线和 stiffness 已接；启用用“值是否大于 0”隐式表达。 | 可用但不完全等价；需要补显式 useAngleLimit。 |
| `motionConstraint` | `useMaxDistance`、maxDistance 曲线、`useBackstop`、backstopRadius 标量、backstopDistance 曲线、stiffness 标量。 | maxDistance/backstopDistance 曲线已接；backstopRadius 标量已接；motion stiffness 固定 1.0；use flags 由数值隐式表达。 | 需要补 useMaxDistance/useBackstop 与 motion stiffness 输入。 |
| `tetherConstraint.distanceCompression` | MeshCloth 可配置标量 compression；stretch 为系统常量。 | 目前 compression 使用系统常量 `TETHER_COMPRESSION_LIMIT`，没有节点输入。 | 需要暴露 tether compression 标量；stretch 仍可保持常量。 |
...
| 参数曲线 | distance/bend/angle restoration/angle limit/max distance/backstop distance 的值 + 曲线输入 | `PropertyCurve/*`、`physicsMC2/params.py`、`physicsMC2/solver.py`、`physicsMC2/__init__.py` | 已接入当前热路径；仍需补 damping/radius/tether/motion stiffness 等 MC2 参数面 |
...
| tether compression 输入 | MC2 `distanceCompression` 是 MeshCloth 可配置参数；当前 HoTools 使用系统常量。 | P1 |
| motion stiffness 输入和 use flags | MC2 有 `useMaxDistance/useBackstop/stiffness`；当前 HoTools 用数值隐式启用，motion stiffness 固定 1.0。 | P1 |
| angle use/attenuation/falloff | MC2 有显式 `useAngleRestoration/useAngleLimit`、restoration velocityAttenuation、gravityFalloff；当前 HoTools 主要用数值和常量隐式表达。 | P1 |
...
| P1 | 补齐 MC2 剩余曲线参数面。 | damping/radius/tether compression/motion stiffness/angle use 与 attenuation/falloff 等参数补齐后，当前 MeshCloth 参数面才可接近完整对齐。 |
