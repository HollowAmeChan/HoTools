...
| TeamState 字段 | `TeamData` 持有 flags、updateCount/skipCount、frameInterpolation、timeScale、scale/negativeScale、gravityDot/gravityRatio、velocityWeight/blendWeight、sync/culling/skipWriting、各 DataChunk。 | `MC2TeamState` 只保存 `frame_interpolation/scale_ratio/negative_scale*/animation_pose_ratio/gravity*/velocity_weight/blend_weight` 摘要，并从 legacy state 同步。solver 仍主要读写 dict 字段，不直接以 TeamState 为数据源。 | 摘要字段已开口，但 Team 生命周期未完成。还缺 flags、updateCount/skipCount、culling/sync/skipWriting/scaleSuspend、DataChunk 索引语义。 |
| CenterState 字段 | `CenterData` 管 component old/frame/now/step pose、anchor、frame shift、inertia vector/rotation、angular velocity、initLocalGravityDirection、smoothingVelocity、negativeScaleMatrix。 | `inertia.py` 的 `inertia_state` 已覆盖普通对象路径的 anchor、frame shift、negative scale teleport、smoothing、step vector/rotation、angular velocity 等一部分；`MC2CenterState.inertia_state` 目前只是从 legacy dict 摘要同步，solver 仍读写 `state["inertia_state"]`。 | 物理行为已有部分对齐，但容器迁移没完成。下一步应把 `inertia_state` 变成 CenterState 的权威字段，并镜像回 legacy 兼容层。 |
| native 持久上下文 | MC2 的 particle/constraint/base/work buffers 长期存在，`WorkBufferUpdate()` 只按粒子数 resize work buffer，normal/split job 直接拿数组和 DataChunk。 | `native_context` 字段存在但没有真实 C++ handle。`native_bridge.state_arrays_for_native(state)` 每次从 dict 构建 ABI view；`solve_meshcloth_core()` 每帧传入整包 numpy 数组和 runtime 参数。 | 关键性能地基未完成。现在具备把 handle 挂到 `MC2RuntimeOwner.native_context` 并由 dispose 释放的入口，但还没有 create/update/free native context API。 |
| 拓扑/静态数组缓存 | MC2 proxy common/baseLine/distance/bending/fixed/particle chunks 都是稳定数组区段。 | `topology_cache` 已被节点入口传入 `mesh_build.mesh_signature_key()` 和 build_state；正常运行用轻量 key 避免每帧完整拓扑 hash。 | Python 侧缓存已减轻 rebuild，但 C++ 仍未持久化 topology arrays。可优先下沉 parent/root、baseline、distance、bending、rest/local pose 等拓扑固定数组。 |
| 参数/曲线缓存 | MC2 `ClothParameters` 是 unmanaged 参数数组，曲线采样/参数由 manager 压成 job 可读数据。 | `runtime_params.py` 已用 `curve_cache` 缓存 param slot 与采样；`solve_setup.params.*` timing 已能观察参数构建成本。 | 参数层已有缓存地基。后续可按 dirty key 把固定参数/曲线样本推进 native context，避免每帧重复传整包样本。 |
...
1. `MC2TeamState` 还不是 solver 的权威数据源；目前只是 legacy state 的摘要镜像。
2. `MC2CenterState.inertia_state` 还不是 `inertia.py` 的权威数据源；`state["inertia_state"]` 仍在热路径里传递。
3. `native_context` 还没有 C++ 资源、没有 dirty key、没有 create/update/free ABI，也没有拓扑固定数组驻留。
4. `native_cache` 目前主要承载 debug/native slot，不等同 MC2 的 persistent NativeArrays。
5. C++ full-core 仍每帧通过 `state_arrays_for_native()` 和 `solve_meshcloth_core()` 消费整包数组；这和 MC2 `DataChunk + ExNativeArray` 模式还有明显距离。
...
1. 先把 `inertia_state` 迁入 `MC2CenterState`，提供读写 helper，solver/inertia 以 CenterState 为权威，legacy dict 只做镜像。
2. 再把 `animationPoseRatio/gravityDot/gravityRatio/velocityWeight/blendWeight/scaleRatio/negativeScale*` 迁入 `MC2TeamState`，solver 读 TeamState，legacy dict 只保留兼容字段。
3. 设计 native context ABI：`create_meshcloth_mc2_context(static_arrays, topology_key)`、`update_meshcloth_mc2_context_params(handle, param_key, samples)`、`solve_meshcloth_mc2_context(handle, dynamic_inputs)`、`free_meshcloth_mc2_context(handle)`。
4. 第一批下沉 native context 的内容只放拓扑固定数组：parent/root、baseline、vertex local pose、distance neighbor/rest/type、bend/dihedral/volume、collision local radii、attributes/depths。不要先下沉 depsgraph/BasePose 读取。
5. 第二批下沉参数固定/曲线样本：damping、distance/bend/angle/motion samples、component enable flags、normalAxis、tether/motion scalar params，并用 dirty key 控制更新。
...
5. 第一阶段 owner 包住现有 dict 已完成；第二阶段必须把 TeamState、CenterState 逐步变成 solver 权威数据源，再拆 MeshState、CurveCache、NativeContext。
