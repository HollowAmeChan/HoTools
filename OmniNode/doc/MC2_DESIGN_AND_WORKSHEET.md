...
5. 不复制 MC2 wall-clock/catch-up/timeScale 调度。Blender 输出帧率给出真实帧长：`frame_dt = fps_base / fps`，`step_dt = frame_dt / substeps`。
...
| RuntimeOwner | `_OmniCache.replace(owner)` 用于重建/跳帧，`_OmniCache.mutate(owner)` 用于连续帧；owner 实现 dispose/debug snapshot。 | 地基已落地 |
| TeamState | 已收进 `frame_delta_time/step_delta_time/update_count/skip_count/substep_count/frame_interpolation/time_scale`、`skip_writing/culling/sync/scale_suspend`、scale/negativeScale、animation/blend/gravity/velocityWeight。 | 字段已权威化，策略未完全接入 |
| CenterState | 已收进 inertia/CenterData 摘要、`MC2ParticleState` 和 `MC2BasePoseState`；solver 从这些子状态读取并在 post_pack 提交，同时镜像 legacy ABI。 | 第一批权威化完成 |
| Native context | `MC2NativeContext` 持有 C++ capsule、dirty key、静态数组、param slots、param sample arrays；C++ context 已驻留拓扑静态数组和 per-vertex 参数样本数组。 | 第一、二批 native residency 完成 |
...
| 项目 | 当前缺口 | 优先级 |
| --- | --- | --- |
| 拓扑/native context schema | 拓扑约束数组和一部分 native ABI 打包仍通过 legacy dict 流转。下一步应继续把 topology/native context 边界从 legacy dict 中拆出来。 | P0 |
| Team 生命周期策略 | `skip_writing/culling/sync/scale_suspend/time_scale` 已是可观测字段，但尚未驱动真实 Blender 调度行为。需要明确等价行为或显式不做。 | P0 |
...
1. P0：继续收紧 `MC2CenterState`，把 topology/native context 边界从 legacy dict 中拆出，让 legacy dict 只做 ABI 镜像。
2. P0：收敛 `MC2TeamState` 策略字段。对 `skip_writing/culling/sync/scale_suspend/time_scale` 逐项决定 Blender 等价行为或明确不做。
3. P0：保持 Python/C++ parity harness 作为每次 native 改动的底线测试。
4. P1：评估 dynamic particle/base/work buffers native-owned。前提是 writeback、BasePose、debug timing 和 Python reference 都稳定。
5. P1：继续校准复杂骨架/修改器链 BasePose 场景，尤其是顶点顺序变化、高模绑定、负缩放和 root/parent 链。
...
4. native fallback 必须保留：旧 `solve_meshcloth_mc2` 数组 ABI 和 `solve_meshcloth_mc2_context` 仍用于定位 Python/C++ 差异。
...
4. `velocityWeight/blendWeight` 已按基础 TeamStepUpdate 语义接入；完整 culling/skip/timeScale/render interpolation 不复制或待明确。
...
| `physicsMC2/state.py` | `MC2RuntimeOwner`、`MC2TeamState`、`MC2CenterState`、`MC2ParticleState`、`MC2BasePoseState`、schema guard、cache/native context 生命周期。 |
