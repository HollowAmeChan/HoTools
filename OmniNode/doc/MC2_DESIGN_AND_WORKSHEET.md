...
| native 持久上下文 | MC2 的 particle/constraint/base/work buffers 长期存在，`WorkBufferUpdate()` 只按粒子数 resize work buffer，normal/split job 直接拿数组和 DataChunk。 | `native_context` 已升级为 `MC2NativeContext`，记录 topology/config/param dirty key、buffer 规模、native capsule handle、static arrays cache、param slots cache 与 debug snapshot，并由 CenterState dispose。`native_bridge` 已拆出 `static_state_arrays_for_native()` 与 `dynamic_state_arrays_for_native()`；C++ 侧已新增 `create/update_static/update_static_arrays/update_params/info/free/solve_meshcloth_mc2_context`。C++ context 现在持久保存 attributes/depth/root/tether/parent/baseline/vertex local/distance/bend/dihedral/volume/edges 等拓扑静态数组；C++ full-core 可走 context solve，每帧只传 base pose、动态粒子、参数采样、collider、substep inertia 等动态输入。py311/py313 native 已构建，Blender Python py311 最小 context solve smoke 已通过。 | 第一批 native residency 已落地：生命周期、dirty-key、静态/动态 ABI 分层、参数 slot 缓存、C++ 静态拓扑数组驻留和 context solve ABI 均已完成。仍未完成：参数/曲线样本驻留、dynamic particle/work buffers native-owned、DataChunk 化、多 center 调度和 split/job。 |
| 拓扑/静态数组缓存 | MC2 proxy common/baseLine/distance/bending/fixed/particle chunks 都是稳定数组区段。 | `topology_cache` 已被节点入口传入 `mesh_build.mesh_signature_key()` 和 build_state；正常运行用轻量 key 避免每帧完整拓扑 hash。native ABI 层已把 rest/depth/root/baseline/distance/bending/dihedral/volume/edges 等静态数组拆出；`MC2NativeContext.upload_static_arrays()` 只在 topology/config dirty 或 context 未就绪时上传到 C++ vectors，后续 `solve_meshcloth_mc2_context()` 直接复用。 | Python 侧和 C++ 侧的拓扑静态数组缓存都已落地。下一步应下沉参数/曲线固定样本，再评估 dynamic particle/work buffer 是否 native-owned。 |
| 参数/曲线缓存 | MC2 `ClothParameters` 是 unmanaged 参数数组，曲线采样/参数由 manager 压成 job 可读数据。 | `runtime_params.py` 已用 `curve_cache` 缓存 param slot 与采样；`MC2NativeContext` 已缓存 `runtime.param_slots()` 结果和 param dirty key，pack 阶段复用这份 slots；`solve_setup.params.*` 与 `solve_setup.native_context` timing 已能观察参数构建和 context 更新成本。 | 参数层已有 Python 侧 dirty-key/slot cache 地基。后续才是把固定参数/曲线样本推进 C++ native context，避免每帧重复传整包样本。 |
...
3. `native_context` 已有 Python 侧 dirty key/debug metadata、静态/动态 ABI 分层缓存、param slot cache、C++ capsule handle、C++ 拓扑静态数组驻留和 context solve ABI；但参数/曲线样本还没有进入 C++ context，动态 particle/work buffers 仍由 Python numpy 每帧传入。
4. `native_cache` 目前主要承载 debug/native slot，不等同 MC2 的 persistent NativeArrays。
5. C++ full-core 已优先使用 `solve_meshcloth_mc2_context()` 复用 native context 内的拓扑静态数组；旧 `solve_meshcloth_mc2` 数组 ABI 仍作为 fallback。当前仍每帧传入动态粒子、base pose、参数采样、collider、substep inertia numpy 数组，这和 MC2 `DataChunk + ExNativeArray` 全量模式还有距离。
...
5. 第二批应下沉参数固定/曲线样本：damping、distance/bend/angle/motion samples、component enable flags、normalAxis、tether/motion scalar params，并用 param dirty key 控制更新。
...
| Native bridge | buffer 校验、state/params/collider/inertia 打包、old/next collider ABI、full-core 调用、native context 静态数组上传和 context solve fallback | `native_bridge.py`、`_native/src/hotools_native.cpp` | 已完成第二阶段：旧 `solve_meshcloth_mc2` 数组 ABI 保留，优先走 `solve_meshcloth_mc2_context` 复用 C++ 静态拓扑数组。 |
| Native full core | `solve_meshcloth_mc2` 整帧数组 core 与 `solve_meshcloth_mc2_context` context core | `_native/src/mc2.cpp`、`_native/src/hotools_native.cpp` | 已完成当前 MeshCloth full-core 域；核心算法函数仍同一个 `hotools::solve_meshcloth_mc2(view)`，context 入口只改变静态数组来源。 |
...
| P1 已完成第一批 | persistent native context。 | `MC2NativeContext` dirty-key/debug metadata、static/dynamic native ABI 分层、param slots cache、C++ capsule 生命周期已落地；C++ context 已持久化拓扑静态数组，并新增 `solve_meshcloth_mc2_context()`。py311/py313 native 已构建，Blender Python py311 最小 context solve smoke 已通过。后续 P1 继续推进参数/曲线样本驻留，P2 再评估 dynamic particle/work buffers native-owned。 |
