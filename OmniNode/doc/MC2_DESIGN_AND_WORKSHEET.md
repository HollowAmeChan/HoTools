...
| `animationPoseRatio` / `normalAxis` | distance rest lerp、motion/backstop 法线轴和 baseRot 相关。 | `animation_pose_ratio` 固定 0；normalAxis 已接入 motion/backstop，使用 `baseRot * axis`；BasePose 双代理已经提供 animated baseRot 来源。 | 仍缺的是 `animation_pose_ratio` 的 rest/base pose lerp，不是 external base pose rotation。 |
...
| P0 | RuntimeOwner + TeamState/CenterState cache 重构。 | 当前 MC2 每帧以 `_OmniCache(next_state)` 替换 dict；后续改为重建时 `_OmniCache.replace(owner)`，连续帧 `_OmniCache.mutate(owner)`。owner 持有 TeamState/CenterState、曲线采样缓存、固定拓扑数据和未来 native context，统一承载生命周期、dispose、debug snapshot 与轻量 dirty 判断。 |
...
| P1 | 继续收敛 MC2 参数面与上下文语义。 | damping 与 angle attenuation/falloff 已按值+曲线接入，normalAxis 已接入；radius 暂保持 mesh/顶点组语义。后续重点转向 animationPoseRatio、anchor/velocityWeight 等上下文缺口。 |
| P3 | self collision、wind、BoneCloth、job/chunk。 | 等主路径稳定后作为独立阶段推进。 |

### P0：RuntimeOwner 与 TeamState/CenterState 缓存约定

MC2 后续如果继续补 TeamData、CenterData、gravityDot、velocityWeight、native context 和多对象调度，不能继续把所有运行时状态塞进每帧替换的 dict。正式方向是保留 OmniNode per-node evaluation/cache 模式，但把 MC2 cache payload 升级为 RuntimeOwner：
