...
| Motion/max distance/backstop | `Runtime/Cloth/Constraints/MotionConstraint.cs` | `constraints.project_motion_constraint()` | `project_motion_constraints_mc2()` | 高度对齐。max distance、backstop radius/distance、motion stiffness、`use_max_distance/use_backstop`、velocity attenuation 已有；normalAxis 已按 MC2 的 `baseRot * axis` 语义接入 Python 与 C++。 | P0：external animated base pose position/rotation 必须接入，否则骨架驱动角色上的 motion/backstop 只会围绕静态 rest proxy 工作。 |
...
| external animated base pose position/rotation | normalAxis 已接入；外部 animated base pose rotation/position 输入源仍未实现，Motion/backstop 目前使用当前 proxy mesh 构建/对象变换同步得到的 baseRot。角色代理 mesh 被骨架驱动时，这是核心行为缺口。 | P0 |
...
| P0 | external animated base pose 输入源。 | 在 OmniNode 里显式指定 base pose source，优先方案是在 proxy 物体的 MC2/物理属性中配置骨架对象/Armature modifier 或独立 base pose object；solver 每帧读取该 source 的 evaluated mesh 作为 `base_positions/base_normals`，并重建/更新 `base_rotations/step_basic_positions/step_basic_rotations`。严禁默认从输出 shape key 所在同一 evaluated mesh 隐式读取，避免模拟结果反馈进 base pose。 |
...
这个功能的目标不是给 solver 多传一个 rotation 参数，而是让 MC2 能在“骨架/动画驱动后的代理姿态”上叠加物理。当前代理 mesh 由 shape key 写回模拟结果，如果直接读取同一对象的 evaluated mesh，可能把上一帧模拟输出读回 base pose，形成反馈环。因此该功能必须使用显式 base pose source。

建议实现路径：

1. 在 proxy 物体的 HoTools/MC2 物理属性中增加 base pose source 设置：骨架对象、Armature modifier 名称、或独立 base pose object。节点可以先只读取这些属性，避免继续拉长 MC2 节点输入面。
2. 每帧在 solver 前读取 base pose source 的 evaluated mesh，要求顶点数与 proxy solver mesh 一致。若不一致，直接报错并回退到 rest base pose。
3. 用 evaluated 顶点生成 `base_positions/base_normals`，并基于现有 parent/root/depth/local pose 关系更新 `base_rotations/step_basic_positions/step_basic_rotations`。拓扑关系仍来自缓存构建时的 proxy rest mesh，不每帧重建拓扑。
4. 写回仍只写 proxy 的目标 shape key；base pose source 只读，不参与写回。
5. 该数据源会同时服务 motion/backstop、tether 当前基准距离、animationPoseRatio、angle restoration step basic pose。完成后再讨论 gravityDot/velocityWeight 这类 TeamData 语义。
