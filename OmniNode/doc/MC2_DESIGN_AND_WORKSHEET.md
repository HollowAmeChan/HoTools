...
| 运行空间 | solver state 内部以 world-space particle state 递推；节点边界负责 object local/world 转换和输出写回。普通静态场景可保留 shape key 路径；BasePose/骨架叠加物理必须走 GN 后置 delta。 | 已落地 |
...
| --- | --- | --- | --- |
| Manager 入口 | `Runtime/Manager/Cloth/ClothManager.cs`：PreSimulation、Time.FrameUpdate、AlwaysTeamUpdate、WorkBufferUpdate、ClothSimulationSchedule、PostSimulation。 | `physicsMC2/__init__.py`：OmniNode 函数调用、cache 校验、跳帧保护、collider 快照、shape key/GN delta 输出写回。 | 模式不同。MC2 是全局 manager；HoTools 是 per-node evaluation。 |
...
| Motion/max distance/backstop | `Runtime/Cloth/Constraints/MotionConstraint.cs` | `constraints.project_motion_constraint()` | `project_motion_constraints_mc2()` | 高度对齐。max distance、backstop radius/distance、motion stiffness、`use_max_distance/use_backstop`、velocity attenuation 已有；normalAxis 已按 MC2 的 `baseRot * axis` 语义接入 Python 与 C++。 | P0：骨架驱动角色上的 motion/backstop 仍只围绕静态 rest proxy 工作；同对象临时屏蔽 Key 读取 evaluated mesh 的方案已撤回。 |
...
| Blender I/O | shape key/GN delta 写回、local/world 转换、scene dt、BasePose evaluated mesh 帧缓存 | `blender_io.py` | 已完成 |
...
| GN 后置 delta 输出 | MC2 Unity 侧不是这个输出形态；这是 Blender 修改器栈下为了在骨架/基础变形后叠加物理的必要适配。BasePose 场景不再把最终结果写入 shape key。 |
...
### P0：external animated base pose 接入设计

这个功能的目标不是给 solver 多传一个 rotation 参数，而是让 MC2 能在“骨架/动画驱动后的代理姿态”上叠加物理。Blender 里不适合手动反推 Armature/顶点组变形，因为修改器链是黑箱；双对象 source 又会让低模代理工作流变复杂。

最终结论：BasePose/骨架叠加物理不能把最终世界坐标写入 shape key；正式方案必须是“双对象读写分离 + `mc2_delta` + GN 后置位移”。shape key 可以作为早期静态代理输出手段保留参考，但不再是 MC2 的正确输出模型。

#### shape key 写回为什么被否定

1. Blender 的 shape key 在 Armature 等形变修改器之前生效。MC2 求解得到的是骨架/基础动画之后的最终世界坐标，把这个结果写进 shape key 后还会被 Armature 再变形一次，必然产生双重变换。
2. pin 顶点应该忠实跟随原本骨架/基础修改器。最终坐标写入 shape key 会让 pin 顶点同时吃到 BasePose evaluated 运动和当前对象 Armature 运动，表现为 pin 区域漂移。
3. 如果为了规避双重变换而要求当前物体不挂形变修改器，就破坏了“整张低模代理跟随骨架，未 pin 区域叠加布料物理”的真实工作流，高模也无法自然绑定到最终低模代理。
4. 反解 Armature 写回 shape key 不适合作为通用方案。普通线性蒙皮可以近似反解，但 Preserve Volume、B-Bone、多修改器链、非线性修改器都无法可靠通用反解，且维护成本高。
5. 同一对象既读 BasePose 又写 shape key 会产生反馈环：上一帧物理输出会污染下一帧基础姿态。为了读干净 base pose 必须临时屏蔽输出 key 并强刷 depsgraph，这条路性能和稳定性都不可接受。

#### 最终实现采用“双对象读写分离 + 后置 delta”路线：

1. 当前 MC2 物理对象仍可保留骨架和基础修改器，但它不再把模拟最终坐标写入 `output_shape_key`。节点只保证该 shape key 处于 rest 状态，真正输出写入 `mc2_delta` 点域属性。
2. `hotools_mesh_collision.mc2_base_pose_proxy` 引用一个只读 Mesh 对象。这个对象可以挂骨架和必要的基础形变修改器，MC2 每帧只读取它的 evaluated mesh 世界坐标和法线，更新 `base_positions/base_rotations/step_basic_*`。
3. BasePose 只读对象和当前物理对象必须保持相同顶点数量、loop 数量、面数量以及顶点顺序；运行时只做轻量拓扑数量检查，不做重拓扑判断。用户改拓扑后应刷新/替换 BasePose 只读对象，并 reset/清缓存。
4. PhysicsTools 的 Mesh 碰撞属性面板提供 `BasePose只读对象` 引用和创建/刷新按钮。按钮会复制或刷新当前物体和 mesh 数据，保留修改器链，移除复制体上的当前 MC2 输出 shape key、`mc2_delta` 属性与 `MC2 后置位移` 修改器，禁用复制体自身物理开关，然后把复制体回填为当前对象的 BasePose 来源。
5. 如果 MC2 首帧运行时发现 `BasePose只读对象` 为空，会自动调用同一套创建逻辑生成只读对象；已有对象则只做轻量拓扑检查和归档管理。
6. 自动生成/刷新的 BasePose 对象统一放入场景集合 `HoPhysicsCache`，便于用户集中查看、删除和替换物理缓存对象。
7. 这个方案的边界是明确的：BasePose 对象只读，当前物理对象只写后置 delta。后续不能再回到“临时屏蔽输出 Key + 强刷 depsgraph + 读取同对象”的路线。

#### 踩坑与已否决方案

1. 每帧在 solver 前临时把目标 MC2 输出 shape key value 置 0，刷新 view layer，读取同一对象的 evaluated mesh，再恢复 shape key value。
2. 这个方案理论上能避免把上一帧模拟输出反馈进 base pose，但实测不可用：帧率从约 70fps 降到约 20fps，并且点击/选择骨架会因为 Blender 刷新链路触发崩溃。
3. 双对象 + shape key 写回方案解决了“读取源污染”，但没有解决 shape key 在 Armature 前生效的问题。移动骨骼/骨架时 pin 顶点仍会出现双重变换。
4. 当前对象作为纯写入容器、取消形变修改器的方案能避免反解，但破坏低模代理/高模绑定工作流，也无法满足 pin 区域忠实跟随骨架的需求。
5. BasePose 模式下继续读取写入对象 `matrix_world` 来重建 rest/约束或驱动整体 inertia，会把基础动画重复计入物理。移动骨架对象时日志中 `solve_setup` 可飙到 50ms+，原因是进入了 Python 顶点循环；该路径已被禁止。
6. 因此节点侧 `BasePose模式`、`base_pose_eval` timing、同对象 evaluated mesh 读取函数已撤回。以后不能在运行帧里用“临时屏蔽输出 Key + 强刷 depsgraph”的方式实现 animated base pose。

后续重点是测试复杂骨架、修改器链、顶点顺序变化和高模形变工作流；如果每帧读取 evaluated mesh 成为瓶颈，再考虑显式烘焙或 C++ 侧缓存。

#### BasePose 后置 delta 输出

当前工作流已经从“把最终世界坐标写回 shape key”调整为“shape key 保持 rest，运行时写后置位移属性”：

1. `BasePose只读对象` 继续保留骨架和基础修改器，只读 evaluated mesh，提供 `base_positions/base_normals`。
2. 当前 MC2 物理对象也允许保留骨架和基础修改器；它不再把 MC2 最终位置直接写进 shape key，避免 shape key 在 Armature 前生效后被二次变形。
3. 节点首帧/构建时会确保当前物体存在 `mc2_delta` 点域 `FLOAT_VECTOR` 属性，并添加/复用 `MC2 后置位移` Geometry Nodes 修改器。该修改器必须位于 Armature/基础变形之后，读取 `mc2_delta` 并用 `Set Position` 追加偏移。
4. 每帧求解结束后写入的是 `display_positions - base_positions`。写入属性前会把 world-space delta 转成当前物体的局部 offset，因为 Geometry Nodes `Set Position` 的 Offset 输入工作在当前几何空间。
5. pin 顶点的 delta 应接近 0，因此会忠实跟随原本骨架/基础修改器；未 pin 顶点才叠加 MC2 物理偏移。
6. BasePose 复制体会移除 `MC2 后置位移` 修改器和 `mc2_delta` 属性，避免上一次 MC2 输出反馈回只读 base pose。
7. BasePose 模式下，写入对象的对象级 `matrix_world` 不再用于重建 rest/约束，也不再驱动整体 inertia。基础位移已经由 BasePose evaluated mesh 表达；如果再读写入对象矩阵，会导致移动骨架对象时触发 Python 顶点循环和双重变换。
8. 当前物理对象的 shape key 只作为历史兼容/静态输出路径存在。BasePose 模式下它应保持 rest，后续计划把 MC2 旧 shape key 输出和碰撞属性里的 `output_shape_key` 彻底移除。
