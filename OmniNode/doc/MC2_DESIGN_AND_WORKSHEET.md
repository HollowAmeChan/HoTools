...
| 参数曲线/组分开关 | 已接入 “基础值 + 浮点曲线” 形式，覆盖 distance、bend、angle restoration、angle limit、max distance、backstop distance 等当前热路径；已补 `useTether/useDistance/useBend/useAngleRestoration/useAngleLimit/useMaxDistance/useBackstop/useColliderCollision`、tether compression、motion stiffness。damping/radius/angle attenuation/falloff 等仍需补齐。 | 部分完成 |
...
| Tether | `Runtime/Cloth/Constraints/TetherConstraint.cs` | `constraints.project_tether()` | `project_tether_mc2()` | 高度对齐。compression/stretch limit、stiffness width ramp、velocity attenuation、节点级 `use_tether` 与 compression 输入已对齐。 | 不完美点：MC2 每 step 用 `stepBasicPositionBuffer` 算当前基准距离；当前 HoTools 使用构建出的 root rest length，动态 base pose 场景仍需补。C++ full-core 暂不改 ABI，关闭 tether 时由 Python wrapper 传入不会触发的阈值，后续可在 native view 内补显式早退。 |
...
| Motion/max distance/backstop | `Runtime/Cloth/Constraints/MotionConstraint.cs` | `constraints.project_motion_constraint()` | `project_motion_constraints_mc2()` | 部分到高度对齐。max distance、backstop radius/distance、motion stiffness、`use_max_distance/use_backstop`、velocity attenuation 已有。 | 不完美点：MC2 依赖 baseRot 和 normalAxis；当前主要用 base normal，外部 base pose rotation/axis 还未完整接入。 |
...
| --- | --- | --- | --- |
| `damping` | `CurveSerializeData`，转成 `dampingCurveData * 0.2`，在 post 阶段按 depth 采样阻尼。 | 节点只有标量 `damping`，当前按帧/子步转换成统一 damping，没有 depth 曲线。 | 需要改成值 + 曲线，并按 MC2 的 20% 系数和 depth 采样进入 post。 |
| `radius` | `CurveSerializeData`，碰撞与 motion/backstop 按 depth 采样粒子半径。 | 当前 `collision_radius` / `hotools_mesh_collision.radius` 是构建期/顶点组半径，非 MC2 `radiusCurveData` 的运行期曲线语义。 | 需要新增 radius 值 + 曲线，并把 `collision_radii` 改成运行期曲线采样或缓存采样。 |
...
| `angleRestorationConstraint` | `useAngleRestoration`、stiffness 曲线、velocityAttenuation、gravityFalloff。 | `use_angle_restoration` 与 stiffness 曲线已接；关闭时 Python fallback 跳过 angle 投影，C++ full-core 走零数组空操作；velocityAttenuation/gravityFalloff 仍用常量。 | 仍需补 velocityAttenuation、gravityFalloff 输入。 |
...
| `motionConstraint` | `useMaxDistance`、maxDistance 曲线、`useBackstop`、backstopRadius 标量、backstopDistance 曲线、stiffness 标量。 | `use_max_distance/use_backstop`、maxDistance/backstopDistance 曲线、backstopRadius 标量、motion stiffness 输入已接；两者都关闭时跳过/零数组。 | 基本接入；仍需补 baseRot/normalAxis 语义。 |
| `tetherConstraint.distanceCompression` | MeshCloth 可配置标量 compression；stretch 为系统常量。 | `use_tether` 与 `tether_compression` 已暴露；stretch 仍保持系统常量。Python fallback 关闭时直接跳过；C++ full-core 暂以不会触发的阈值实现关闭。 | 基本接入；后续可补 native 显式早退以降低关闭态循环开销。 |
...
| `animationPoseRatio` / `normalAxis` | distance rest lerp、motion/backstop 法线轴和 baseRot 相关。 | `animation_pose_ratio` 固定 0；motion 主要用 base normal。 | external base pose rotation/normal axis 任务仍需继续。 |
...
| Cache/schema | world-space runtime state、schema guard、轻量缓存判定、jump/reset 保护、collider previous snapshot | `state.py`、`__init__.py`、`collision.py` | 已完成；当前 schema version 13。运行期为了避免每帧扫描拓扑，缓存命中只比较对象/mesh 标识和顶点/loop/面数量；同数量拓扑变化不会自动失效，必须通过 reset/清缓存重建。 |
...
| 参数曲线/组分开关 | distance/bend/angle restoration/angle limit/max distance/backstop distance 的值 + 曲线输入；tether/distance/bend/angle/motion/collision 显式开关；tether compression 与 motion stiffness 输入 | `PropertyCurve/*`、`physicsMC2/params.py`、`physicsMC2/solver.py`、`physicsMC2/__init__.py` | 已接入当前热路径；仍需补 damping/radius/angle attenuation/falloff 等 MC2 参数面 |
...
| damping 曲线 | MC2 `damping` 是值 + 曲线并乘 0.2 后按 depth 采样；当前 HoTools 仍是统一标量 damping。 | P1 |
| radius 曲线 | MC2 `radius` 是运行期值 + 曲线，collision/motion 按 depth 采样；当前 HoTools 半径来自 `collision_radius` 或顶点组。 | P1 |
| angle attenuation/falloff | MC2 有 restoration velocityAttenuation、gravityFalloff；当前 HoTools 的 useAngleRestoration/useAngleLimit 已补，但 attenuation/falloff 仍是常量。 | P1 |
...
| MC2 组分开关 | distance/bend/angle/motion/collision 关闭时减少曲线采样、投影调用或 collider 快照扫描；tether 在 Python fallback 中直接跳过，在当前 C++ full-core ABI 下通过不会触发的阈值保持空操作，后续可升级 native 早退。 |
...
| P1 | 补 negative scale、external base pose rotation。 | 提高 MC2 MeshCloth 行为等价度。 |
| P1 | 补齐 MC2 剩余曲线参数面。 | damping/radius/angle attenuation/falloff 等参数补齐后，当前 MeshCloth 参数面才可接近完整对齐。 |
