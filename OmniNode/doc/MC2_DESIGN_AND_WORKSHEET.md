...
| Collider point collision | `Runtime/Cloth/Constraints/ColliderCollisionConstraint.cs`、`Runtime/Manager/Simulation/ColliderManager.cs` | `collision.py` | `project_collisions_mc2()` | 部分到高度对齐。sphere/capsule point collision、group filter、collision normal/friction、post friction、moving collider old/next pose 推出平面已实现。 | 不完美点：plane、edge collision、BoneSpring soft collider 未实现；HoTools collider 来源仍是 Blender scene snapshot，而不是 MC2 全局 ColliderManager。 |
...
| 碰撞 | HoTools sphere/capsule point collision、group filter、friction/normal、moving collider old/next pose、native collider arrays | `collision.py`、`native_bridge.py`、`_native/src/mc2.cpp` | 已完成当前 point collision 域 |
