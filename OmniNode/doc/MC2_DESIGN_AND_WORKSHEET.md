# OmniNode 旧 MC2 迁移审计与公式参考

更新日期：2026-07-11

文档状态：**历史参考，不是当前执行计划，也不是新 MC2 的实现状态文档**。

当前 MC2 实施顺序与源码 worksheet 见 `MC2_SOURCE_ALIGNMENT_EXECUTION_PLAN.md`；统一物理世界当前状态见 `PHYSICS_WORLD_IMPLEMENTATION_STATUS.md`；长期架构边界见 `PHYSICS_SIMULATION_PIPELINE_CONTRACT.md`。

本文保留旧 `physicsMC2MeshCloth` 的迁移审计、公式对拍、自碰撞研究、性能记录和踩坑。文中的“当前”“下一步”和完成度均以旧 MeshCloth 路径的历史快照为语境，不能据此冻结 `physicsWorld.mc2` 的 spec、slot 或 native ABI。
...
旧 Python/C++ MeshCloth 只保留为公式与 parity 参考，不再作为新 solver 的节点中控、cache、写回架构或实施顺序蓝本。

## 已废止的新 MC2 B1-B4 工作草案

本节是 2026-07-10 源码复审前形成的草案，**不具备契约效力**。其中 B2/B3 只能视为已提交的 framework scaffold；B4 把 SelectionData、VertexAttribute、baseline/depth 和 constraint data 过早合并，且 AutomaticMesh、distance、bending 规则与 MC2 源码存在实质偏差。真实审查结论和替代阶段见 `MC2_SOURCE_ALIGNMENT_EXECUTION_PLAN.md`。
...
5. B3 初始状态本身不猜 MeshCloth fixed/depth；它们由下一层 selection capability 显式提供，再写入 initial state 和 particle buffer。

### B4 Selection/Constraint Topology 契约

1. `MC2SelectionSpec` 对齐 Unity `VertexAttribute`：Fixed=`0x01`、Move=`0x02`，并持有 baseline parent、depth、ready/blocker 与独立 selection signature。
2. Mesh selection 的优先级是 task source `fixed_mask/fixed_indices`、兼容 RNA `Object.hotools_mesh_collision.pin_*`、setup 默认。显式 source payload 是未来 selection/paint 节点的稳定接口；solver core 不读取顶点组。
3. 未配置 Mesh Pin 时所有粒子保持 Move，但 `backend_ready=false`，不会隐式选择固定点。BoneCloth/BoneSpring 自动把每条 root 标为 Fixed，其余标为 Move。
4. `MC2ConstraintTopologySpec` 第一批冻结 distance pairs/rest lengths/kinds、triangles、triangle-adjacency bending pairs 和 baseline parents。MeshCloth 直接使用代理网格边/面；骨骼 setup 使用父子 structural links。
5. BoneCloth `AutomaticMesh` 按 root 世界位置做最近邻排序并按 Unity 1.5 倍阈值决定闭环；`SequentialLoopMesh` 按 task source 发布顺序连接同层粒子并生成跨链三角形。BoneSpring setup options 仍强制 Line。
6. selection signature 变化执行 slot dispose + rebuild；profile 和 step settings 变化继续复用 selection、constraint topology 与 particle buffer。B4 仍只构建静态图，不执行约束。
...
本节只保留旧路径研究方向，不代表当前执行顺序。当前顺序以 `MC2_SOURCE_ALIGNMENT_EXECUTION_PLAN.md` 为准。
