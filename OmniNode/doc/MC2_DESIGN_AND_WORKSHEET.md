...
| Collider point collision | `Runtime/Cloth/Constraints/ColliderCollisionConstraint.cs`、`Runtime/Manager/Simulation/ColliderManager.cs` | `collision.py` | `project_collisions_mc2()` | 部分到高度对齐。sphere/capsule/plane point collision、group filter、collision normal/friction、post friction、moving collider old/next pose 推出平面已实现。Plane 按 MC2 `PointPlaneColliderDetction()` 的无限半空间语义处理：`cpos + normal * particleRadius` 为推出平面。当前 `hotools_mesh_collision` 只作为本 cloth 顶点 `collision_radii` 的来源，不会把其他 mesh 顶点展开成外部 sphere collider。 | 不完美点：edge collision、BoneSpring soft collider 未实现；Box collider 是 HoTools 额外 OBB point collision，MC2 源码只保留 Box enum/size 数据但当前 point solver 无 Box 分支；HoTools collider 来源仍是 Blender scene snapshot，而不是 MC2 全局 ColliderManager。 |
...
| Cache/schema | world-space runtime state、schema guard、jump/reset 保护、collider previous snapshot | `state.py`、`__init__.py`、`collision.py` | 已完成；当前 schema version 12 |
...
| 碰撞 | HoTools sphere/capsule/plane point collision、Box OBB 额外碰撞、group filter、friction/normal、moving collider old/next pose、native collider arrays | `collision.py`、`native_bridge.py`、`_native/src/mc2.cpp` | 已完成当前 point collision 域；Plane 已按 MC2 无限半空间平面对齐 |
...
| 优先级 | 工作 | 目标 |
| --- | --- | --- |
| P0 已完成 | Blender 场景级 `meshClothMC2` vs `meshClothMC2Cpp` parity harness。 | `_native/tests/test_mc2_blender_scene_parity.py` 已落地，当前 12 帧 max/RMS/stretch/collision count delta 为 0。 |
...
| P1 | 补 edge collision、negative scale、external base pose rotation。 | 提高 MC2 MeshCloth 行为等价度。 |
