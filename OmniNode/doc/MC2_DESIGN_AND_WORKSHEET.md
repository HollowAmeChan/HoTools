...
| Self collision broadphase / job 化 | self collision 已并入 `solve_meshcloth_mc2()` full-core/context ABI；`project_self_collisions_mc2()` 已从 `mc2.cpp` 拆到 `_native/src/mc2_self_collision.cpp`；当前 native contact 构建仍是首版 O(V*T + E^2)，未做 MC2 grid broadphase/job 化。 | P1 |
...
2. P1：优化 self collision native 实现。下一步应在 `_native/src/mc2_self_collision.cpp` 补 MC2 风格 grid broadphase/job 化，避免当前 O(V*T + E^2) contact 构建在大网格上成为新瓶颈。
