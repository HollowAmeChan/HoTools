...
Normal path 的源码顺序也给了后续落点：先 proxy/base pose，再 center/inertia/wind，再每个 substep 做 TeamStepUpdate，随后 particle preupdate、baseline pose、tether、distance、angle、bending、collision、distance、motion、post，最后 display/proxy post。HoTools 现在核心约束顺序接近，但 TeamStepUpdate 的 `gravityDot/gravityRatio`、`animationPoseRatio`、`velocityWeight/blendWeight` 仍是主要上下文缺口。
...
| Post velocity/friction | `SimulationManagerNormal.cs` post step | `constraints.apply_post_step()` | `apply_post_step_mc2()`/`solve_meshcloth_mc2()` 内 post | 高度对齐。velocity、real_velocity、old_positions、friction、static_friction、particle speed limit 已有。 | 不完美点：MC2 reset 后用 `TeamData.velocityWeight` 平滑恢复速度，并参与最终 `blendWeight`；culling/skip/timeScale 相关分支也未完整复制。 |
| Inertia/teleport/centrifugal | `Runtime/Cloth/Constraints/InertiaConstraint.cs`、`Runtime/Manager/Team/TeamManager.cs`、`SimulationManagerNormal.cs` | `inertia.py` | `apply_substep_inertia_mc2()`、`apply_centrifugal_velocity_mc2()` | 部分到高度对齐。world/local/depth inertia、movement smoothing、world/local speed limit、teleport reset/keep、negative scale teleport、particle speed limit、centrifugal 已有。 | 未完整实现 anchor inertia、sync team、velocityWeight、culling/skip/timeScale。frame/object runtime state 暂留 Python。 |
...
| `gravityFalloff` | `TeamManager.SimulationStepTeamUpdate()` 由 `CenterData.initLocalGravityDirection` 和当前 center rotation 算 `gravityDot`，再得到 `gravityRatio`；Angle restoration 也直接消费 `TeamData.gravityDot`。 | 未接 TeamData gravityDot，仅有普通 gravity direction/power；angle restoration gravityFalloff 已退回标量输入，但缺少 TeamStepUpdate 的方向上下文。 | 需要先把 CenterData/TeamData 字段收进 RuntimeOwner，再统一驱动 gravityRatio 与 angle gravity falloff。 |
| `stablizationTimeAfterReset` / `blendWeight` | reset 后 `velocityWeight += simulationDeltaTime / stablizationTimeAfterReset`，并与参数 `blendWeight`、`distanceWeight` 合成 `TeamData.blendWeight`；post velocity 与 display blend 都会用到。 | 未实现。 | 明确缺口。应在 TeamStepUpdate 语义落地后再接，不应散落在 post/display 局部函数里。 |
| `animationPoseRatio` / `normalAxis` | `SimulationStepUpdateBaseLinePose()` 用 `animationPoseRatio` 混合初始 pose 与 animated base pose；`DistanceConstraint` 用它在初始 rest length 和 animated base pose 距离间 lerp；`MotionConstraint` 注释明确 max distance/backstop 始终使用 animated base pose，不受 animationPoseRatio 影响。 | `animation_pose_ratio` 固定 0；normalAxis 已接入 motion/backstop，使用 `baseRot * axis`；BasePose 双代理已经提供 animated baseRot 来源。 | 仍缺的是 baseline 与 distance 的 `animation_pose_ratio` 接线，不是 external base pose rotation。 |
...
| velocityWeight/stabilization | MC2 reset 后速度权重稳定化未完整复制；该值属于 TeamStepUpdate，不应在 post/display 中孤立实现。 | P2 |
...
| P1 | 继续收敛 MC2 参数面与上下文语义。 | damping 与 angle attenuation/falloff 已按值+曲线接入，normalAxis 已接入；radius 暂保持 mesh/顶点组语义。后续重点转向 animationPoseRatio、TeamStepUpdate gravityDot/gravityRatio、anchor/velocityWeight 等上下文缺口。 |
