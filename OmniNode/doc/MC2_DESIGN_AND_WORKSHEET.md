...
| external animated base pose position/rotation | 同对象 evaluated mesh + 临时屏蔽 MC2 输出 Key 已实测不可行并从节点撤回。原因：每帧强制 depsgraph 刷新成本过高，且和骨架选择/刷新机制存在崩溃风险。 | P0 待重新设计 |
...
| P0 待重新设计 | external animated base pose。 | 已否决并撤回同对象 evaluated mesh + 临时屏蔽输出 Key 方案。后续只能考虑不会在运行帧里反复刷新 depsgraph 的路线，例如显式离线/缓存 base pose、改变写回策略、或更明确的代理工作流。 |
...
1. 优先寻找不需要每帧强制 depsgraph 刷新的 base pose 缓存路线。
2. 如果必须读 Blender evaluated mesh，应考虑离线烘焙/显式缓存，而不是在 solver 运行帧里即时读取。
3. 如果要保持实时骨架叠加物理，可能需要改变写回策略或代理工作流，而不是继续复用同一对象同一修改器链。
