...
physicsWorld/
  ...
  mesh_cloth/
    __init__.py                 # MeshCloth Blender 数据/capability owner
    capabilities.py
  ...
  mc2/
    ...
    adapters/                   # 后续三种 setup 的拓扑输入与写回适配
...
collision + rigid + mesh_cloth data owner + spring_vrm + mc2
  <- physics_ui
```
...
- `mesh_cloth` 从占位 solver descriptor 收口为 Blender data adapter component；solver registry 不再暴露第二个 MeshCloth solver id。
...
- 当前属性 domain 拓扑为 `collision -> mesh_cloth adapter -> rigid -> physics_ui`，五个稳定 RNA binding 未变化
