...
九条已固定架构支柱（框架+模块化 solver、显式身份卡、通用世界状态输入、通用写回、隐式表达、纯 C++ 后端、旧 solver 移除、跨 solver 交互、物理属性动态注册）见架构设计文档。当前落地态：spring_vrm 已完成原子化、旧路径删除与 solver-owned RNA 注册闭环；rigid/Jolt 已完成 names/capabilities/declaration/debug 下沉，并通过 registry scope hook 接入 Begin 阶段，MeshCloth/BoneCloth 仍待迁移。

### 2026-07-10 SpringBone 最终收口

- `Physics.py` 中旧 VRM/Base 节点、Python solver、旧 cache/scene scan/inline PoseBone writeback 已删除。
- Native 只保留 SpringBone context API；35 参数 binding、公开旧 View 和 C++ wrapper 已删除。
- `spring_vrm/capabilities.py` 是 bone collision 字段、默认值、RNA 元数据和更新策略的事实源。
- `spring_vrm/properties.py` 生成显式 `PG_Hotools_BoneCollision`；`physicsWorld.registry` 统一注册/注销 class 与 `Bone.hotools_collision` binding。
- `PhysicsTools` 只保留 UI、操作器和预览，不再定义或注册 SpringBone 字段。
- 验证：Blender 35/35，py311/py313 native context-only 各 10/10，solver RNA 生命周期 1/1，双 ABI Release 重建通过。
...
- 为 `.blend` 数据兼容保留 `Bone.hotools_collision` 存储名，但 `PG_Hotools_BoneCollision` 的定义位于 `spring_vrm/properties.py`。

其它尚未原子化的属性仍按各自 domain 迁移，不应把 SpringBone 已完成的所有权重新退回 `PhysicsTools/physicsProperty.py`。
...
- `collision_snapshot_cpp(scene)` 改为由 SpringBone native 包装层从 `world.collider_snapshot` 打包；当前已接入 `SPHERE` / `CAPSULE` / `PLANE` / `BOX`，并沿用 MC2/Jolt 的公共 collider type code。`spring_vrm/native.py` 已在 `SpringVRMNativeContext` 内按当前 collider snapshot、armature 和 chain bones 做 lazy collider array cache，同一帧同一链不会重复重打包。
...
**骨骼碰撞字段（含 pin）**：solver 不直接散读 `Bone.hotools_collision`。`spring_vrm/bone_collision.py` 的统一 resolver 按 `bone_collision.override` 隐式对象 > solver 显式 RNA > capability 默认值解析。显式 RNA 已由 capability 生成并由 physicsWorld registry 注册，支柱 9 对 SpringBone 已闭环。
...
### Phase 0：命名、目录与属性装载边界 ✅ SpringBone 完成，其他 solver 继续推进

上面的目录清单是**目标结构**，当前尚未完全落地。实际进度是“spring_vrm 领先、rigid/Jolt 正在原子化收口”：

- `physicsWorld/registry.py` 负责 solver 描述符、declaration/node/capability/debug/scope hook 汇总，并新增 `blender_properties` 声明解析与统一 register/unregister。`sources.py` 仍缺，source 解析目前仍并入 `world.py` / `physicsWorldBegin`。
- `spring_vrm/` 已达到目标原子化结构：除 names/capabilities/declaration/implicit/spec/solver/debug 外，新增 `properties.py` 持有由 capability 生成的 Blender RNA 声明。
...
1. 已完成：`OmniNodeRegister.py` 不再直接导入 `physicsWorld.rigid.nodes` / `spring_vrm.nodes`，而是通过 `physicsWorld/registry.py` 从 solver module descriptor 装载节点模块。
2. SpringBone 已完成：`SOLVER_MODULE` 的 `blender_properties` 指向 solver 自有 RNA 声明；通用 registry 执行注册/注销。Rigid/Jolt 等 domain 仍需按同一协议迁移自己的属性。
3. 已完成基础版：`registry.py` 对 solver id、slot kind、result channel、implicit object tag、debug draw mode id 做冲突检查；exchange channel 显式索引仍待从 declaration/capability 抽出。
4. 已完成主体：中央 `names.py` / `declarations.py` 只保留兼容惰性导出，不再作为任何 solver 自有语义的权威事实源。
...
因此 Phase 0 对 SpringBone 可以记作“solver 子模块装载、属性注册和旧路径删除全部闭合”。全局剩余项是其他 domain 的属性迁移、`sources.py` 拆分、exchange channel 显式索引和 MC2/BoneCloth 迁移。
...
1. **SpringBone VRM 蓝本已最终收口。** `SpringVRMNativeContext` 强制使用 context symbols，static arrays 常驻、dynamic arrays 每帧同步、step 后 readback；35 参数 ABI 与旧 Python/CPP/Base 节点均已删除。批次 result/writeback、records 复用、32 collider 基准、bone collider capability/RNA 与注册生命周期均已完成。
...
旧节点不作为统一物理世界的兼容目标。SpringBone 已执行完删除审查并移除全部旧入口；后续 solver 应复用同一门槛，覆盖 direct bpy write、旧 cache owner、scene-wide scan、native lifecycle 和 writeback side effects。
...
统一物理世界不为旧节点接法保留兼容层。SpringBone 旧节点已删除；其它未迁移 solver 的临时旧节点不能让 result stream、exchange 和 writeback contract 增加双路径。
...
第一版不要做复杂 participant schema。SpringBone 已证明可保留稳定 Blender 存储名，同时把字段定义、RNA 生成和注册权收回 solver capability/physicsWorld registry；其它 solver 应沿用这一模式。
...
Phase 5 的声明、runtime cache、帧语义与 PoseBone 写回测试已关闭。SpringBone context-only C++ handle、旧 ABI/节点删除、solver-owned RNA 和 registry 生命周期均有 native / Blender 测试覆盖；Jolt constraint state/lambda、breakable policy 与独立 A/B anchor frame 已接入。剩余重点转向 MC2/BoneCloth 的 Mesh/PoseBone 写回样板及后续跨 solver 交互。
...
物理属性注册已由 SpringBone 首次进入这个装载流程。外部 `PhysicsTools` 面板和操作器继续存在，但 `PG_Hotools_BoneCollision` 的字段事实源和 class 定义已提升到 `physicsWorld.collision` 共享 component；SpringBone 与面板都只是消费者。

## 2026-07-10 追加：外部物理属性所有权迁移

`PhysicsTools` 的属性、注册生命周期、UI/preview 和 MeshCloth I/O 将整体迁入 Physics World。详细的领域归属、稳定 RNA 契约、component registry、根插件加载顺序、分阶段文件迁移与验收矩阵见：

- `PHYSICS_WORLD_EXTERNAL_PROPERTY_OWNERSHIP_PLAN.md`

核心修正是：`Bone.hotools_collision` 与 `Object.hotools_object_collision` 属于 Physics World 共享 collision capability，不属于 SpringBone 或面板；RigidBody/RigidConstraint 继续由 rigid solver 持有，MeshCollision 由 mesh_cloth domain 持有。Blender ID 保存原始持久值，Physics World 模块持有 schema 和注册权，`PhysicsWorldCache`/solver slot 只保存归一化 profile/spec，不把 live PropertyGroup 当作跨帧事实源。
