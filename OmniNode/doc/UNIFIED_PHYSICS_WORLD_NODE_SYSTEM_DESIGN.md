...
**骨骼碰撞字段（含 pin）**：新路径不再由 solver 直读 `Bone.hotools_collision`。`spring_vrm/bone_collision.py` 的 `resolve_bone_collision_fields` / `resolve_bone_pin` 是统一 resolver，解析优先级 `bone_collision.override` 隐式对象 > 旧 `Bone.hotools_collision` > `BONE_COLLISION_CAPABILITY` 默认值。override 属性/注册节点已落地，native 消费端只认 resolver，不再各写一份 `getattr`。这样等属性注册最终迁到物理世界侧（支柱 9）时，唯一真值来源始终收敛在 capability + resolver 上。
...
2. 已完成主体：`SOLVER_MODULE` 描述符覆盖 `nodes`、`capabilities`、`debug_draw_modes`、`declaration`、scope hooks；`property_registration` 仍待接入。
...
因此 Phase 0 可以记作“solver 子模块装载骨架已闭合、spring_vrm 与 rigid/Jolt 主体已原子化”。剩余不是目录结构问题，而是 `sources.py` 拆分、property registration 接入、exchange channel 显式索引和后续 MC2/BoneCloth 迁移。
