...
实际执行必须沿同一个 `PhysicsWorldCache` owner 线性传递。不能把 world 分叉写入后再合并。

运行时内部链路：

```text
world.implicit_objects["spring_vrm.chain"]
...
| `VRM骨链对象注册` | 写入 `spring_vrm.chain` 隐式对象 | 不直接调用 solver |
...
| `SpringBone VRM模拟步` | 收集声明、维护 slot/context、推进 native、发布结果 | 不扫描整个 scene，不直接写 PoseBone |
...
| 文件 | 所有权 |
|---|---|
| `spring_vrm/names.py` | solver id、slot kind、tag、result/debug 名称 |
...
| `spring_vrm/implicit_objects.py` | 骨链和碰撞覆写注册、stable id、signature |
...
`spring_vrm.chain` 的 stable id 由 armature pointer、armature data pointer、root 和 bones hash 组成。Signature 还覆盖 enabled、刚度、阻尼、重力方向和强度。

相同 `tag + stable_id` 后写覆盖前写。Signature 不变时 version 不递增；变更通过 spec/slot dirty 路径生效。
...
| 刚度、阻尼、重力 | implicit object dirty，下一次 step 刷新 |
...
- world generation 替换；
- Cache Delete、runtime cache clear、成功重编译；
- addon 注销。
...
| Blender 4.5 集成 | `36/36` | slot、same-frame、暂停/跳帧、碰撞覆写、预设、debug、writeback、cache dispose |
...
1. 新输入属于 implicit object、world/frame input、slot 私有状态还是 exchange，所有权是否唯一。
...
7. stale slot、Cache Delete、重编译和 addon 注销是否释放新增资源。
