...
本文定义 Physics World 通用 Bake 节点、关键帧记录、Mesh 外部缓存、跳帧清理、播放代理和文件生命周期的产品与实现合同。它服务 MC2、Rigid/Jolt、SpringBone 和未来 solver，不把 Bake 做成 MC2 私有功能。
...
4. Mesh 第一版使用每对象一个 `.pc2` 文件，使用标准库直接流式写入，不引入第三方库。PC2 是可恢复、可逐帧覆盖的工作缓存，不是最终交换格式。
5. 完成工作缓存后，可由独立 `Finalize Physics Cache` 操作调用 Blender 内置 Alembic exporter，生成一个多对象 `.abc` 交付缓存。Alembic 不作为逐帧 Bake Node 的直接 writer。
6. MDD 不作为第一版 writer；USD/USDC 保留为实验后端，不作为首发主路径。
7. Blender `PointCache/ptcache` 没有把任意插件顶点 buffer 写入磁盘缓存的公开 API，不能直接承载 MC2。Geometry Nodes Bake 可调用，但必须先通过完整的 operator-driven 业务验证，当前不进入生产主路径。
...
| `gn_attribute` / `mesh_vertex_offset` | `hotools_physics_offset` + GN Set Position | PC2 absolute local positions |
...
| `cache_directory` | 缓存文件夹 | 必填 | manifest、PC2 和最终 ABC 的根目录；支持 `//` |
...
| `bake_mesh` | Bake Mesh | `False` | 记录 Mesh final positions 到 PC2 |
...
| `mesh_cache_policy` | `KEEP / INVALIDATE_FROM_CLEAR_FRAME / DELETE_SESSION` | `KEEP` | PC2 默认保留，用户明确选择后才截断/删除 |
...
- animation、PC2、finalized cache 三类留存权彼此独立，不能因为用户清了一类就顺手清其他类。
...
- Clear Physics Bake 只在指定帧按用户选择处理该 session 拥有的动画、PC2、finalized cache 和 live output。
...
Writeback 的生命周期清零必须发布 `recordable=False, writeback_reason="lifecycle_clear"`。Bake 默认不把这种清零动作 K 成动画；Clear 节点可以消费它解析精确 live target。只有 solver 的正式可记录结果或 boundary baseline 才能进入 Action/PC2。
...
|---|---|---|---|---|---|---|---|
| PC2 | Mesh Cache modifier | 很适合 | 固定偏移，可直接覆盖 | 否 | 无压缩，约 `12*N*F` bytes | 无 | 第一版工作缓存 |
...
| GN Bake | NodesModifier | operator 控制整段求值 | 私有 bake 数据管理 | 每 modifier | Blender 内部格式 | Blender operator/context | 研究项 |
| PointCache | 对应内置 simulator | 无通用 writer | 无 | 否 | 内置压缩 | 私有 simulator integration | 不可用 |

### 为什么第一版选 PC2

MC2 MeshCloth 已保证顶点数量和顺序稳定，正好满足 PC2 的核心前提。PC2 writer 可以：
...
因此 PC2 是工作缓存，不应伪装成理想归档。需要交付或长期保存时再转 Alembic。
...
Finalize 应是显式 operator，而不是 Bake Node 在最后一帧隐式启动长任务。operator 只导出本 manifest 的 playback targets，完成后校验 archive 路径和帧范围，不改变 PC2 工作缓存，除非用户另行选择清理。
...
mesh_cache_policy:
  KEEP                    文件完全保留，只更新 manifest 状态
  INVALIDATE_FROM_CLEAR_FRAME
                          保留当前帧之前的连续 PC2 前缀，从当前帧起失效/截断
  DELETE_SESSION          删除本 session 拥有的 PC2 文件
...
delete all .pc2/.abc in directory
...
对 source/write object 确保一个固定名字的 Mesh Cache modifier：
...
- modifier 类型必须是 `MESH_CACHE`；
- `cache_format = 'PC2'`；
- filepath 指向 manifest 当前 target file；
- play/time/deform mode 与 header 对齐；
- modifier 在 topology-preserving base modifier 之后；
- live bake 时 `show_viewport/show_render=False`；
- finalized playback 时启用 Mesh Cache，禁用物理 GN output，并把 GN offset 清零。

live simulation 和 cache playback 必须互斥，防止：

```text
Base animation -> PC2 final positions -> GN physics offset again
```

切回 live 时恢复 manifest 记录的修改器启用状态，不删除用户自己的其他 Mesh Cache modifier。所有自动 modifier 都带 target/session tag，并只按 tag 清理。
...
但它仍不是任意 buffer writer。需要真实 Geometry Nodes Bake/Simulation node，并由 operator 接管帧范围和 depsgraph 求值。要让它记录 MC2，必须证明 operator 推进时间轴时 OmniNode 以正确顺序运行，并且不会把上一帧 GN writeback 反馈成下一帧 BasePose。

因此只建立研究 spike：

1. 构造最小 GN Bake node，输入共享物理 offset 后 Set Position。
2. background 调用 `geometry_node_bake_single`。
3. 验证 OmniNode 每帧执行、frame context、跳帧、取消和恢复。
4. 比较磁盘体积、写入时间、读取时间和版本兼容。

没有通过这些门槛前，不把 GN Bake 作为优化捷径。
...
- 项目已有 numpy；
- Blender RNA 和 `MESH_CACHE` modifier。
...
- PC2 每 target 每帧最多生成一个连续 `float32[N,3]` sample。
...
physicsWorld/
  bake/
    ...
    pc2.py                # PC2 header/read/write/overwrite/truncate
    playback.py           # Mesh Cache modifier与live/baked切换
...
### Phase 2：MC2 Mesh PC2

1. 实现 PC2 writer/reader oracle。
2. 实现 MeshBakeSample provider registry。
3. MC2 BasePose + final offset provider。
4. 多 Mesh manifest、事务和路径回填。
5. ensure Mesh Cache modifier 与 live/playback 互斥。

完成门槛：Blender 实际用 Mesh Cache modifier 回放的每帧顶点与 live MC2 result 在 object local 空间逐点一致；向后跳、当前帧覆盖、拓扑变化和崩溃恢复通过。

### Phase 3：Alembic Finalize

1. 显式 operator，从 manifest 选择 playback targets。
2. 逐 target 启用 PC2 playback，禁用 live GN offset。
3. 调 Blender Alembic exporter 生成一个 archive。
4. 新场景/后台重新导入 ABC，逐帧对拍 Mesh positions。
5. 保留 PC2，manifest 记录 ABC freshness。
...
- Geometry Nodes Bake operator-driven spike。
- 只有实测优于 PC2 工作缓存或显著简化交付时才产品化。
...
- PC2 10k/100k/500k 顶点的 provider/write/flush 分离统计。
...
- 清理是独立、可 mute 的节点；动画、PC2、finalized cache 留存策略由用户分别选择。
...
- Geometry Nodes Bake 是否能可靠驱动 OmniNode 全时间轴执行。
...
Playback
  base modifier stack
    -> HoTools PC2 Mesh Cache
    -> live GN physics output disabled/zeroed
...
这条路径把“可逐帧写、可恢复”和“紧凑、多对象、可交换”拆成两个阶段。PC2 负责稳定生产过程，Alembic 负责最终交付；Writeback receipt 和 manifest 保证 Bake 永远只作用于真实物理 target。是否保留 Action、PC2 和最终 ABC 则始终由用户在 Clear 节点分别决定。
