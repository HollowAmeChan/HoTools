...
第一阶段目标是把 `网格物理-XPBD` / `meshPhysicsXPBD` 节点里最耗时的 XPBD solve 阶段移动到 C++，Python 侧继续负责 Blender 数据读取、cache 管理、shape key 写回和 OmniNode 接口。
...
5. 再迁移 `meshPhysicsXPBD` 的 XPBD solve 核心，并新增 `网格物理-XPBD-CPP` 并行节点。
