...
插件根目录的 `version_info.json` 是独立于 Blender `bl_info` 的版本元数据，发布 ZIP 会在 GitHub Actions 中自动写入时间戳版本、发布标签和提交号。安装插件后，在 Blender 的插件偏好设置中可以查看当前版本，点击“检查 HoTools 更新”获取 GitHub 最新发布版；确认后会自动下载当前 Blender Python ABI 对应的 ZIP，关闭 Blender 后替换插件并重新启动。重启前请先保存未保存的文件。
