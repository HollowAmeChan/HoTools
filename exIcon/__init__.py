...
class ExIcon():
    def __init__(self):
        ...
        self.texture = "exIcon.png"  # 图片文件名
        self.v_shader = "image.vert"  # 顶点shader文件名
        self.f_shader = "image.frag"  # 片段shader文件名
...
class OP_drawExIcon(Operator):
    ...
    def makeDrawFunction(self, shader, gpuTex, batch):
        def draw():
            ...
            shader.uniform_float(
                "iconSize", bpy.context.preferences.addons["HoTools"].preferences.hoTools_ExIconSize)
            shader.uniform_float(
                "iconAlpha", bpy.context.preferences.addons["HoTools"].preferences.hoTools_ExiconAlpha)
            batch.draw(shader)
        return draw
    ...
    def execute(self, context):
        ...
        shader_info.push_constant('FLOAT', "iconSize")
        shader_info.push_constant('FLOAT', "iconAlpha")
        shader_info.fragment_out(0, 'VEC4', "FragColor")  # 颜色输出，必要

        # 编译着色器
        shader_info.vertex_source(
            exIcon.readFromFile(exIcon.v_shader))  # 顶点着色器
        shader_info.fragment_source(
            exIcon.readFromFile(exIcon.f_shader))  # 片段着色器
