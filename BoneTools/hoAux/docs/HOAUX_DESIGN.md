...
UI 不提供 L/R 开关。每次预览和生成都严格读取模块要求的两根角色骨名称：两者必须都以 `.L/.R`、`_L/_R` 或 `-L/-R` 结尾，并且解析到同一侧；无法判侧或侧别冲突立即报错。角色骨不要求互为父子。开启“同时处理对称侧”时，使用 Blender 翻名规则取得并验证对侧两骨；对侧缺失时不静默跳过。预览同时绘制两侧，生成对两侧执行同一组参数并在失败时清理本次已生成侧。
...
1. 父骨方向与下段骨方向确定弯曲平面和弯曲角；
2. 下段骨起点为关节原点，下段骨方向为纵轴；
3. 弯曲平面的外侧确定凸角基准轴；
4. 下段骨静置矩阵的 local X/Z 提供自身 roll，`rollFollow` 控制从纯凸角基准到跟随骨骼 roll 的比例；
5. `twistOffset` 在上述结果上提供模块级人工修正；
6. 仅当弯曲角低于 `straightThreshold` 时，才退回下段骨静置局部轴。

JointFrame 的凸角轴、roll 跟随和直线退化规则属于模块实现，不暴露给用户。体积保持模块只允许公共 TRK/DEF 长度与整体扭转偏移；肩部额外允许 X0 特殊方向角。旋转约束 influence、Driver 响应角、目标 head/tail、头部偏移和不同 marker 的独立缩放全部固定。Twist 只暴露分段数、头端旋转影响和尾端旋转影响。
...
生成器会临时进入 Object/Edit 模式创建数据，但事务结束后必须恢复调用前的 Object、Pose 或 Edit 模式，不能固定停在 Object 模式。
